...
**Prior research:** `research/telegram/{telegram_integration,telegram_improvements,slash_command_migration}.md`. None for gchat yet.
...
- `sase-gchat/src/sase_gchat/credentials.py:1-44` — `get_space_id()`, `get_gchat_bin()` (default `"gchat"`), `get_rate_limit()`.
...
- Both honor the `silent` flag identically (`sase-gchat/src/sase_gchat/outbound.py:43`, `sase-telegram/.../outbound.py:44`).
...
- Google Chat entry point: `sase-gchat/src/sase_gchat/scripts/sase_gc_inbound.py` (582 LOC). Dot-command parsing, message-strike on TUI dismissal via `find_externally_handled` → `_strike_message`, per-thread `awaiting_feedback` state.
...
### R9 — Create `sase-gchat/ROADMAP.md`
...
- Investigate whether gchat CLI exposes anything like a streaming/draft-message API (the equivalent of `sendMessageDraft` discussed in `research/telegram/telegram_improvements.md`). If not, document the absence so it stops coming up.
...
**sase-gchat:**
- Inbound script: `src/sase_gchat/scripts/sase_gc_inbound.py` (582 LOC)
- Outbound script: `src/sase_gchat/scripts/sase_gc_outbound.py` (185 LOC)
