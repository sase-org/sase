...
| 2 | sase-google missing 3 VCS dispatch hooks | Critical | ../sase-google/ (complex) |
...
2. **`vcs_create_proposal(payload, cwd)`** — Delegates to `vcs_create_commit` (same as bare_git, since "proposal" has
   no distinct meaning for GitHub).
...
## Phase 4: sase-google VCS dispatch hooks
...
This is the most complex gap. The old sase-google workflows (`sase_google_commit_workflow`, `sase_propose_workflow`,
`sase_cl_workflow`) contained substantial Google-internal logic: `hg amend`, `hg addremove`, `hg commit --name`,
`hg fix`, `hg upload tree`, branch/CL number resolution, etc. These scripts still exist in the sase-google repo and can
be used as reference.

### Implementation

**File: `../sase-google/src/sase_google/plugin.py`** — Add 3 `@hookimpl` methods:

1. **`vcs_create_commit(payload, cwd)`** — Port from `sase_google_commit_workflow`:
...
5. **Phase 4** (sase-google hooks) — Most complex, may need dedicated attention
