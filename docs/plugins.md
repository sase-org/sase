...
| `sase_llm`        | LLM provider plugins (Claude, Codex, Gemini, Qwen, OpenCode, Jetski) | `sase-google`                |
...
Core sase ships Claude, Codex, Gemini, Qwen, and OpenCode providers as built-in entry points. The Jetski provider ships
with the `sase-google` plugin — don't reach for "add it to core" as a fix; its skill files are intentionally deployed
under `~/.gemini/jetski/` rather than `~/.jetski/` because it shares the Gemini CLI parent directory by design.
