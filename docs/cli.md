...
| `sase flag` / `flag list`               | Inspect registered feature flags, resolved values, provenance, `flag` task beads, and removal countdowns; `flag new` scaffolds a flag and its typed removal bead from three required prose arguments.                     | [Configuration](configuration.md#feature_flags), [Beads](beads.md#flag-bead-lifecycle)                               |
