...
| `sase validate`                     | Run portable memory/repo/skills initialization checks plus SDD frontmatter validation.                                                                                                                                 | [Initialization](init.md), [SDD](sdd.md)                                                                             |
...
| `sase artifact` / `artifact list`   | List indexed artifacts with kind, project, agent, `--since`, and query filters; bare `sase artifact` defaults to `list`. Aliased as `sase artifact-file`. ACE browses the same index on the Artifacts → Files sub-tab. | [Files sub-tab](ace.md#files-pane) · [Configuration CLI flags](configuration.md#sase-artifact)                       |
| `sase artifact create`              | Move an explicit file into persistent agent artifact storage and print its durable `file:` ref; agent-only (`SASE_AGENT=1`).                                                                                           | [Agent attachments](agent_images.md#explicit-artifact-contract)                                                      |
| `sase artifact doctor`              | Report artifact-index health; `-f` backfills `sha256`/`size_bytes`/`mime_type` and `-v` re-hashes stored files.                                                                                                        | [Configuration CLI flags](configuration.md#sase-artifact)                                                            |
| `sase artifact open`                | Resolve any artifact reference and open it with the viewer matching its kind and mime type.                                                                                                                            | [Configuration CLI flags](configuration.md#sase-artifact)                                                            |
| `sase artifact path`                | Print the single absolute filesystem path an artifact reference resolves to.                                                                                                                                           | [Configuration CLI flags](configuration.md#sase-artifact)                                                            |
| `sase artifact show`                | Show one artifact reference's metadata and resolution report, or a JSON envelope with `-j`.                                                                                                                            | [Configuration CLI flags](configuration.md#sase-artifact)                                                            |
...
| `sase var set`                      | Attach named string output variables from assignments, literal text, files, or stdin.                                                                                                                                  | [XPrompt variables](xprompt.md#cross-agent-output-variables)                                                         |
