...
| `sase artifact` / `artifact list`   | List indexed artifacts with kind, project, agent, `--since`, `--unused`, and query filters; bare `sase artifact` defaults to `list`. Aliased as `sase artifact-file`. ACE browses the same index under Artifacts → Files. | [Files pane](ace.md#files-pane) · [Configuration CLI flags](configuration.md#sase-artifact)                          |
