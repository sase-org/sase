...
| `sase chat show`                | Show one chat transcript by agent name, path, or basename; `-x` prints its stored pre-expansion XPrompt and `-r` its stored provider-prompt representation.                                      | [Stored prompt renderings](xprompt.md#stored-prompt-renderings) |
...
| `sase bead work`                             | Launch an epic's phase/land agents or one standalone task worker.                                            | [Beads](beads.md#sase-bead-work-target)                           |
...
| `sase agent prompts`                | Browse and validate the canonical agents-sidecar prompt archive. `show` prints the primary body, `show --rendered` prints the stored provider-prompt representation, and `migrate --write` moves historical plans-sidecar prompts into `prompts/<YYYYMM>/`. | [Agent Hood Synchronization](agents_sidecar.md#prompt-and-artifact-archive)                                          |
