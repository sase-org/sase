...
| `sase chat show`                | Show one chat transcript by agent name, path, or basename; `-x` prints its stored XPrompt prompt and `-r` its rendered provider prompt.                                                          | [Stored prompt renderings](xprompt.md#stored-prompt-renderings) |
...
| `sase agent prompts`                | Browse and validate the canonical agents-sidecar prompt archive. `show` prints the XPrompt body, `show --rendered` prints the final provider prompt, and `migrate --write` moves historical plans-sidecar prompts into `prompts/<YYYYMM>/`. | [Agent Hood Synchronization](agents_sidecar.md#prompt-and-artifact-archive)                                          |
