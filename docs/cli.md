...
| `sase artifact` / `artifact list`   | List indexed artifacts with kind, project, agent, `--since`, `--unused`, and query filters; bare `sase artifact` defaults to `list`. Aliased as `sase artifact-file`. ACE browses the same index on the Artifacts → Files sub-tab. | [Files sub-tab](ace.md#files-pane) · [Configuration CLI flags](configuration.md#sase-artifact)                       |
...
| `sase artifact prune`               | Plan retention with durable-reference and consumption protection; `--apply` moves selected automatic rows into restorable trash and refuses if a required protection source is unavailable.                                        | [Artifact consumption](agent_images.md#artifact-consumption-ledger)                                                  |
| `sase artifact reclaim`             | Plan conversion of stored automatic rows to verified VCS-backed identities; consumed and durably referenced rows stay protected, and mutation requires `--apply`.                                                                  | [Artifact consumption](agent_images.md#artifact-consumption-ledger)                                                  |
...
| `sase artifact stats`               | Report artifact-store economics, protection-source counts and availability, trash occupancy, and the default retention plan.                                                                                                       | [Artifact consumption](agent_images.md#artifact-consumption-ledger)                                                  |
