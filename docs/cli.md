...
| `sase artifact` / `artifact list`   | List indexed artifacts with kind, project, agent, `--since`, and query filters; bare `sase artifact` defaults to `list`. Aliased as `sase artifact-file`.             | [Configuration CLI flags](configuration.md#sase-artifact)                                                            |
