...
| `sase agent sync`               | Import shared history, publish eligible hoods, and drain Referenced By write-backs; `--check --refresh` fetches fresh status, while `--retry-quarantined` and `--drop-retired` handle stopped publication requests.                                                                                                                                                                                                                                                                                                                                                                                  | [Agent hood synchronization](agents_sidecar.md)                          |
...
| `sase memory write`                          | Agent-side proposal for human-reviewed reference memory; `--notify` can add an inbox item.                                                            | [Memory](memory.md#propose-memory)                                |
...
| `sase memory log`                            | Summarize audited memory reads; `--include proposals` also shows proposal/review events, `--include glossary` folds in legacy pre-web glossary reads. | [Memory](memory.md#audited-reads)                                 |
...
| `sase bead show`                             | Show one or more issues; `<epic-id>..` expands the epic plus its direct children, and long terminal output can use the shared [SASE Pager](pager.md). | [Beads](beads.md#cli-commands)                                    |
...
| `sase launch request`                        | Register a launch gate; agent callers hand off to a gate shell, while non-agent callers can wait for JSON.                                            | [Agent groups](agent_families.md#agent-initiated-family-launches) |
