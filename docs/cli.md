...
`sase snippet`, `sase stitch`, `sase telemetry`, `sase var`, `sase workspace`, and
`sase xprompt`. Nested groups such as `sase agent prompts`, `sase agent tribe`,
`sase artifact trash`, `sase axe chop`, `sase axe lumberjack`, `sase bead dep`,
`sase bead ref`, `sase bead task-type`, `sase memory agent-docs`, `sase patch ref`,
...
| `sase memory show`                           | Print one reference memory file without recording an audited read.                                                                | [Memory](memory.md#show-a-note)                                   |
| `sase memory read`                           | Agent-side read of one reference memory file with an attributable audit event.                                                    | [Memory](memory.md#audited-reads)                                 |
...
| `sase glossary add`                          | Add a glossary term, then regenerate agent instruction files unless `-I`.                                                         | [Glossary](memory.md#glossary)                                    |
| `sase glossary del`                          | Delete a glossary term by name, alias, or prefix; prints the restore command.                                                     | [Glossary](memory.md#glossary)                                    |
| `sase glossary` / `sase glossary list`       | List a project's configured glossary terms, filtered by an optional substring.                                                    | [Glossary](memory.md#glossary)                                    |
| `sase glossary all`                          | Print every glossary term in full, alphabetically, with aliases, definitions, and inbound references.                             | [Glossary](memory.md#glossary)                                    |
| `sase glossary show`                         | Print one or more glossary terms plus the recursive closure of terms their definitions depend on. Pass every term in one command. | [Glossary](memory.md#glossary)                                    |
| `sase glossary read`                         | Agent-side audited read of one or more glossary terms in one command; requires `-r/--reason`.                                     | [Glossary](memory.md#glossary)                                    |
| `sase glossary log`                          | Summarize audited glossary reads by term, by agent, and by event.                                                                 | [Glossary](memory.md#glossary)                                    |
...
| `sase artifact link` / `link list`      | Add, list, migrate, and remove typed artifact links. `link add` takes `<source-ref> <relation> <target-ref> <why>` and `link list` can filter by direction, origin, relation, limit, or JSON.                                                                                                                                                                     | [Artifact Links](artifact_links.md)                                                                                  |
