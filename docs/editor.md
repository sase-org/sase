...
| Artifact references     | Fuzzy-completes bare `@` and `@query` tokens as artifact kinds, adding local paths on a kind-prefix miss or manual completion request, then completes local payloads after `@kind:`. |
