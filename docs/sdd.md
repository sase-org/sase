...
| ------------------------------ | ----------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sdd.bead_refresh.mode`        | string      | `background` | `background` launches TTL-gated command refreshes, `blocking` pulls before commands, and `off` also disables the axe bead-wait refresh chop and the waiting runner's fallback.                                                        |
