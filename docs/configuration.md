...
| `log`      | `-a/--author`, `-b/--branch/--ref`, `-c/--color`, `-o/--current-only`, `-F/--fetch`, `-f/--format pretty\|full\|oneline\|json`, `-n/--limit`, `-N/--no-fetch`, `-T/--no-tags`, `-r/--repo`, `-R/--reverse`, `-s/--since/--after`, `-u/--until/--before` | Show a merged commit timeline with local/remote presence markers.                                |
...
| `sase plan` / `sase plan list`  | `-j/--json`                                                                                                                   | List pending proposals, recent approvals, and inferred rejected rows. |
