...
slow markers in the [Agents Tab LLM Calls Panel](ace.md#agents-tab-llm-calls-panel). A
...
| `llm_provider.usage_limit.honor_usage_windows`                      | bool      | `true`   | When no reset hint is found, fall back to a corroborated reset from the collected usage-window data (see [Usage-Limit Auto-Disable](llms.md#usage-limit-auto-disable)) before using the flat `disable_seconds`.                                                                                                                                                                                                                                                                     |
...
[Muse Code subscription usage](agent_providers.md#muse-subscription-usage).
...
| `llm_provider.usage_metrics.refresh_seconds`                          | number | `300`                           | Background refresh cadence in seconds. Must be finite and at least `60`. Automatic probes never run faster than the provider's plugin floor (`max(refresh_seconds, floor)`); display freshness uses the same max. Explicit refreshes bypass the floor but still respect the 60 s cooldown and any `Retry-After`. |
...
| `sase notify rules`        | `-e/--explain ID`, `-j/--json`                                                                | List merged [delivery rules](notifications.md#delivery-rules) in evaluation order, or explain one row's toast and sound |
