...
| `relations_expanded` | bool | `false`   | Whether the [Artifacts relation panel](ace.md#navigation-in-agent-stitches-beads-provider-documents-and-files) starts expanded; `.` (`toggle_relation_panel`) toggles it in memory for the current session only.                         |
...
| `sase gate answer` | `-i/--id`, `-k/--kind`, repeatable `-o/--option`, `-s/--set`, `-O/--option-input`, `-f/--feedback`, `--detach` / `--no-detach`, `--resume` / `--restart`, `-j/--json` | Answer one branch; shell gates run selected commands detached by default                  |
...
[Artifact Links](artifact_links.md#health-and-recovery).
...
| `index`     | `status` / `rebuild` / `verify` / `gc` / `repair` / `vacuum`, `-i/--index-path`, `-p/--projects-root`, `-j/--json`; `repair` and `vacuum` accept `-a/--apply`                                        | Maintain the persistent artifact index. `status` is a lightweight check, `verify` compares source artifacts, `gc` rebuilds the index and dismissed projection, `repair` handles invalid future-dated import state, and `vacuum` reports or reclaims SQLite freelist pages.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `names`     | `forget-import`, `migrate-auto`, `purge-local-state`, `-f/--force`, `-j/--json`; `forget-import` also takes `<machine>`, `-a/--apply`, `-t/--transport`; `purge-local-state` also takes `-a/--apply` | Maintain the permanent agent-name registry. `migrate-auto` runs the historical generated-name namespace migration; `--force` reruns it after the completion marker exists. `forget-import <machine>` removes that machine's fully-superseded legacy v1 import closure (artifacts, chats, dismissed bundles, identities, receipts); a dry run unless `-a/--apply` is given. `purge-local-state` generalizes that sweep to every locally materialized import closure regardless of transport or source machine (artifacts, chats, dismissed bundles, identities, import journals/staging, the incoming cache, and receipts); also a dry run unless `-a/--apply` is given. `-j/--json` emits a machine-readable summary for any subcommand. |
...
`sase agent index vacuum` is also a dry run unless `-a/--apply` is supplied. It reports
freelist pages left by normal SQLite deletes and dismissed-identity row counts. Apply
runs SQLite `VACUUM` to rebuild the index file and reclaim that free space; it never
removes or alters an index row. `-i/--index-path` selects another index and `-j/--json`
