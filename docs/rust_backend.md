...
separate-repo mode, the repository-root `events/**` in a schema-3 `--beads` sidecar, and `beads/events/**` in a schema-2
split `--plans` sidecar) when present; `issues.jsonl` is regenerated as a compatibility projection, and `beads.db` is a
