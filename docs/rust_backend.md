...
separate-repo mode, and `beads/events/**` in the split `--plans` sidecar) when present; `issues.jsonl` is regenerated as
a compatibility projection, and `beads.db` is a compatibility cache. Event reduction, JSONL/config parsing, cache
