...
| `,r`       | Revert focused or marked agent commits, including suffix-strategy linked repos                |
...
focused agent name. The preview includes the primary workspace plus existing `linked_repos` entries that use the
numbered-workspace suffix strategy; static linked repos are not part of this action. Each repository is checked before
execution, and a dirty or non-git linked repo is reported and skipped while clean repositories can still be reverted.
Successful execution creates one revert commit per repository, pushes when a remote tracking branch is available, and
writes `revert_result.json` beside the agent artifacts.
...
- **PLAN REJECTED** — The plan was rejected. A plain rejection from ACE or `sase plan reject` writes the rejection
  response, dismisses the notification, user-kills the matching planner, and persists dismissed-agent state so the row
  is hidden on refresh. Rejected archived plans can still appear in history-oriented views, and redundant completion
...
`sase plan reject <id-prefix>` to write the same response protocol used by the TUI modal. If the selector is omitted,
the CLI acts only when exactly one proposal is pending. `approve` starts the coder without committing an SDD plan,
`tale` commits the plan as an SDD tale and starts the coder, `epic` and `legend` commit the matching SDD tier and launch
the bead follow-up, and `commit` records the approved plan in SDD without launching a coder. `-m/--model` picks the
follow-up agent's model, while `-p/--prompt` adds extra coder instructions for the `approve` and `tale` paths. CLI
rejection also runs the durable planner cleanup used by plain TUI rejection.
...
| `Ctrl+G s`     | Stash every non-empty pane                                                                    |
| `Ctrl+G S`     | Overwrite a pinned stashed prompt with the current draft                                      |
| `Ctrl+G x`     | Save the current draft as a reusable xprompt                                                  |
...
| `gS`        | Overwrite a pinned stashed prompt with the current draft, leaving the bar open                         |
| `gx`        | Save the current draft as a reusable xprompt, leaving the bar open                                     |
...
stashed-prompt picker instead. `gs` captures all non-empty panes in their current order and dismisses the bar. `gS`
opens an update flow for an existing pinned stash and overwrites the chosen row with the current non-empty panes. `gx`
opens a save-as-xprompt flow: choose an existing xprompt target to overwrite or create a new Markdown/config xprompt,
then ACE writes the draft body (joining multiple panes with `---`) and offers to commit the changed file when
applicable.
