# Problems with Generalizing sase for Claude Code + Git/GitHub

Critique of the design in `plans/sase_git_claude_plan.md` and its implementation so far.
...
**Hg/Google model (what sase assumes):**
...
returns `(True, None)` for both — silent no-ops. This means after `sase commit` in a git repo:
...
- Claude Code is the _outer loop_, not something you shell out to. You'd be running Claude Code, which runs sase, which
