...
[SASE: Structured Agentic Software Engineering](structured-agentic-software-engineering.md) makes that argument. The two
pages can be read in either order; this one runs first and names the parts afterward.
...
- **Artifacts** — four focused sub-tabs for PRs, commits, bugs, and plans. The PRs view contains every ChangeSpec on the
  ...
  hooks, comments, and mentor activity all in one place. The [ChangeSpec guide](../../change_spec.md) goes deeper when
  you're curious. This first read-only run should not have created one yet; editable committed work is where ChangeSpecs
  appear.
...
- [SASE: Structured Agentic Software Engineering](structured-agentic-software-engineering.md) — the conceptual front
  door, for when you want the _why_ to match the _how_.
