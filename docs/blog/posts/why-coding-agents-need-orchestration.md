...
- **Artifacts**: Commits, Beads, Bugs, PRs, and Files top-level views, with Plans, Chats, and Other nested under Files.
  The PRs view owns ChangeSpec status, hooks, comments, mentor output, diffs, file deltas, mail/submit flows, rewind,
  revert, restore, and archive operations; Chats browses transcripts with local/shared/remote sync provenance.
