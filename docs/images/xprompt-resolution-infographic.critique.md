...
3. **`%directives` chip says "append, override, repeat" — those are not the salient categories.** The actual functional
   categories of directives are launch overrides (`%model`, `%name`, `%group`, `%hide`, `%approve`, `%plan`, `%epic`),
   scheduling (`%wait`, `%time`), and fan-out (`%alt`/`%(`, `%repeat`, `%m(...)`). "Append, override, repeat" picks
   three behaviors that don't map cleanly to the directive surface in `docs/xprompt.md` and isn't a useful taxonomy for
   a new reader.
