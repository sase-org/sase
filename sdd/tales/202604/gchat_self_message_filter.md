...
description: Stop legacy-chat-integration from launching agents in response to its own outbound
...
# legacy-chat-integration — Stop Self-Triggering Agent Launches

## Problem

legacy-chat-integration is launching agents in response to messages **sase itself just posted** to the chat space. The user's
...
- All sase → chat traffic flows through one of three message-creating wrappers in `legacy_chat_integration/gchat_client.py`:
...
- The existing storage pattern lives in `legacy_chat_integration/pending_actions.py`: a single JSON file under `~/.sase/gchat/`,
...
1. legacy-chat-integration must never treat a message it just sent as a fresh inbound user prompt — for any payload type
...
- No changes to the main `sase_100` repo. All edits live in the sibling `legacy-chat-integration` plugin repo at
...
### New module: `legacy_chat_integration/self_messages.py`
...
Add to `legacy-chat-integration/tests/test_inbound.py` and `legacy-chat-integration/tests/test_gchat_client.py` (and a new
...
1. Edits made in the sibling `legacy-chat-integration` repo workspace; run `just install && just check` in that workspace before
   declaring done.
