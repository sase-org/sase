...
When a sase agent on the Google/hg machine commits to an **existing** CL (via `legacy_mercurial_plugin_amend`), the file panel on
...
The `hg.yml` diff step (legacy-mercurial-plugin, line 72) runs `hg diff -c .` after the commit. In Mercurial, `hg amend` replaces
...
### Phase 1: Prefer pre-commit diff in `hg.yml` diff step (legacy-mercurial-plugin)

**File**: `../legacy-mercurial-plugin/src/legacy_mercurial_plugin/xprompts/hg.yml` (diff step, lines 61-91)
...
**File**: `../legacy-mercurial-plugin/tests/test_hg_xprompts.py` (or equivalent test file)
