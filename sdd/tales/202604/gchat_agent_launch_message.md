...
description: Send a "🚀 Launched" message to Google Chat when legacy-chat-integration's inbound
...
- The gchat counterpart `_launch_single_agent()` lives at
  `legacy-chat-integration/src/legacy_chat_integration/scripts/sase_gc_inbound.py:302-326`. It calls `launch_agent_from_cwd(prompt)` and only logs
  on exception — no message is sent to chat.
- The `gchat` CLI exposes `send_message(space, text, *, thread=None, markdown=True)` (see
  `legacy-chat-integration/src/legacy_chat_integration/gchat_client.py:137`). It supports CommonMark markdown and threading but **no inline
...
- `_format_workflow_complete()` in `legacy-chat-integration/src/legacy_chat_integration/formatting.py:228-283` already shows the established gchat
...
The change lives in the **sibling `legacy-chat-integration` plugin repo** (`/home/bryan/projects/github/sase-org/legacy-chat-integration/`), not
...
In `legacy-chat-integration/tests/test_inbound.py`, add cases mirroring the sase-telegram parity tests in
...
1. `test_launch_single_agent_posts_launched_message`: patches `sase.agent.launcher.launch_agent_from_cwd` to return a
   stub `AgentLaunchResult(workspace_num=7, pid=123)` and patches `legacy_chat_integration.gchat_client.send_message`; calls
...
Per `AGENTS.md`, run `just check` in the legacy-chat-integration workspace before declaring done. Smoke check by sending a free-text
