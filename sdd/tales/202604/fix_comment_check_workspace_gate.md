...
On machines using the `legacy-mercurial-plugin` plugin (Mercurial/CITC workflow), `sase axe` never adds a COMMENTS field to Mailed
...
(`../legacy-mercurial-plugin/src/legacy_mercurial_plugin/workspace_plugin.py:94`), which shells out to `legacy_mercurial_plugin_get_workspace <project> 1`.
...
(`../legacy-mercurial-plugin/workspace_plugin.py:92`), which takes only the changespec name — it looks up the CL number via the sase
...
### 3. No changes required in `legacy-mercurial-plugin` or `sase-github`
