...
status: wip
prompt: sdd/prompts/202604/parent_p4head_leak.md
---
...
hg-specific sentinel used by the sase-google plugin to mean "the main branch of the repo"; it is a **checkout target**,
...
1. `sase-google/src/sase_google/scripts/sase_split_setup:75` — when the CL being split has no parent ChangeSpec, this
   ...
   `sase_google_update`) _and_ the intended **ChangeSpec parent name** (for `sase commit -p`). The two concepts are not
   interchangeable.

2. `sase-google/src/sase_google/xprompts/split.yml:41, 50` — the workflow pipes `default_parent` into both
   `sase_split_prepare_execute` and the `split_executor` agent xprompt.

3. `sase-google/src/sase_google/xprompts/split_executor.md:26, 36-38` — the executor uses `{{ default_parent }}` in two
   places:
   - As the argument to `sase_google_update` (correct use — a checkout target).
...
So the bug has two distinct layers: a **caller bug** in sase-google conflating checkout-target with
...
Two-part fix keeps VCS-specific knowledge in sase-google and makes sase_100 reject bogus `PARENT` values.

### Part 1: Separate checkout target from ChangeSpec parent in sase-google
...
- `default_checkout_target` (VCS ref, falls back to the hg-specific `"p4head"`) — consumed by `sase_google_update`.

Changes:

| File                                                             | Change                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sase-google/src/sase_google/scripts/sase_split_setup`           | Split the current `default_parent` into two variables. `default_parent = target.parent if target and target.parent else ""`. `default_checkout_target = target.parent if target and target.parent else "p4head"`. Print both. Keep `parent_submitted` logic intact (still resolved via the CS name, which is now empty when no parent). |
| `sase-google/src/sase_google/xprompts/split.yml`                 | Add `default_checkout_target: word` to the `setup` step's output schema. Pass it to `sase_split_prepare_execute` via a new `--default-checkout-target` flag, and to the `split_executor` xprompt as a new `default_checkout_target` input.                                                                                              |
| `sase-google/src/sase_google/scripts/sase_split_prepare_execute` | Add a `--default-checkout-target` arg. Use it (not `--default-parent`) for the `sase_google_update` call on line 106.                                                                                                                                                                                                                   |
| `sase-google/src/sase_google/xprompts/split_executor.md`         | Accept both `default_parent` and `default_checkout_target`. In step 1, navigate using `default_checkout_target`. In step 4, pass `-p <parent>` only when a parent ChangeSpec name is present; omit the flag entirely when the entry has no `parent` AND `default_parent` is empty.                                                      |
| `sase-google/tests/*`                                            | Extend tests for `sase_split_setup` and `sase_split_prepare_execute` to cover both the "target has parent" and "target has no parent" cases, asserting that `default_parent` is empty (not `"p4head"`) in the latter.                                                                                                                   |

Why this is VCS-agnostic: the hg-specific string `"p4head"` is kept entirely inside `sase-google`, never crossing the
process boundary into `sase commit` as a would-be ChangeSpec name. Conceptually, sase-google keeps a private
...
3. **Manual smoke test (hg, sase-google)**: run the `#split` xprompt on a CL that has no parent; each generated child
...
4. `just check` in both `sase_100` and `sase-google` passes.
