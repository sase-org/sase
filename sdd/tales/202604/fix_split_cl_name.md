...
The `#split` workflow (`split.yml` in legacy-mercurial-plugin) uses `{{ cl_name }}` in its first step's bash command, but `cl_name`
...
- No changes needed in legacy-mercurial-plugin's `split.yml` (it correctly expects `cl_name` from context)
