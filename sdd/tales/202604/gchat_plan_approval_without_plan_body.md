...
- `../legacy-chat-integration/src/legacy_chat_integration/formatting.py` formats `PlanApproval` notifications in `_format_plan_approval()`.
...
1. Modify `../legacy-chat-integration/src/legacy_chat_integration/formatting.py`.
...
2. Update Google Chat tests in `../legacy-chat-integration/tests/test_formatting.py`.
...
3. Review docs in `../legacy-chat-integration/README.md` and `../legacy-chat-integration/docs/outbound.md`.
...
5. Verification.
   - Run targeted Google Chat tests first, especially `tests/test_formatting.py` and the plan attachment integration
     tests.
   - Run `just check` in `../legacy-chat-integration` because the plugin repo instructions require it after modifications.
