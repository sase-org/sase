...
A Codex agent edited the sibling `sase-gchat` repository from a `sase` workspace and finished without being prompted to
...
can pass whenever the main workspace is clean, even if a primary sibling repo such as `../sase-gchat` is dirty.
...
4. Verify the hook actually blocks for dirty `sase-gchat`-style sibling repos and still skips ephemeral workspaces.
...
sibling repos like `sase-gchat`. The failure mode from the 2026-04-25 11:48 run is closed because Codex now runs the
