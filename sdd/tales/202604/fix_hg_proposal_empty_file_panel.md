...
In `sase ace`, when a Mercurial agent (Gemini + retired-hg-plugin plugin) successfully creates a proposal via the
...
1. **During agent execution**, `/sase_hg_commit` → `sase commit` runs. Inside
   ...
   - The proposal is then created via the retired-hg-plugin plugin's `vcs_create_proposal` hook, and the workspace is cleaned.

2. **After the agent finishes**, the `#propose` embedded workflow's post-steps run. The plugin's
   `../retired-hg-plugin/src/retired_hg_plugin/xprompts/propose.yml` overrides the in-repo `src/sase/xprompts/propose.yml`. The
...
- Is overridden by the retired-hg-plugin plugin copy on hg workspaces.
...
1. On Mercurial / retired-hg-plugin, the file panel populates after `/sase_hg_commit` creates a proposal.
...
4. Keep the two propose.yml files (in-repo vs. retired-hg-plugin plugin) consistent in spirit so future edits don't drift.
...
#### 1. retired-hg-plugin plugin: `../retired-hg-plugin/src/retired_hg_plugin/xprompts/propose.yml`
...
The current retired-hg-plugin `propose.yml` calls `sase_propose_workflow --create` unconditionally when `has_changes` is true.
...
| File                                                  | Type                              |
| ----------------------------------------------------- | --------------------------------- |
| `../retired-hg-plugin/src/retired_hg_plugin/xprompts/propose.yml` | rework gates + split commit path  |
...
| `../retired-hg-plugin/tests/xprompts/test_propose_yml.py`   | new/extended coverage             |

## Post-Implementation

- Run `just check` in `sase_100` (required before reporting).
- Run `just check` in `../retired-hg-plugin` (plugin repo).
