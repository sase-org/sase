...
workspaces via the legacy-mercurial-plugin plugin. Instead of showing the specific commit entry message (e.g., "Restore
...
The `hg.yml` xprompt in legacy-mercurial-plugin has a `diff` step that runs as a `finally` step after every agent run. When it
...
**File: `legacy-mercurial-plugin/src/legacy_mercurial_plugin/xprompts/hg.yml`** (the `diff` step)
