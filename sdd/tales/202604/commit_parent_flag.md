...
`#split` xprompt workflow in sase-google.
...
In the sase-google split workflow specifically, the executor navigates to the parent CL via
`sase_google_update <parent>` (which calls `hg update` + `clear_branch_name`) before running `sase commit`. The
...
### Phase 2: Update split executor to pass parent (sase-google repo)
...
- `src/sase_google/xprompts/split_executor.md` — Change the `sase commit` invocation in step 4 to include `-p <parent>`,
...
- **In scope**: Adding the `-p`/`--parent` flag, improving error handling in auto-detection, updating the sase-google
  split executor
- **Out of scope**: Fixing the `branch_name` / `clear_branch_name` behavior in sase-google (the explicit flag makes this
  unnecessary)
