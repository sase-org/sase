...
is especially visible on machines using the adjacent `legacy-mercurial-plugin` plugin for Mercurial/Fig workspaces.
...
2. The `legacy-mercurial-plugin` Mercurial VCS plugin implements committed and uncommitted diff helpers, but does not implement
...
The result is: core does not request a DELTAS refresh for newly-created ChangeSpecs, and even if it did, `legacy-mercurial-plugin`
...
3. Add Mercurial name-status support in `../legacy-mercurial-plugin`.
...
4. Add `legacy-mercurial-plugin` tests.
...
5. Verify.
   ...
   - Run focused `legacy-mercurial-plugin` hg plugin tests.
   - Because both repos will be modified, run `just check` in `sase_100` and `../legacy-mercurial-plugin` after `just install` if
     needed.
