...
- Google Chat lives in `../legacy-chat-integration`.
  - Google Chat cannot render buttons, so the formatter renders numbered options.
  - The visible option label is in `src/legacy_chat_integration/formatting.py` as `🚀 Run`.
...
3. Update tests.
   ...
   - In `../legacy-chat-integration`, update tests and fixtures that assert or persist option labels while preserving payload
     assertions for `run`.
...
5. Verify.
   - Run focused tests first:
     ...
     - `just test tests/test_formatting.py tests/test_inbound.py tests/test_integration.py` in `../legacy-chat-integration`
