...
The provided `sase ace` snapshot is from the Google/Mercurial workflow. The `sase-google` plugin currently implements
...
1. Add Mercurial line-stat support in `../sase-google/src/sase_google/plugin.py`.
...
3. Add plugin unit tests in `../sase-google/tests/test_hg_plugin.py`.
...
5. Verification.
   ...
   - Run focused `sase-google` plugin tests.
...
- This task spans the core repo and the `sase-google` plugin repo. Avoid modifying generated skill files or CLI
