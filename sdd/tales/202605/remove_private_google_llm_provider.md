...
Remove repo-local references to the Google-private LLM provider that now belongs only in the private `../retired-hg-plugin`
...
- `README.md`: supported-agent table and LLM provider feature summary name the private provider as available through
  `retired-hg-plugin`.
- `docs/plugins.md`: plugin group examples, `retired-hg-plugin` package description, entry-point table, and LLM plugin section
...
- `memory/short/gotchas.md`: contains provider-specific operational guidance and a generic YAML example using the
  private name.
- `memory/long/external_repos.md`: lists the private provider as part of `../retired-hg-plugin`.
...
1. Preserve the plugin architecture while removing private-provider specifics.
   ...
   - Keep `retired-hg-plugin` documented as the Mercurial/GitHub-internal VCS/config/xprompt plugin where appropriate, but do
...
- Do not edit `../retired-hg-plugin` unless verification proves this repo depends on a cross-repo change.
