...
Remove repo-local references to the Google-private LLM provider that now belongs only in the private `../sase-google`
...
- `README.md`: supported-agent table and LLM provider feature summary name the private provider as available through
  `sase-google`.
- `docs/plugins.md`: plugin group examples, `sase-google` package description, entry-point table, and LLM plugin section
...
- `memory/short/gotchas.md`: contains provider-specific operational guidance and a generic YAML example using the
  private name.
- `memory/long/external_repos.md`: lists the private provider as part of `../sase-google`.
...
1. Preserve the plugin architecture while removing private-provider specifics.
   ...
   - Keep `sase-google` documented as the Mercurial/GitHub-internal VCS/config/xprompt plugin where appropriate, but do
...
- Do not edit `../sase-google` unless verification proves this repo depends on a cross-repo change.
