...
`install_legacy_mercurial_plugin`.
...
- `~/.local/share/chezmoi/home/dot_config/sase/sase_work.yml`
  - set the work command, after confirming the spelling of `install_sase_gopgle` vs `install_legacy_mercurial_plugin`
...
- `legacy-chat-integration`: `just install`, targeted inbound tests, then `just check`.
...
`install_sase_gopgle`; if that is literal, configure it as written. If it is a typo, use `install_legacy_mercurial_plugin`.
