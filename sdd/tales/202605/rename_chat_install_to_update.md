...
- `../legacy-chat-integration`
  - Parses dot commands in `src/legacy_chat_integration/inbound.py`.
  - `_DOT_COMMANDS` contains `install`; `process_dot_command(".install")` returns `DotCommand("install", [])`.
  - Dispatches `cmd.command == "install"` in `src/legacy_chat_integration/scripts/sase_gc_inbound.py`.
...
- Run checks:
  ...
  - In `../legacy-chat-integration`: `just install`, targeted inbound tests, then `just check`.
