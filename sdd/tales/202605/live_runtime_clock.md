...
The requested UI change is presentation-only: add a small clock emoji to the left of any runtime that is actively being
...
Rows whose runtime suffix is live-ticking render the suffix with a clock marker immediately before the elapsed duration:

- Running row: `🕒 38m45s`
- Waiting row that has already started running: `🕒 5m`
- Retrying row: `🕒 12s`
- Approved workflow parent whose visible runtime is still advancing: `🕒 1m`
- Running workflow child with `step_type == "agent"`: `🕒 2m05s`
...
timestamp and elapsed duration. The clock is visual chrome and belongs in the rendering layer.
...
plus a following space, for example `🕒 `, and all alignment tests should assert rendered suffix content rather than
...
1. Active rendering:
   - Change `test_format_agent_option_active_suffix_contains_only_elapsed` to expect the clock-prefixed live suffix.
...
3. Patch path:
   - Update active row patching assertions so before/after rows end with `🕒 59s` and `🕒 1m`.
