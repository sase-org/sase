...
- Because `.update` uses the same shared worker, add the same pending/delivery flow in `sase-gchat` either in the same
...
- If touched: in `sase-gchat`: `just test tests/test_inbound.py`

Then run required checks in each modified repo:

- In `sase`: `just check`
- In `sase-telegram`: `just check`
- In `sase-gchat` if modified: `just check`
