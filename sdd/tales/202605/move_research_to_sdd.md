...
---
# Move research/ under sdd/

## Goal

Move the repository-local research corpus from `research/` to `sdd/research/` and update references so current docs,
default prompts, tests, and historical SDD artifacts no longer point at the old root-level location.

## Current State

- `research/` currently contains date-sharded material under `202602/`, `202603/`, `202604/`, and `202605/`, including
  one PNG asset in `research/202604/`.
...
- Tests contain at least one fixture creating `workspace/research/example.md` and asserting attachment PDF names for
  that path.
...
1. **Move the directory as a single tree.**
   - Create `sdd/research/` by moving the existing `research/` directory there.
...
2. **Rewrite path references in tracked text files.**
   - Replace file and directory path references from `research/` to `sdd/research/`.
   ...
   - Update `@research/...` references to `@sdd/research/...`.
   - Update absolute or embedded design paths only when they include the repo-relative `research/...` segment.
...
3. **Update current behavior-facing documentation and prompts.**
   ...
   - Update `.prettierignore` from `research/` to `sdd/research/`.

4. **Adjust tests and expected generated paths.**
   - Update fixtures that create sample research files under `workspace/research/` so they use
...
5. **Verify no stale root path references remain.**
   - Run a targeted search for `research/` and inspect remaining hits.
   ...
   - Run a second search for `@research/`.
...
- All files previously under `research/` exist under `sdd/research/`.
- The root-level `research/` directory no longer exists.
- No tracked references to root-level `research/...` file or directory paths remain.
