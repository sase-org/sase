...
# Fix: hg amend newline failure in retired-hg-plugin vcs_create_commit
...
5. **retired-hg-plugin `vcs_create_commit`** (`plugin.py:355-365`) passes the entire multi-line message as a single note
   argument to `retired_hg_plugin_amend`:
   ```python
   note = f"[run] {message}" if message else "[run] Agent changes"
   out = self._run(["retired_hg_plugin_amend", note], cwd)
   ```
6. `retired_hg_plugin_amend` script passes note to `hg amend -n "${NOTE}"` -- hg rejects the newline
...
- Runtime: Gemini on Mercurial (retired-hg-plugin plugin)
...
### Change 1: `../retired-hg-plugin/src/retired_hg_plugin/plugin.py` -- `vcs_create_commit`
...
### Change 2: `../retired-hg-plugin/tests/test_hg_plugin.py` -- add test

Add a test that verifies multi-line messages only pass the first line to `retired_hg_plugin_amend`.
