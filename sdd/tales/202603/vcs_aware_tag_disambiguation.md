...
`legacy-mercurial-plugin/default_config.yml`, plugin-config priority) works for Mercurial.
...
Allow both sase-github and legacy-mercurial-plugin to define a `diff_file`-tagged xprompt, and resolve the correct one based on
...
- Plugin config xprompts: `"plugin_config:legacy_mercurial_plugin"`
...
2. **Add `sase_config` entry point** in `pyproject.toml`:
   ...
   (Mirrors how legacy-mercurial-plugin registers its `default_config.yml`.)
...
4. **Add `_extract_plugin_module()` helper**:
   ...
   def _extract_plugin_module(source_path: str | None) -> str | None:
       ...
       if source_path.startswith("plugin_config:"):
           # "plugin_config:legacy_mercurial_plugin" → "legacy_mercurial_plugin"
