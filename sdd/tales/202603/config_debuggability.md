...
@dataclass
class ConfigLayer:
    name: str              # e.g., "default", "plugin:retired_hg_plugin", "user", "overlay:sase_hg.yml", "local"
...
- In `load_merged_config()`, log each layer as it's loaded:
  ```
  DEBUG config.core: Loading layer 'plugin:retired_hg_plugin' from /path/to/default_config.yml (keys: mentor_profiles, xprompts)
