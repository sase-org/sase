...
tier: epic
---
# Remove Obsolete `legacy-chat-integration` and `legacy-mercurial-plugin` References

## Goal

Remove all tracked references to the obsolete `legacy-chat-integration` and `legacy-mercurial-plugin` repositories from:
...
literal sweep for `legacy-chat-integration`, `legacy-mercurial-plugin`, `legacy_chat_integration`, and `legacy_mercurial_plugin` should be clean across the in-scope repos
...
- `src/sase/integrations/agent_status_groups.py`: `# pyvision: https://github.com/sase-org/legacy-chat-integration.git`
...
Initial direct sweeps found no `legacy-chat-integration` / `legacy-mercurial-plugin` / `legacy_chat_integration` / `legacy_mercurial_plugin` matches in:
...
2. Do not edit the obsolete repos (`../legacy-chat-integration`, `../legacy-mercurial-plugin`) except possibly to inspect them. They are removal
...
3. Prefer replacing obsolete external API validation with retained non-obsolete consumers or in-repo documentation. For
   ...
   remove the `legacy-chat-integration` pragma.
4. If removing `legacy-mercurial-plugin` references exposes dead Mercurial-only test fixtures or docs, either generalize them to
...
1. Remove the `legacy-chat-integration` pyvision URI pragma from `src/sase/integrations/agent_status_groups.py`.
2. Run `just pyvision` early. If pyvision reports that `AgentStatusGroup`, `agent_status_bucket_glyph`, or
   ...
   - make unused symbols private or delete them only if surrounding code/tests prove they are not part of the current
     integration surface.
3. Update tests that use `legacy-chat-integration` merely as a dirty sibling repo fixture. Use a non-obsolete sibling name such as
   `sase-telegram` or a neutral fake repo name, depending on what the test is proving.
4. Update tests that mention `legacy-mercurial-plugin` / `legacy_mercurial_plugin` only as a plugin-discovery fixture. Prefer neutral synthetic
...
- No `legacy-chat-integration`, `legacy-mercurial-plugin`, `legacy_chat_integration`, or `legacy_mercurial_plugin` matches remain under `src/` or `tests/`.
- `just pyvision` no longer attempts to fetch or inspect `legacy-chat-integration` or `legacy-mercurial-plugin`.
...
2. Update `AGENTS.md` and `memory/long/external_repos.md` to remove `legacy-mercurial-plugin` and not mention `legacy-chat-integration`.
3. Update `README.md` plugin diagrams/lists to remove `legacy-mercurial-plugin`.
4. Update plugin docs:
   - `docs/plugins.md`: remove the `legacy-mercurial-plugin` package row and any `jetski` provider claims.
   - `docs/workspace.md`: remove obsolete Mercurial workspace plugin references.
   - `docs/vcs.md`: remove install instructions and package-specific references to `legacy-mercurial-plugin`; if Mercurial concepts
...
5. Update generated-skill or commit-workflow memory/docs only if they name `legacy-mercurial-plugin` / `legacy_mercurial_plugin`; do not remove
...
4. Do not touch `../legacy-chat-integration` or `../legacy-mercurial-plugin`.
5. Verification:
   ```bash
...
- No unapproved references to `legacy-chat-integration`, `legacy-mercurial-plugin`, `legacy_chat_integration`, or `legacy_mercurial_plugin` remain in the in-scope repos.
...
> in `sase_100`, especially the failing `legacy-chat-integration` pyvision pragma. Do not edit docs or SDD history in this phase. Run
...
> in `sase_100` so they no longer mention `legacy-chat-integration` or `legacy-mercurial-plugin`. Do not edit `src/`, `tests/`, or SDD history in
...
> run that repo's required checks. Do not edit `../legacy-chat-integration` or `../legacy-mercurial-plugin`.
