...
- Add `precommit_command` config field (set to `just fix` in sase repo, `sase_hg_fix` in retired-hg-plugin)
...
**retired-hg-plugin repo (../retired-hg-plugin):**

- `src/retired_hg_plugin/default_config.yml` — Add `precommit_command: "sase_hg_fix"`
- Create a new `sase_hg_fix` script: a thin wrapper that checks if we're in an hg repo (`hg root` succeeds) and runs
  ...
  (following the same pattern as other scripts in retired-hg-plugin — check how they register scripts there).
...
- In retired-hg-plugin: install and verify `sase_hg_fix` is available on PATH

### Commit instructions

Use `ccommit` to commit changes in each repo after validation passes. For sase:
`cd <repo_root> && ccommit chore "Add precommit_command config field" <files...>`. For retired-hg-plugin:
...
**retired-hg-plugin repo (../retired-hg-plugin/src/retired_hg_plugin/plugin.py):**
...
- In retired-hg-plugin: `just install` or equivalent passes
...
For retired-hg-plugin: `cd <repo_root> && ccommit feat "Add hg update sync to vcs_create_commit" <files...>`.
...
| ----- | ------------------------------------------------------------ | ----------------- |
| 1     | `precommit_command` config + `sase_hg_fix` script            | sase, retired-hg-plugin |
| 2     | Enhance CommitWorkflow + VCS providers with ccommit features | sase, retired-hg-plugin |
