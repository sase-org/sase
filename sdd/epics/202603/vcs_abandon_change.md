...
By contrast, the Google/hg provider's `legacy_mercurial_plugin_prune` script runs `hg cls-drop --prune` which handles both local and
...
### Phase 3: Google implementation (legacy-mercurial-plugin)

#### 3.1 `src/legacy_mercurial_plugin/plugin.py` - Implement hook

For Google, `vcs_prune` already calls `legacy_mercurial_plugin_prune` which does `hg cls-drop --prune`. The `abandon_change` hook
should handle the CL-number removal part, while prune handles the local branch deletion.

However, since `legacy_mercurial_plugin_prune` already bundles both operations, and refactoring it would be risky, the Google
implementation should be a **no-op with a comment** explaining that `vcs_prune` already handles remote CL cleanup:

```python
@hookimpl
def vcs_abandon_change(self, cl: str, revision: str, cwd: str) -> tuple[bool, str | None]:
    """No-op — legacy_mercurial_plugin_prune (called via vcs_prune) already handles CL cleanup."""
    return (True, None)
```

Alternatively, if the user wants a cleaner separation, we could refactor `legacy_mercurial_plugin_prune` to only handle local
...
| `src/legacy_mercurial_plugin/plugin.py`                | legacy-mercurial-plugin | Implement `vcs_abandon_change` (no-op, prune handles it) |
