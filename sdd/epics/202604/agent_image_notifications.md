...
This plan spans `sase`, `sase-telegram`, and `legacy-chat-integration`. The work should be split across distinct agent instances,
...
- `sase-telegram` already detects image attachments in `src/sase_telegram/scripts/sase_tg_outbound.py::_is_image_file()`
  ...
  paths.
- `legacy-chat-integration` formats workflow-complete notifications in `src/legacy_chat_integration/formatting.py::_format_workflow_complete()`
  and uploads formatted attachments in `src/legacy_chat_integration/scripts/sase_gc_outbound.py`.
...
- `just install` if needed, then `just check` in `legacy-chat-integration`.
