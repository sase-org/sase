...
- `../legacy-chat-integration/src/legacy_chat_integration/scripts/sase_gc_outbound.py` already converts non-image Markdown attachments to PDF
  before `gchat_client.upload_file()`.
- `../legacy-chat-integration/src/legacy_chat_integration/pdf_convert.py` is a near-duplicate Pandoc-based converter.
...
- Move or replicate the Pandoc command construction currently duplicated in `sase-telegram` and `legacy-chat-integration`, with
...
- In `src/legacy_chat_integration/scripts/sase_gc_outbound.py`, make the attachment upload decision explicit:
...
- Keep `legacy_chat_integration.pdf_convert.md_to_pdf()` for plan approval and legacy Markdown attachments unless Phase 2 has already
...
- Run `just install` if needed, then `just check` in `legacy-chat-integration`.

## Phase 6: Optional Plugin Converter Consolidation

Repos: `sase-telegram`, `legacy-chat-integration`, possibly `sase`

Objective: remove duplicated Markdown-to-PDF converter implementations after the core helper is stable.

Scope:

- Update `sase_telegram.pdf_convert.md_to_pdf()` and `legacy_chat_integration.pdf_convert.md_to_pdf()` to delegate to the new core
...
Repos: `sase`, `sase-telegram`, `legacy-chat-integration`
