...
status: done
bead_id: sase-p
prompt: sdd/prompts/202604/legacy_chat_integration_integration.md
---
# Plan: `legacy-chat-integration` — Google Chat integration plugin

## Goal

Build a new `../legacy-chat-integration` plugin repository that mirrors the functionality of `../sase-telegram` but speaks Google
...
Because the gchat CLI cannot render interactive buttons, every place sase-telegram uses an inline keyboard, legacy-chat-integration
...
├── pyproject.toml             # name=legacy-chat-integration, hatchling, scripts = sase_chop_gc_{outbound,inbound}
...
phases. Each phase ends with `just check` passing in the `legacy-chat-integration` repo.
...
- `src/legacy_chat_integration/credentials.py` — env-var helpers (`get_space_id()`, `get_gchat_bin()`, `get_rate_limit()`); no `pass`
  integration needed because the CLI handles auth.
- `src/legacy_chat_integration/gchat_client.py` — sync subprocess wrapper exposing the verbs we need:
...
- `src/legacy_chat_integration/option_codes.py` — encode/decode for "out-of-band" actions that don't live in a notification thread
  (kill, retry, dot-command callbacks if we ever need them). Mostly a much smaller reimplementation of
  `callback_data.py`.
- `src/legacy_chat_integration/pending_actions.py`, `src/legacy_chat_integration/rate_limit.py`, `src/legacy_chat_integration/pdf_convert.py`,
  `src/legacy_chat_integration/pdf_style.css` — ported from sase-telegram with paths rewritten to `~/.sase/gchat/`.
...
- `src/legacy_chat_integration/formatting.py` containing:
...
- `src/legacy_chat_integration/outbound.py` — port of high-water-mark + lock from sase-telegram, paths rewritten.
- `src/legacy_chat_integration/inbound.py` (pure logic, no CLI calls):
...
- `src/legacy_chat_integration/scripts/sase_gc_outbound.py`:
...
- `src/legacy_chat_integration/scripts/__init__.py` re-exports `outbound_main`, `inbound_main`.
...
- **Treat all sase agent runtimes uniformly** — no Claude/Gemini/Codex special-casing in agent-launch code ported from
  sase-telegram.
- **`just check` before reply** — every phase's agent must run `just check` in `../legacy-chat-integration` and (because this repo is
