...
- `legacy-mercurial-plugin/src/sase_hg/workspace_plugin.py` - Implement `ws_get_workflow_metadata` returning
...
- `legacy-mercurial-plugin/src/sase_hg/workspace_plugin.py` - Implement: extract `http://cl/<N>`, generate `is_cl_submitted` script,
...
- `legacy-mercurial-plugin/src/sase_hg/plugin.py` - Implement `vcs_detect_repo_type`: check for `.hg/` dir, return `"hg"`
...
- `legacy-mercurial-plugin` - Implement both hooks (workspace_directory_for_num, full hg metadata tags)
