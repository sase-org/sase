...
- `sase-gchat` and `sase-telegram` register chop executables with `[project.scripts]`, not with pluggy entry points.
- `sase-gchat` stores channel state under `~/.sase/gchat/`, uses a lock file, bootstraps its high-water mark on first
...
1. A package does **not** need a custom `chop_script_dirs` entry if its console scripts are installed into the same
   environment or visible on `$PATH`; this is how `sase-gchat` and `sase-telegram` work.
...
Create a sibling plugin at `~/projects/github/sase-org/sase-gmail`, mirroring `sase-gchat` and `sase-telegram`.
...
- Add gmail-specific formatter branches in `sase-gchat` and `sase-telegram`.
...
1. Create `sase-gmail` skeleton mirroring `sase-gchat`: `pyproject.toml`, `Justfile`, `README.md`, `src/sase_gmail/`,
   `tests/`.
...
- `~/projects/github/sase-org/sase-gchat/src/sase_gchat/scripts/sase_gc_outbound.py` - modern chop entry point.
- `~/projects/github/sase-org/sase-gchat/src/sase_gchat/outbound.py` - high-water mark and lock pattern.
- `~/projects/github/sase-org/sase-gchat/src/sase_gchat/gchat_client.py` - retry/debug-log wrapper pattern.
