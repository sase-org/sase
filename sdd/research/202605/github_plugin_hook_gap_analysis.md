...
- Google plugin
  ...
  - `../sase-google/src/sase_google/scripts/*` (24 console scripts)
  - `../sase-google/src/sase_google/xprompts/*.{yml,md}`
  - `../sase-google/src/sase_google/llm_jetski/provider.py`
  - `../sase-google/pyproject.toml` (entry points)
...
Google support: **49 hooks** (all defined directly in `sase_google/plugin.py`
...
| `vcs_rewind` | Runs `sase_google_rewind` over reverse-ordered diff files | Missing | The core rewind workflow's `provider.rewind(...)` call has no GitHub backend. |
...
| `vcs_reword` / `vcs_reword_add_tag` | Run `sase_google_reword` | Inherited git stubs | Programmatic CL/PR description rewrite is Google-only today. |
...
| `ws_get_workspace_directory` | yes (git clone) | yes (`sase_google_get_workspace`) | |
...
- `vcs_provider.default_hooks: ["!$sase_google_presubmit", "$sase_google_lint"]`
...
| Entry point group | sase-github | sase-google |
|---|---|---|
...
| `project.scripts` | **0** | **24** (every `sase_google_*`, `sase_metahook_*`, `sase_hg_fix`, `sase_refresh_cl_desc`, `sase_split_*`) |
...
- sase-google (6 files): `hg.yml`, `crs.md` (Critique-comments response),
...
3. **Rewind workflow provider operation.** The core rewind workflow calls
   ...
   `sase_google_rewind`; GitHub has no implementation, so the core rewind
   workflow cannot complete on a GitHub workspace.
4. **Default ChangeSpec hooks for PRs.** Google attaches
   `!$sase_google_presubmit` and `$sase_google_lint` to every new ChangeSpec
...
9. **Programmatic description rewrite.** `vcs_reword` and
   `vcs_reword_add_tag` are Hg-specific (`sase_google_reword`). GitHub
