---
plan: sdd/tales/202604/gemini_memory_files.md
---
Can you help me create new long-term memory files in the ~/tmp/gemini/memory/ directory that correspond with the below
mentors defined in the legacy-mercurial-plugin repo? Make sure to also update the ~/tmp/gemini/GEMINI.md file accordingly. These
files are for another machine (I will copy them over when you are done). Think this through thoroughly and create a plan
using your `/sase_plan` skill before making any file changes.

- aaa
- g3doc
- sql
- ui

### DYNAMIC MEMORY

- @.sase/memory/long-external-repos.md (matched: `legacy-mercurial-plugin`)

### Additional Requirements

- These new memory files shouldn't reference mentors in any way. It is only the knowledge that I want to copy over to
  long-term memory files.
