...
description. The default PR tags from config (e.g., sase-google's `AUTOSUBMIT_BEHAVIOR`, `MARKDOWN`, etc.) ARE added
correctly.
...
The workspace provider's `ws_format_commit_description` hook (sase-google's workspace_plugin.py:123-149) DOES add
...
### Phase 3: Deduplicate sase-google `ws_format_commit_description` (follow-up)

The sase-google workspace plugin's `ws_format_commit_description` hardcodes the same tags now available via
