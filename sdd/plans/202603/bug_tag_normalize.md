...
- **Google plugin** (`sase-google/plugin.py`): overrides to escape special chars for hg
...
## Phase 2: Google Plugin Override (`../sase-google`)

In `sase-google/src/sase_google/plugin.py`, add after `vcs_prepare_description_for_reword`:
...
### 4b. sase-google tests

Add a test in `../sase-google/tests/test_hg_plugin.py` verifying the Google provider normalizes various input formats to
`http://b/<id>`:
