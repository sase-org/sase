...
which dispatches to `vcs_create_proposal` on the HgPlugin (`sase-google/plugin.py:368`). The HgPlugin's
`vcs_create_proposal` calls `vcs_create_commit` (which runs `sase_google_amend` — amending the CL), then cleans the
...
### 1. `../sase-google/src/sase_google/plugin.py` — Save diff instead of amending CL
...
| File                                             | Repo        | Change                                                                      |
| ------------------------------------------------ | ----------- | --------------------------------------------------------------------------- |
| `../sase-google/src/sase_google/plugin.py`       | sase-google | `vcs_create_proposal`: save diff + clean instead of amend + clean           |
