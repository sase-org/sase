...
# Fix: hg amend newline failure in sase-google vcs_create_commit
...
5. **sase-google `vcs_create_commit`** (`plugin.py:355-365`) passes the entire multi-line message as a single note
   argument to `sase_google_amend`:
   ```python
   note = f"[run] {message}" if message else "[run] Agent changes"
   out = self._run(["sase_google_amend", note], cwd)
   ```
6. `sase_google_amend` script passes note to `hg amend -n "${NOTE}"` -- hg rejects the newline
...
- Runtime: Gemini on Mercurial (sase-google plugin)
...
### Change 1: `../sase-google/src/sase_google/plugin.py` -- `vcs_create_commit`
...
### Change 2: `../sase-google/tests/test_hg_plugin.py` -- add test

Add a test that verifies multi-line messages only pass the first line to `sase_google_amend`.
