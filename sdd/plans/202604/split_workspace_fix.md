...
The `#split` workflow's setup step (`sase_split_setup`) calls `sase_google_update` to navigate to the target CL branch,
...
3. So `sase_google_update` → `hg update` fails because the cwd isn't an hg workspace
...
(`sase_google_update`, `sase commit`) directly in the workspace — it doesn't invoke `#hg` — so there's no double-claim
risk.

## Changes

### 1. `sase-google: sase_split_setup` — Add workspace claiming + `_chdir`

**File:** `src/sase_google/scripts/sase_split_setup`

Add workspace claiming logic at the top of `main()`, before the existing `sase_google_update` call:
...
- Print `_chdir=workspace_dir` so the executor changes cwd **before** `sase_google_update` runs
...
The `_chdir` must be printed **before** calling `sase_google_update`, since the executor processes `_chdir` as it parses
...
`sase_google_update`. The `_chdir` output will then ensure subsequent workflow steps also run in the workspace.

### 2. `sase-google: split.yml` — Add outputs + release step

**File:** `src/sase_google/xprompts/split.yml`
