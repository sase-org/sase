...
Completed agent diffs don't display in the TUI on the work machine (sase-google / Mercurial VCS). Live diffs for running
agents work fine on both git and hg machines.

## Root Cause

The `hg.yml` xprompt workflow in sase-google is missing a `diff` step. Both `git.yml` and `gh.yml` have a `diff` step
...
### Phase 1: Add `diff` step to `hg.yml` (sase-google plugin)

**File**: `../sase-google/src/sase_google/xprompts/hg.yml`
