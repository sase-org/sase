...
`CHANGELOG.md` is generated by release-please from conventional commit messages and must never be hand-edited. There is
no `Unreleased` section; `just lint` enforces this. Add changelog entries with a proper `feat:` or `fix:` commit subject
and a `BREAKING CHANGE:` footer where applicable.
