...
- When the `%mark:<name>` directive is included in an agent's prompt, that agent will me named `<name>`.
...
- We should support marking an agent with a name after it has already started executing by selecting the agent the
  "Agents" tab of the `sase ace` TUI and using a new keymap (e.g. `m`) to trigger a prompt that allows the user to input
