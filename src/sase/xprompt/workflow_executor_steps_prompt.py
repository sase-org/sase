...
class PromptStepMixin:
    ...
    _expand_embedded_workflows_in_prompt: (
        Any  # (prompt, *, runner_active_provider_name) -> tuple
    )
    ...
    def _resolve_runner_provider(
        runner_directive: tuple[str, str, dict[str, str]],
    ) -> tuple[RunnerProvider, str, int | None, bool]:
        """Resolve a runner provider from a parsed runner directive.

        Args:
            runner_directive: ``(provider_name, ref, named_args)`` tuple from
                :class:`PromptDirectives`.

        Returns:
            ``(provider, ref, n, release)`` ready for ``provider.pre_agent()``.

        Raises:
            WorkflowExecutionError: If the named provider is not registered.
        """
        provider_name, ref, named_args = runner_directive
        providers = get_runner_providers()
        provider = providers.get(provider_name)
        if provider is None:
            raise WorkflowExecutionError(
                f"Runner provider '{provider_name}' not found. "
                f"Available: {sorted(providers)}"
            )
        n_str = named_args.get("n")
        n: int | None = int(n_str) if n_str is not None else None
        release = named_args.get("release", "true").lower() != "false"
        return provider, ref, n, release
    ...
    def _execute_prompt_step(
        ...
        # Runner provider pre-agent lifecycle
        runner_provider: RunnerProvider | None = None
        runner_ctx: RunnerContext | None = None
        runner_active_provider_name: str | None = None
        if early.directives.runner is not None:
            provider, ref, n, release = self._resolve_runner_provider(
                early.directives.runner
            )
            runner_provider = provider
            runner_active_provider_name = provider.name
            runner_ctx = provider.pre_agent(ref, n=n, release=release)
        ...
        try:
            ...
            expanded_prompt, embedded_workflows, pre_step_count = (
                self._expand_embedded_workflows_in_prompt(
                    early.prompt,
                    runner_active_provider_name=runner_active_provider_name,
                )
            )
            ...
            # Runner provider post-agent lifecycle
            if runner_provider is not None and runner_ctx is not None:
                try:
                    post_result = runner_provider.post_agent(runner_ctx)
                    if post_result.diff_path:
                        if step_state.output is None:
                            step_state.output = {}
                        step_state.output["diff_path"] = post_result.diff_path
                    if post_result.meta:
                        if step_state.output is None:
                            step_state.output = {}
                        step_state.output.update(post_result.meta)
                    # Re-save marker with updated runner output
                    if post_result.diff_path or post_result.meta:
                        self._save_prompt_step_marker(
                            step.name,
                            step_state,
                            diff_path=post_result.diff_path
                            or (
                                step_state.output.get("diff_path")
                    ...
                    _logger.exception(
                        "Runner provider '%s' post_agent failed",
                        runner_provider.name,
                    )
