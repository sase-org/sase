...
from sase.xprompt.workflow_executor_types import (
    HITLHandler,
    HITLResult,
...
class WorkflowExecutor(StepMixin, LoopMixin, ParallelMixin):
    ...
    def __init__(
        ...
        Args:
            ...
            hitl_override: Force HITL on (True) or off (False) for all steps,
                ...
                per-step behavior.
            model_override: Top-level ``%model`` override from the user prompt.
                When set, agent steps that don't declare their own ``%model``
                directive will use this model instead of the default.
        """
    ...
    def _save_prompt_step_marker(
        ...
        # Preserve model/llm_provider from existing marker when not provided
