...
from sase.feature_flags.cli_set import handle_flag_set
...
def handle_flag_command(args: argparse.Namespace) -> None:
    ...
    if sub == "disable":
        sys.exit(handle_flag_set(args, enabled=False))
    if sub == "enable":
        sys.exit(handle_flag_set(args, enabled=True))
    if sub == "list":
    ...
    print("Usage: sase flag {disable,enable,list,new,show}", file=sys.stderr)
    sys.exit(2)
