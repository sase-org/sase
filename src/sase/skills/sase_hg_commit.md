...
1. **Examine uncommitted changes** — Run `hg status` or `hg diff` to understand what
   files have changed and why.

2. **Write a commit message file** — Create a file (e.g., `commit_message.md`)
   containing a good commit message.
...
3. **Run the commit** — Execute:
   ...
   NOT pass `--type` unless you need to override.
...
1. **Find conflicted files**: Run `hg resolve --list` (lines starting with `U` are
   unresolved).
2. **Read each file** and resolve conflict markers. Prefer the INCOMING version when
   uncertain.
