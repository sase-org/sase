...
description:
  Guide audited SASE long-term memory reads through `sase memory read`. Use when
  instructions require reading long-term memory context or mention the long-memory read
  procedure.
skill: true
log_skill_use: false
---

Use this skill when project instructions or a prompt require reading SASE long-term
memory.

## Rules

- Read canonical long-term memory only through `sase memory read`; it checks project
...
- Do not read canonical long-term memory files directly with shell commands or
  file-reading tools.
