...
have corroborated it with at least `bead.task_triage.min_plus_ones` `+1`
reports (see the epic plan's "Why" for the noise this bar removes). Every
surface that decides whether a ready task bead is withheld from triage, or
whether a withheld bead is old enough to sweep, derives it through
:func:`task_gate_suppressed` and :func:`stale_task_bead` rather than
recompute the comparison itself -- mirroring :mod:`sase.bead.flag_due`.

Both are pure functions of their arguments: callers own the clock and the
configured threshold, so no test needs to freeze time globally and no
consumer can disagree with another about what "suppressed" or "stale" means.
"""
