    ...
    *,
    ...
    try:
        ...
        if repo_root is None:
            return PushOutcome(pushed=False, skipped_no_remote=True, error=None)

        if not has_push_remote(repo_root):
            return PushOutcome(pushed=False, skipped_no_remote=True, error=None)

        from sase.bead.sync_worker import run_managed_sync_worker

        log_path = new_sync_log_path()
        semantic_beads_dir = semantic_beads_dir_for_sync(repo_root, beads_dir)
...
def publish_bead_claim(
    ...
    *,
    ...
    push: Callable[[Path], PushOutcome],
) -> PushOutcome:
...
def has_push_remote(repo_root: Path) -> bool:
...
def head_is_published(repo_root: Path) -> bool:
    try:
...
def semantic_beads_dir_for_sync(repo_root: Path, requested_path: Path) -> Path:
