"""Settle a closed bead's pending gate the moment ``sase bead close`` closes it.
...
_CLOSE_SETTLE_GATE_KINDS = (TASK_TRIAGE_KIND, BEAD_SNOOZE_KIND, FLAG_TRIAGE_KIND)
...
def settle_closed_task_bead_gates(
    ...
    *,
    ...
    """Cancel the pending TaskTriage, BeadSnooze, or FlagTriage gate for each closed bead.

    Scans the gate directories once no matter how many beads a bulk close
    touches. Callers must pre-filter *bead_ids* to task or flag beads that
    were just closed; an empty *bead_ids* or unresolved *project* does no
