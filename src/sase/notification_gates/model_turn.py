...
from sase.turns.status import clamp_turn_status

DEFAULT_GATE_TURN_PENDING_STATUS = "GATE"
DEFAULT_GATE_TURN_SETTLED_STATUS = "GATED"
GATE_TURN_DEFAULT_TIMEOUT_SECONDS = 24 * 60 * 60.0
GATE_TURN_STATUS_MAX_CHARS = 20
GATE_TURN_STATUS_ELLIPSIS = "\u2026"

GATE_TURN_WORKSPACES = frozenset({"inherit", "release"})
GATE_TURN_NEXT_FORKS = frozenset({"session", "turn", "none"})
# legacy sase-shell spelling: gate-spec input and pre-rename bundles carry
# ``"shell"``; new writers emit only ``"turn"``.
LEGACY_GATE_TURN_NEXT_FORK_SHELL = "shell"
# legacy agent-family spelling: gate-spec input and pre-rename bundles carry
# ``"family"``; new writers emit only ``"session"``.
LEGACY_GATE_TURN_NEXT_FORK = "family"
GATE_TURN_NEXT_OUTPUTS = frozenset({"none", "results", "tail", "file"})
GATE_TURN_RESERVED_BRANCHES = frozenset({"timeout", "stopped", "failed"})
...
class GateTurnNext:
    ...
    def from_mapping(
        ...
        *,
        target: str,
        inherited: GateTurnNext | None = None,
        ...
        if fork == LEGACY_GATE_TURN_NEXT_FORK_SHELL:
            fork = "turn"
        if fork not in GATE_TURN_NEXT_FORKS:
            raise GateError(
                "invalid_shell",
                f"{target}.fork",
...
class GateTurnBranchSpec:
    ...
    def from_mapping(
        ...
        *,
        ...
        next_policy = GateTurnNext.from_mapping(
            next_data,
            target=target,
...
class GateTurnSpec:
    ...
    suffix: str | None = None
    pending_status: str = DEFAULT_GATE_TURN_PENDING_STATUS
    settled_status: str = DEFAULT_GATE_TURN_SETTLED_STATUS
    accent: str = field(default_factory=lambda: _default_accent("GATE\x1fGATED"))
    workspace: str = "inherit"
    next: GateTurnNext = field(default_factory=GateTurnNext)
    branches: dict[str, GateTurnBranchSpec] = field(default_factory=dict)
    ...
    def from_mapping(
        ...
        *,
        ...
        pending_status = _status(
            data.get("pending_status", DEFAULT_GATE_TURN_PENDING_STATUS),
            "shell.pending_status",
        )
        settled_status = _status(
            data.get("settled_status", DEFAULT_GATE_TURN_SETTLED_STATUS),
        ...
        if workspace not in GATE_TURN_WORKSPACES:
            raise GateError(
                "invalid_shell",
                "shell.workspace",
...
def _valid_branch_keys(
    ...
    *,
    ...
    return frozenset(keys) | GATE_TURN_RESERVED_BRANCHES
...
def _branches(
    ...
    *,
    ...
    inherited_next: GateTurnNext,
    fork_normalizer: Callable[[object], object] | None = None,
) -> dict[str, GateTurnBranchSpec]:
    data = json_object(value, "shell.branches")
    result: dict[str, GateTurnBranchSpec] = {}
    for raw_key, raw_branch in data.items():
        ...
        result[key] = GateTurnBranchSpec.from_mapping(
            raw_branch,
...
def _next_output(value: object, target: str) -> tuple[str, ...]:
    ...
    invalid = sorted(set(items) - GATE_TURN_NEXT_OUTPUTS)
    if invalid:
        raise GateError(
            "invalid_shell",
            target,
