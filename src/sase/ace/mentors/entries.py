...
from sase.ace.mentors.status import update_patch_mentors_field
...
def add_mentor_entry(
    ...
    """Add a new MENTORS entry for a Patch.
    ...
    try:
        with patch_lock(project_file):
            ...
            write_patch_atomic(
                project_file,
                content,
                f"Add MENTORS entry ({entry_id}) for {patch_name}",
            )
            return True
    except Exception:
        logger.warning(
...
def clear_mentor_draft_flags(project_file: str, patch_name: str) -> bool:
    ...
    try:
        ...
        for cs in patches:
            if cs.name == patch_name:
                ...
                draft_entries.sort(key=lambda e: parse_stitch_id(e.entry_id))
                ...
                # Write back
                return update_patch_mentors_field(project_file, patch_name, cs.mentors)
        return True  # Patch not found, nothing to do
    except Exception:
        logger.warning(
...
def set_mentor_draft_flags(project_file: str, patch_name: str) -> bool:
        ...
        for cs in patches:
            if cs.name == patch_name:
                ...
                sorted_entries = sorted(
                    cs.mentors, key=lambda e: parse_stitch_id(e.entry_id)
                )
...
def remove_mentor_data(
    ...
    *,
    ...
    Unlike update_patch_mentors_field which merges status_lines from
    ...
    try:
        with patch_lock(project_file):
            ...
            updated_lines = apply_mentors_update(lines, patch_name, updated_mentors)
            content = "".join(updated_lines)

            write_patch_atomic(
                project_file,
                content,
                f"Remove mentor data for {patch_name}",
            )
            return True
    except Exception:
        logger.warning(
