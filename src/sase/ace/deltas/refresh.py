...
rewind, sync, accept-proposal, re-parent) call to keep a Patch's
...
from .persistence import update_patch_deltas_field


_log = logging.getLogger(__name__)


def _find_patch(project_file: str, patch_name: str) -> object | None:
    try:
...
def refresh_deltas_for_patch(
    project_file: str,
    patch_name: str,
    workspace_dir: str | None = None,
) -> bool:
    """Recompute DELTAS for *patch_name* and persist atomically.

    Args:
        project_file: Path to the ``.gp`` project file.
        patch_name: NAME of the Patch to refresh.
    ...
    patch = _find_patch(project_file, patch_name)
    if patch is None:
        _log.debug(
            "Patch %s not found in %s; skipping DELTAS refresh",
            patch_name,
            project_file,
        )
        return False
    ...
    try:
        deltas = compute_deltas(patch, provider, cwd)  # type: ignore[arg-type]
    except DeltaComputationError as exc:
        # VCS hiccup or missing parent ref — preserve existing DELTAS.
        _log.warning("DELTAS compute failed for %s: %s", patch_name, exc)
        return False
    except Exception as exc:
        _log.warning(
    ...
    return update_patch_deltas_field(project_file, patch_name, deltas)
...
def refresh_deltas_after_commits_change(
    ...
    base, or VCS head used by a Patch should call this helper after the
    atomic write.
    """
    try:
        return refresh_deltas_for_patch(project_file, patch_name, workspace_dir)
...
refresh_deltas_for_changespec = refresh_deltas_for_patch  # legacy compatibility alias
