...
When a Patch transitions to a terminal status (Submitted, Archived, Reverted),
...
from .locking import patch_lock, write_patch_atomic
...
def extract_patch_block(
    lines: list[str], patch_name: str
    ...
    Args:
        ...
        patch_name: The NAME value to find.

    Returns:
        ``(extracted_lines, remaining_lines)`` where *extracted_lines* is
        ``None`` if the Patch was not found.
    """
...
def move_patch_to_file(source_file: str, dest_file: str, patch_name: str) -> bool:
    """Move a Patch block from one file to another.

    The move uses an add-then-remove strategy so the Patch temporarily
    exists in both files (safe) rather than neither (unsafe).

    Args:
        source_file: Path to the file containing the Patch.
        dest_file: Path to the file to move the Patch to.
        patch_name: The NAME of the Patch to move.

    Returns:
        ``True`` on success, ``False`` if the Patch was not found.
    """
    # Step 1: Read source and extract the block
    with patch_lock(source_file):
        if not os.path.isfile(source_file):
            return False
        with open(source_file, encoding="utf-8") as f:
            source_lines = f.readlines()

        extracted, _ = extract_patch_block(source_lines, patch_name)

    if extracted is None:
        logger.debug(
            "Patch '%s' not found in %s, skipping move",
            patch_name,
            source_file,
        )
        return False
    ...
    with patch_lock(dest_file):
        ...
        write_patch_atomic(dest_file, dest_content, f"Add {patch_name}")
    ...
    with patch_lock(source_file):
        ...
        _, remaining = extract_patch_block(source_lines, patch_name)
        new_content = "".join(remaining)

        write_patch_atomic(source_file, new_content, f"Remove {patch_name}")
