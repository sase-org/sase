...
class AgentInteractionMixin:
    ...
    def action_focus_pinned_panel(self) -> None:
        ...
        if not self._pinned_panel_focused:  # type: ignore[attr-defined, has-type]
            # Try to focus pinned panel
            if not self._pinned_agent_objects:  # type: ignore[attr-defined]
                return
            self._pinned_panel_focused = True  # type: ignore[attr-defined]
            self._pinned_panel_idx = min(  # type: ignore[attr-defined, has-type]
                self._pinned_panel_idx,  # type: ignore[attr-defined, has-type]
                len(self._pinned_agent_objects) - 1,  # type: ignore[attr-defined]
            )
        else:
            # Return to main list
    ...
    def action_pin_agent(self) -> None:
        ...
        # Reload agents to re-partition pinned vs unpinned
    ...
    def action_edit_panel(self) -> None:
        ...
        if self._get_selected_agent() is None:  # type: ignore[attr-defined]
            self.notify("No agent selected", severity="warning")  # type: ignore[attr-defined]
            return
