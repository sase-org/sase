...
class AgentMarkingMixin:
    ...
    def _toggle_mark_agent(self) -> None:
        ...
        # Auto-advance cursor to the next entry within the same panel (with wraparound).
    ...
    def _prune_stale_marked_agents(self) -> None:
        ...
        # Also include unfiltered (workflow children may be folded out of _agents)
    ...
    def _bulk_kill_marked_agents(self) -> None:
        ...
        def on_dismiss(confirmed: bool | None) -> None:
            if not confirmed:
                return
            # Skip identities already removed by earlier cascades
            # (e.g., marking both a workflow parent and its child).
            for agent in list(killable):
                if agent.identity not in {
                    a.identity for a in self._agents_with_children
                }:
                    continue
