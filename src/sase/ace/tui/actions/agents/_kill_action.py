...
class AgentKillMixin:
    ...
    def _build_agent_cleanup_panel_state(self) -> AgentCleanupPanelState:
        ...
        return AgentCleanupPanelState(
            ...
            focused_clan_label=self._focused_cleanup_clan_label(clans),
        )
    ...
    def _run_agent_cleanup_panel_action(self, action: AgentCleanupAction) -> None:
        ...
        if action == "tag":
            ...
            return
        if action == "clan":
            self._open_clan_cleanup_selector()
            return
        if action == "custom":
    ...
    def _present_clan_cleanup(self, result: AgentCleanupClanResult) -> None:
        ...
        self._present_planned_cleanup(request, header=header, targets=targets)
