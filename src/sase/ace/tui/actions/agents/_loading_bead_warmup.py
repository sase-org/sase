...
only show the bead glyph / ``Bead:`` field when a confirmed display string is
already cached (see :mod:`sase.ace.tui.models.agent_bead`). This mixin warms
that cache off the startup-critical loader path: after an Agents refresh has
applied and the list has painted, a coalesced background worker resolves the
small set of visible rows whose bead candidate has not been confirmed yet, runs
the bead-store lookups off the event loop, and patches the affected rows in
place by identity once a candidate is confirmed.

First paint and j/k navigation are never gated on bead confirmation; the glyph
and field fill in shortly after the TUI is already interactive. Missing
candidates stay silent and are not re-resolved until the cache TTL expires.
"""
...
class AgentBeadWarmupMixin(AgentLoadingStateMixin):
    ...
    def _bead_warmup_candidates(self) -> list[Agent]:
        """Snapshot visible rows with an unconfirmed bead candidate.

        Bounded to the currently visible agents and to candidates whose cache
        entry is a miss (``should_resolve_bead_display``). Known-missing and
        already-confirmed candidates are skipped, so rapid refreshes do not
        repeatedly re-resolve them until the cache TTL expires.
        """
    ...
    def _apply_bead_warmup_results(
        ...
        """Patch rows that gained a confirmed bead on the UI thread.

        Matches results back to the *current* agent objects by identity (the
        list may have been rebuilt since the worker snapshot). Only newly
        confirmed rows appear in *results*, so this patches exactly the rows
        whose confirmed state changed; the confirmed string itself lives in the
        bead cache that row rendering reads. ``_try_patch_agent_row`` is a no-op
        off the Agents tab, where the warmed cache simply feeds the next render.
        """
        if not results:
            return
