"""Dashboard refresh mixin for the ace axe display.

Paints the axe dashboard and info panel, and steps the displayed chop run,
from the in-memory caches populated by ``AxeDisplayLoadersMixin``.
"""
...
def chop_allows_auto_scroll(snapshot: ChopSnapshot | None, run_idx: int) -> bool:
...
class AxeDisplayDashboardMixin(AxeDisplayFooterMixin):
    """Mixin providing the axe dashboard refresh and info-panel methods."""
    ...
    def _refresh_axe_display(self) -> None:
        ...
        try:
            ...
            if self._axe_chop_selection is not None:
                ...
                selected_chop_active = chop_allows_auto_scroll(chop_snap, run_idx)
            if (
