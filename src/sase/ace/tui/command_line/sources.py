...
class SourceCandidate:
    ...
    def to_dynamic(self) -> dict[str, Any]:
        """Render as a ``DynamicCandidate`` dict for ``complete()``."""
...
def _agent_candidates(app: Any) -> list[SourceCandidate]:
...
def _proc_candidates(app: Any) -> list[SourceCandidate]:
...
def _project_candidates(app: Any) -> list[SourceCandidate]:
    projects = (
    ...
    for project in rows:
        ...
        if name:
            candidates.append(
                SourceCandidate(value=name, badge="project", source="tui")
            )
    return candidates


def _patch_candidates(app: Any) -> list[SourceCandidate]:
    ...
    for patch in rows:
        if isinstance(patch, str):
            if patch:
                candidates.append(
                    SourceCandidate(value=patch, badge="patch", source="tui")
                )
            continue
        patch_id = _text(
...
def in_memory_candidates(app: Any, kind: str) -> list[SourceCandidate]:
...
def collect_dynamic_candidates(
    ...
    Returns ``DynamicCandidate`` dicts ready for ``complete(dynamic=…)``:
    ...
    """
    dynamic = [
        candidate.to_dynamic() for candidate in in_memory_candidates(app, value_kind)
    ]
...
class ProviderCache:
    ...
    def __init__(self, *, ttl_seconds: float = PROVIDER_CACHE_TTL_SECONDS) -> None:
