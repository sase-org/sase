"""``PatchGroupingMode`` and date/status bucket helpers."""
...
class PatchGroupingMode(Enum):
    """How the Patches pane is bucketed at L0.
...
def precompute_latest_timestamps(
    ...
    """
    return {id(cs): latest_patch_timestamp(cs) for cs in patches}


def date_bucket_for_patch(
    cs: Patch,
    now: datetime,
    ...
    - ``Earlier``: anything older, plus Patches with no parseable timestamps.
    ...
    if latest_map is not None and id(cs) in latest_map:
        ...
        latest = latest_patch_timestamp(cs)
    if latest is None:
        return _EARLIER
    today = now.date()
    when = latest.date()
...
def latest_from_map(
    ...
    return latest_patch_timestamp(cs)


def date_subgroup_for_patch(
    cs: Patch,
    date_bucket: str,
    latest_map: LatestTimestampMap | None = None,
) -> str:
    ...
    calendar day, and ``Earlier`` by Monday-start week; Patches without a
...
def status_bucket_for_patch(cs: Patch) -> str:
