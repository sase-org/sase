...
def agent_is_rendered_in_agents_panel(agent: Agent) -> bool:
    ...
    A ``STARTING`` agent is always excluded from rendered Agents-tab rows,
    regardless of age or missing ``start_time``. It still contributes to the
    ``<N> starting`` headline count via ``hidden_starting_indices``; a real
    status change is the only thing that makes the row visible.
    """
