"""Loader for per-agent ``sase glossary read`` audit events."""
...
class GlossaryReadDisplayEvent:
    """A glossary-read event paired with an optional session role label.
...
@dataclass
class _GlossaryReadsCacheEntry:
...
@dataclass
class _GlossaryReadsContextCacheEntry:
...
@dataclass
class _GlossaryReadsSnapshotCacheEntry:
    events: tuple[GlossaryReadEvent, ...]
    log_mtime_ns: int
    log_size: int
    last_read_monotonic: float
...
_glossary_reads_cache: dict[tuple[str, str], _GlossaryReadsCacheEntry] = {}
_glossary_reads_context_cache: dict[
    tuple[str, tuple[str, ...]], _GlossaryReadsContextCacheEntry
] = {}
_glossary_reads_snapshot_cache: OrderedDict[str, _GlossaryReadsSnapshotCacheEntry] = (
    OrderedDict()
)
...
def _filter_events_for_agent(
    events: tuple[GlossaryReadEvent, ...], agent: Agent
    ...
    matches: list[GlossaryReadEvent] = []
    for event in events:
...
def _cache_entry_stat(
    entry: _GlossaryReadsCacheEntry | _GlossaryReadsContextCacheEntry,
) -> _LogStat:
    return (entry.log_mtime_ns, entry.log_size)


def _load_glossary_read_events_snapshot(
    project: str,
    *,
    log_path: Path,
    now: float,
    current_stat: _LogStat,
) -> tuple[tuple[GlossaryReadEvent, ...], _LogStat]:
    cached = _glossary_reads_snapshot_cache.get(project)
    if cached is not None:
        ...
        if recent or current_stat == cached_stat:
            _glossary_reads_snapshot_cache.move_to_end(project)
            return cached.events, cached_stat

    events = read_glossary_read_events(project=project)
    entry = _GlossaryReadsSnapshotCacheEntry(
        events=events,
        log_mtime_ns=current_stat[0],
        log_size=current_stat[1],
        last_read_monotonic=now,
    )
    _glossary_reads_snapshot_cache[project] = entry
    _glossary_reads_snapshot_cache.move_to_end(project)
    while len(_glossary_reads_snapshot_cache) > _MAX_SNAPSHOT_CACHE_PROJECTS:
        _glossary_reads_snapshot_cache.popitem(last=False)
    return events, current_stat


def _load_glossary_reads_for_agent(
    agent: Agent, *, limit: int = MAX_KEPT_READS
) -> tuple[GlossaryReadEvent, ...]:
    """Return glossary-read events attributed to ``agent``, newest first.
    ...
    cached = _glossary_reads_cache.get(key)
    ...
    all_events, snapshot_stat = _load_glossary_read_events_snapshot(
        project,
        log_path=log_path,
        now=now,
    ...
    _glossary_reads_cache[key] = _GlossaryReadsCacheEntry(
        events=capped,
...
def load_glossary_reads_for_agent_context(
    agent: Agent, *, limit: int = MAX_KEPT_READS
) -> tuple[GlossaryReadDisplayEvent, ...]:
    """Return display glossary-reads for an agent-session context, newest first.

    For an ordinary row with no follow-up session members this delegates to
    :func:`_load_glossary_reads_for_agent` and wraps each event with no
    label, preserving the single-agent visual shape. For a session row it
    reads the log once, attributes each event to a member, de-duplicates by
    event id, sorts newest first, caps to ``MAX_KEPT_READS``, and labels each
    kept event with its producer's compact role.
    """
    ...
    if len(members) <= 1:
        events = _load_glossary_reads_for_agent(agent, limit=limit)
        return tuple(GlossaryReadDisplayEvent(event=event) for event in events)
    ...
    cached = _glossary_reads_context_cache.get(key)
    current_stat = _log_stat(log_path)
    ...
    matched: list[GlossaryReadDisplayEvent] = []
    ...
    for event in all_events:
        ...
        matched.append(GlossaryReadDisplayEvent(event=event, agent_label=label))

    ordered = tuple(
        sorted(matched, key=lambda item: item.event.timestamp, reverse=True)
    )
    capped = ordered[:MAX_KEPT_READS]
    _glossary_reads_context_cache[key] = _GlossaryReadsContextCacheEntry(
        events=capped,
        log_mtime_ns=snapshot_stat[0],
        log_size=snapshot_stat[1],
        last_read_monotonic=now,
