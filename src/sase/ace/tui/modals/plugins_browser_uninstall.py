...
class UninstallPreview:
    ...
    *plan* is the single :class:`UninstallPlan` outcome — a terminal one
    ...
    routed to a CLI-matching toast, or an :class:`UninstallReady` opened in the
...
def plan_uninstall_preview(query: str, *, offline: bool) -> UninstallPreview:
    ...
    source of truth shared with the CLI — once, cache-first (``refresh=False``).
...
def uninstall_summary(plan: UninstallReady) -> str:
    ...
    Mirrors the CLI's ``uninstall --dry-run`` "Removes ... (other plugins stay
    installed)".
    """
...
def already_absent_message(name: str) -> str:
    """The ``uninstall`` already-absent toast, mirroring the CLI's wording."""
