...
ACTION_PRODUCERS: tuple[ProcProducerSite, ...] = (
    ...
    site(
        ...
        concurrency_keys=("bgcmd-launch:{dedup_key}", "bgcmd-launch-slot:{slot}"),
        optimistic_ui="reserve slot in memory",
    ),
    site(
        "axe.bgcmd.kill",
        "src/sase/ace/tui/actions/axe_bgcmd.py",
        ...
        "proc.kill",
        "durable",
        "AxeBgCmdMixin._stop_bgcmd",
        "sase proc kill",
        identifiers=("proc_id",),
        result_kind="proc.kill",
        concurrency_keys=("proc-kill:{proc_id}",),
        optimistic_ui="dismiss the oneshot row once the kill lands",
    ),
    site(
        "monitor.stop",
    ...
    site(
        ...
        result_kind="agent.drain",
        concurrency_keys=("provider-drain:{provider}",),
        optimistic_ui="pending Procs row; completion toast",
    ),
    site(
    ...
    site(
        ...
        identifiers=("kind", "alias", "request_id"),
        result_kind="machine.attention-action",
        concurrency_keys=("ace:machine-attention:{alias}:{request_id}",),
        optimistic_ui="status override; fleet refresh",
    ),
    site(
    ...
    site(
        ...
        "_submit_durable_neutral_plan_response",
        "sase gate answer",
        identifiers=("notification_id", "request_id"),
    ...
    site(
        "projects.init_check",
        "src/sase/ace/tui/modals/projects_pane_init_actions.py",
        "_start_init_check",
        "session_worker",
        "init-check",
        "ui_only",
        "ProjectsPaneInitActionsMixin._start_init_check",
        "sase init --check --json",
        identifiers=("scope_key",),
        result_kind="init.check",
    ...
    site(
        ...
        "init-apply",
        "ui_only",
        "ProjectsPaneInitActionsMixin._submit_init_apply",
        "sase init --yes",
        identifiers=("scope_key",),
        result_kind="init.apply",
        concurrency_keys=("sase-init",),
    ...
    site(
        "command-line.submit",
        "src/sase/procs/command_line.py",
        ...
        "sase <tokens>",
        identifiers=("tokens",),
        result_kind="command-line.exit",
        concurrency_keys=(),
        optimistic_ui="submitting transcript block; exit toast on settle",
        restart_recovery="durable ordinary proc; observer exit watch settles from the store row",
    ),
)
