...
from ._patch_list_render import _BANNER_ROW, render_grouped
...
__all__ = ["PatchList", "_BANNER_ROW", "_get_status_indicator"]


class PatchList(OptionList):
    """Left sidebar showing list of Patches."""
    ...
    class SelectionChanged(Message):
        ...
        Patch in the group so the detail panel still has something
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._option_idx_by_patch_name: dict[str, int] = {}
        ...
        # Each rendered Option row maps back to a Patch index, or
    ...
    def update_list(
        ...
        grouping_mode: PatchGroupingMode = PatchGroupingMode.BY_PROJECT,
        ...
        Args:
            ...
            current_idx: Index of currently selected Patch
            marked_indices: Set of indices that are marked
            hide_reverted: Whether reverted Patches are currently hidden
            hide_submitted: Whether submitted Patches are currently hidden
            jump_hints: Optional local row index -> adaptive hint mapping
            grouping_mode: Which Patch grouping mode to render.  Banner rows
                are produced by :func:`build_patch_tree` and
                interleaved with the Patch rows.
            ...
            current_group_key: When non-None and pointing at a banner
                ...
                instead of the Patch row at ``current_idx``.
    ...
    def _update_list_impl(
        ...
        # When not hiding, show ◌ prefix on the relevant Patches
        show_hideable = not hide_reverted
        show_submitted = not hide_submitted
        self.clear_options()
        self._option_idx_by_patch_name = {}
    ...
    def update_highlight(
        ...
        cursor moves to the row showing ``patches[current_idx]``,
        falling back to a clamped raw index for legacy flat callers
        that pre-date the row-map bookkeeping.
        """
        with tui_trace("widget.patch_list.update_highlight", count=self.option_count):
    ...
    def watch_highlighted(self, highlighted: int | None) -> None:
        ...
        if self._programmatic_update:
            trace_event(
                "widget.patch_list.watch_highlighted.suppressed",
                highlighted=highlighted,
            )
            return
        trace_event(
            "widget.patch_list.watch_highlighted",
            highlighted=highlighted,
        )
    ...
    def patch_patch_row(
        ...
        *,
        ...
        """Replace one Patch's Option in place when shape didn't change.
        ...
        """
        with tui_trace("widget.patch_list.patch_patch_row", idx=idx):
            return self._patch_patch_row_impl(
                idx,
                patch,
                selected=selected,
                marked=marked,
    ...
    def _patch_patch_row_impl(
        ...
        *,
        ...
        if not (0 <= idx < len(self._patches)):
            return False
        ...
        if self.option_count != len(self._patches):
            return False

        existing = self._patches[idx]
        if existing.name != patch.name:
            return False
        # The option_id is the patch name — keep the cached idx map
        # honest by refusing to patch an entry whose name moved index.
        if self._option_idx_by_patch_name.get(patch.name) != idx:
            return False

        show_hideable: bool = ctx["show_hideable"]
        show_submitted: bool = ctx["show_submitted"]
        stats = compute_mentor_stats(patch)
        ...
        new_option = format_patch_option(
            patch,
    ...
    def _resolve_row(self, option_index: int) -> tuple[int, GroupKey | None]:
        """Translate an OptionList row index to ``(patch_idx, group_key)``.

        Selectable banner rows resolve to their group's first Patch
        ...
        if banner is not None:
            first = banner.patch_indices[0] if banner.patch_indices else 0
            return (first, banner.group_key)
        if entry == _BANNER_ROW:
            # Expanded (non-selectable) banner row that somehow received
            # focus — fall through to the next Patch row so the caller's
