"""Render path for the Patch list widget.

The grouped (banner + Patch) render is factored out of
:class:`PatchList` so the widget module stays focused on widget
...
from ._patch_list_banner import (
    CS_MIN_BANNER_WIDTH,
...
def render_grouped(
    ...
    *,
    ...
    grouping_mode: PatchGroupingMode,
    ...
    ``option_count == len(self._patches)`` so a stale patch can
    ...
    # First pass: format Patch rows so we know the widest content.
    ...
    for i, cs in enumerate(patches):
        ...
        cs_options[i] = format_patch_option(
            cs,
    ...
    for entry in tree:
        ...
        if entry.patch_idx is None:
            continue
        i = entry.patch_idx
        row_index = len(widget._row_entries)
        widget.add_option(cs_options[i])
        widget._row_entries.append(i)
        widget._option_idx_by_patch_name[patches[i].name] = i
