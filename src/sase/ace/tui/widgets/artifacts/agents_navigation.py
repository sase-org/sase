"""Guarded selection and the shared entry-navigation contract for agents."""
...
class AgentsOptionList(OptionList):
    """Agent rows whose guarded highlights retain native viewport following."""
...
class AgentsNavigationMixin(_MixinBase):
    ...
    def _option_list(self) -> AgentsOptionList | None:
        try:
            return self.query_one("#agents-list", AgentsOptionList)
        except Exception:
            return None


__all__ = ["AgentsNavigationMixin", "AgentsOptionList"]
