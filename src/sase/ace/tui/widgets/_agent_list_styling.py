...
# secondary.  L1 (name-root) banners use a single dim-gray style for the
# branch glyph, trailing rule, and chip; only the name-root label gets its
# own teal accent.
...
# edge as a colored bar; ``╭─`` (rounded branch) starts L1 banners and
# reads as "subgroup begins here".  Rules use heavy ``━`` for L0 and
# light ``─`` for L1 so L0 has visibly more weight.
_PROJECT_BAR_GLYPH = "▌"
_PROJECT_RULE = "━"
