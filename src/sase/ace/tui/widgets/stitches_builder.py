"""STITCHES section builder for Patch detail display."""
...
def should_show_stitches_drawers(
    entry: Stitch,
    patch: Patch,
    stitches_fold: FoldLevel,
) -> bool:
    """Determine if drawers (CHAT/DIFF) should be shown for a STITCHES entry.
    ...
    """
    if stitches_fold != FoldLevel.COLLAPSED:
        return True
    if patch.stitches:
        max_number = max(
            (e.number for e in patch.stitches if not e.is_proposed),
            default=0,
        )
...
def _should_show_chat_drawer(
    ...
    """Determine if CHAT drawer should be shown for a STITCHES entry.
    ...
    if entry.number == 1 and not entry.is_proposed:
        return True
    return should_show_stitches_drawers(entry, patch, stitches_fold)


def _get_rejected_proposals_for_entry(
    entry: Stitch,
    all_stitches: list[Stitch],
    max_number: int,
) -> list[Stitch]:
    ...
    Args:
        ...
        all_stitches: All stitch entries in the Patch.
        max_number: The highest numeric stitch entry number.
...
def _should_hide_rejected_proposals(stitches_fold: FoldLevel) -> bool:
    ...
    """
    return stitches_fold != FoldLevel.FULLY_EXPANDED


def truncate_note(note: str, available: int) -> str:
...
def build_stitches_section(
    ...
    """Build the STITCHES section of the display.
    ...
    Args:
        ...
        stitches_fold: Fold level for the stitches section.
    ...
    max_number = max(
        (e.number for e in stitches if not e.is_proposed),
        default=0,
    )

    hide_rejected = _should_hide_rejected_proposals(stitches_fold)

    text.append("STITCHES:\n", style="bold #87D7FF")
    for entry in stitches:
        ...
        show_drawers = should_show_stitches_drawers(entry, patch, stitches_fold)
        show_chat = _should_show_chat_drawer(entry, patch, stitches_fold)
        show_body = entry.body and stitches_fold != FoldLevel.COLLAPSED
        rejected_proposals = (
            _get_rejected_proposals_for_entry(entry, stitches, max_number)
            if hide_rejected
            else []
        )
        ...
        if max_width is not None and stitches_fold == FoldLevel.COLLAPSED:
            ...
            display_note = truncate_note(entry.note, available)
