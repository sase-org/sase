...
Scans ~/.sase/projects/ for .gp files, converts each to a ProjectSpec YAML
...
from sase.ace.changespec.project_spec import (
    ProjectSpec,
...
def _find_gp_files(project_name: str | None) -> list[Path]:
    """Scan ~/.sase/projects/ for .gp files to migrate.
...
def _git_commit_all(migrated_files: list[Path], removed_files: list[Path]) -> None:
    """Stage and commit all migration changes in ~/.sase.
