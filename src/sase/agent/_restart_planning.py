...
def _rewrite_prompt_identity(
    ...
    agent_session_name, role_suffix, is_agent_session_root = (
        _agent_session_rewrite_args(meta)
    )
    try:
...
def _agent_session_rewrite_args(
    meta: dict[str, Any],
    ...
    if agent_session and agent_session_parallel_value(meta) is not True and role_suffix:
        return agent_session, role_suffix, is_agent_session_root
