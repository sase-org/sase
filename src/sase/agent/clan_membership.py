...
def _reserve_clan_membership(
    ...
    *,
    ...
    _reject_agent_session_marked_clan_name(clan_name)
...
def _reject_agent_session_marked_clan_name(clan_name: str) -> None:
    ...
    agent-session marker pushes that marker out of the final segment and produces a
    ...
    raise ClanMembershipError(
        ...
        f"members would be named '{clan_name}.<suffix>', placing the agent-session "
        "role suffix outside the final name segment. Choose a clan name "
        "without an agent-session role suffix."
    )
