...
def _self_parallel_agent_session_root(
    base_name: str,
    ...
    agent_session_name = agent_session_value(meta)
    if not isinstance(agent_session_name, str) or current_owner_agent_name_key(
        agent_session_name
    ...
    if (
        ...
        and agent_session.root is not None
        and agent_session.root.timestamp == parent_timestamp
    ):
    ...
    # late parallel member but does not itself carry agent-session metadata.
...
def resolve_resume_agent_name(name: str) -> NamedAgent | None:
    ...
    exact lookups. A root agent-session reference such as ``foo`` resolves to
    the most recent successful member of the newest agent-session generation,
    falling back to exact-name behavior when no agent session exists.
    """
    ...
    if clan_member is not None:
        return clan_member
    is_self_agent_session, self_root = _self_parallel_agent_session_root(name)
    if is_self_agent_session:
        if (
            ...
            return self_root
        return None
    if not _is_canonical_agent_session_member_name(name):
        agent_session_member = most_recent_completed_agent_session_member(name)
        if agent_session_member is not None:
            return agent_session_member
...
def resolve_wait_dependency(name: str) -> bool:
    ...
    complete = is_agent_session_complete(name)
