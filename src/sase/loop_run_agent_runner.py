...
def _create_new_changespec(
    ...
    Follows the same workflow as sase commit:
    ...
    fd, desc_file = tempfile.mkstemp(suffix=".txt", prefix="sase_ace_desc_")
    try:
...
def main() -> None:
    ...
    # Save prompt to history for future sase run sessions
    ...
    try:
        try:
            ...
            artifacts_dir = os.path.expanduser(
                f"~/.sase/projects/{project_name}/artifacts/ace-run/{artifacts_timestamp}"
            )
            ...
            if has_changes and new_cl_name:
                ...
                from sase.commit_utils import save_diff
                ...
                if prompt_result is not None:
                    ...
                    if action == "reject":
                        ...
                        from sase.shared_utils import generate_workflow_tag
                ...
                from sase.sase_utils import make_safe_filename

                safe_name = make_safe_filename(cl_name)
                diff_path = f"~/.sase/diffs/{safe_name}-{timestamp}.diff"
