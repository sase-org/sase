"""Formatting for agent-session fork sources (sequential member chains)."""
...
def format_agent_session_fork_source(
    ...
    *,
    ...
    if not isinstance(raw_members, list) or not raw_members:
        raise ValueError(f"Agent-session fork source '{name}' has no members")
    members = sorted(
        (_require_agent_session_member(member, name) for member in raw_members),
        key=lambda member: Path(fork_source_string(member, "artifact_dir")).name,
    )

    raw_excluded = source.get("excluded", [])
    if not isinstance(raw_excluded, list):
        raise ValueError(f"Agent-session fork source '{name}' has invalid exclusions")
    excluded = [
        _require_excluded_agent_session_member(member, name) for member in raw_excluded
    ]
    total_members = len(members) + len(excluded)
    header_rows = [
        f"## Source {index} of {count} — agent session `{name}`",
        "",
...
def _require_agent_session_member(
    value: object, agent_session_name: str
) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise ValueError(
            f"Agent-session fork source '{agent_session_name}' has an invalid member"
        )
    return value


def _require_excluded_agent_session_member(
    value: object, agent_session_name: str
) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise ValueError(
            f"Agent-session fork source '{agent_session_name}' has an invalid exclusion"
        )
    return value


def _format_agent_session_member(
