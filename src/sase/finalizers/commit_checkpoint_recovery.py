...
def _recovery_request(
    ...
    *,
    ...
    expected_subject = _first_line(payload.get("message"))
    accepted_subject = _first_line(decision.get("message"))
    method = checkpoint.method
    accepted_action = decision.get("action")
    payload_matches = method in {
        "create_commit",
        "create_pull_request",
        "create_proposal",
    } and (accepted_action in {None, "commit"} or method == "create_pull_request")
    if expected_subject and accepted_subject:
        subject_matches = expected_subject == accepted_subject
    else:
        subject_matches = not expected_subject or not accepted_subject
