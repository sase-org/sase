...
class _PagerBodyScroll(VerticalScroll):
    ...
    def on_resize(self, _event: Resize) -> None:
        screen = self.screen
        if isinstance(screen, PagerScreen):
            screen._ensure_body()
            screen._update_subject()


class _PagerBody(Static):
    ...
    def get_content_height(self, container: Size, viewport: Size, width: int) -> int:
        ...
        screen = self.screen
        if not isinstance(screen, PagerScreen) or screen._body_width is None:
            return height
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="pager-root"):
            ...
            with _PagerBodyScroll(id="pager-body-scroll"):
                yield _PagerBody(id="pager-body")
