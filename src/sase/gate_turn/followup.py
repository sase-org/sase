...
from sase.gate_turn.followup_prompt import (
    GateOptionOutcome,
    compose_gate_followup_prompt,
    format_gate_outcome_line,
)
from sase.gate_turn.log import GATE_TURN_LOG_FILENAME, gate_turn_output_tail
...
from sase.turns.prompt import turn_routing_prefix
...
def launch_gate_followup_agent(
    ...
    *,
    ...
    def _spawn(
        ...
        return spawn_turn_agent_session_successor(
            agent_session=lane,
...
def _base_prompt_kwargs(
    ...
    *,
    ...
    gate_log_path = (
        str(Path(artifacts_dir) / GATE_TURN_LOG_FILENAME)
    ...
    return {
        ...
        "output_text": (
            gate_turn_output_tail(artifacts_dir) if "tail" in policy.output else ""
        ),
        "tail_lines": 200,
...
def _compose_policy_prompt(
    ...
    **kwargs: Any,
    ...
    if policy.raw_prompt:
        ...
        prefix = turn_routing_prefix(fork_target, None, None, next_model)
