...
def reclaim_pending_gate_turns(
    *,
    now: float | None = None,
    ...
    records = (
        ...
        else list_gate_turns(project=project)
    )
...
def _reclaim_one(
    ...
    *,
    ...
    if disposition == DISPOSITION_CANCELLED_TIMEOUT:
        settle_gate_turn(record, gate_state="timeout", reason="gate timed out")
        return "timeout"
    if disposition == DISPOSITION_CANCELLED_LOST:
        settle_gate_turn(record, gate_state="lost", reason="gate deadline grace passed")
        return "lost"
    ...
    *,
    ...
    try:
        ...
        if disposition == DISPOSITION_ANSWERED:
            ...
            return "answered"
        return "accepted_unfinished"
    settle_gate_turn(record, gate_state=settle_state, reason=settle_reason)
    return settle_state
...
def reconcile_incomplete_gate_handoffs(
    *,
    ...
    error_details: list[str] = []
    active_snapshot = (
        snapshot if snapshot is not None else load_gate_turn_snapshot(project=project)
    )
    ...
    records = [
        record for record in pass_state.snapshot.gate_turns if record.is_terminal
    ]
    records.sort(key=lambda record: (record.timestamp, record.artifacts_dir))
    grouped: dict[str, list[GateTurnRecord]] = {}
    for record in records:
...
def _diagnose_one(
    ...
    *,
    ...
    with with_gate_followup_lock(record.artifacts_dir):
        live = read_gate_turn_marker(record.project_name, record.artifacts_dir)
        if live is None:
            return "skipped"
...
def _resolve_agent_session_records(
    ...
    *,
    ...
    pass_state.snapshot = load_gate_turn_snapshot(project=pass_state.project)
...
def _changed_since(live: GateTurnRecord, taken_at: float) -> bool:
