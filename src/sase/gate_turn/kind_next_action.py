...
own body, so ``sase.gate_turn`` never imports a kind's module eagerly while
...
def _plan_next_action(**kwargs: Any) -> str | None:
    from sase.plan_gate_turn.followup import plan_next_action
...
def _question_next_action(**kwargs: Any) -> str | None:
    from sase.question_gate_turn.followup import question_next_action
