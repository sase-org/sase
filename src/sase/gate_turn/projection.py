...
from sase.gate_turn.handoff import classify_gate_handoff, resume_command_for
...
def gate_turn_followup_needs_attention(record: GateTurnRecord) -> bool:
...
def _gate_turn_holds_workspace_claim(
    record: GateTurnRecord, *, needs_attention: bool | None = None
) -> bool:
    ...
    if pid is None:
        return False
    incomplete = (
        gate_turn_followup_needs_attention(record)
...
def gate_turn_runtime_json(record: GateTurnRecord) -> dict[str, Any]:
    ...
    needs_attention = gate_turn_followup_needs_attention(record)
    return {
        "gate_id": record.gate_id,
        "short_id": short_gate_turn_id(record.gate_id),
        ...
        "holds_workspace_claim": _gate_turn_holds_workspace_claim(
