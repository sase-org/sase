    ...
    for entry in _iter_children(root):
        ...
        rows.append(
            _DiskFootprintRow(
                ...
                owner="managed_tmp_reaper",
                horizon=_format_horizon_seconds(horizon_seconds),
                reclaim="sase disk reap --apply",
            )
        )
    return tuple(rows)
...
def _sase_state_rows(
    *, tree_size_fn: Callable[[Path], int]
    ...
    for project_dir in _iter_children(projects):
...
def _cargo_stray_rows(
    ...
    *,
...
) -> tuple[tuple[_DiskFootprintRow, ...], int, bool]:
    ...
    resolved_excludes = tuple(_resolve_soft(path) for path in excludes)
    ...
    while stack:
        ...
        if path.name in {".git", ".cargo", ".rustup"}:
            continue
        resolved = _resolve_soft(path)
        if any(_is_relative_to(resolved, excluded) for excluded in resolved_excludes):
            continue
        ...
        if depth >= max_depth:
            continue
        for child in reversed(_iter_children(path)):
            try:
...
def _managed_tmp_reap_step(*, apply: bool) -> _DiskReapStep:
    if not apply:
        return _DiskReapStep(
            owner="managed_tmp_reaper",
            mode="dry_run",
...
def _proc_runtime_reap_step(*, apply: bool) -> _DiskReapStep:
...
def _artifact_run_reap_step(*, apply: bool, project: str | None) -> _DiskReapStep:
...
def _tree_size_walk(path: Path) -> int:
    ...
    if not stat.S_ISDIR(entry_stat.st_mode):
        return 0
    return sum(_tree_size_walk(child) for child in _iter_children(path))


def _format_horizon_seconds(seconds: float) -> str:
    if seconds % 86_400 == 0:
...
def _resolve_soft(path: Path) -> Path:
