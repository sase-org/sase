...
def _plan_qa_rounds(head_artifacts_dir: str) -> list[QARound]:
    ...
    for artifacts_dir in reversed(_plan_gate_turn_chain(head_artifacts_dir)):
        meta = _read_meta(artifacts_dir)
        path = _meta_get(
            meta, "plan_gate_turn_qa_rounds_path", "plan_gate_turn_qa_rounds_path"
        )
...
def _legacy_prompt_path(artifacts_dir: str, which: str) -> str | None:
    ...
    names = {
        "original": "plan_gate_turn_original_prompt.md",
        "current": "plan_gate_turn_current_prompt.md",
        "qa_rounds": "plan_gate_turn_qa_rounds.json",
    }
...
def _rebuild_exec_inputs(
    ...
    agent_name = _str(
        _meta_get(meta, "plan_gate_turn_agent_name", "plan_gate_turn_agent_name")
    )
    ...
    agent_meta = _meta_get(
        meta, "plan_gate_turn_agent_meta", "plan_gate_turn_agent_meta"
    )
    base_meta = dict(agent_meta) if isinstance(agent_meta, Mapping) else {}
    for key, new_key, legacy_key in (
        ("model", "plan_gate_turn_agent_model", "plan_gate_turn_agent_model"),
        (
            "llm_provider",
    ...
    ctx = AgentExecContext(
        ...
        project_file=_str(
            _meta_get(
                meta, "plan_gate_turn_project_file", "plan_gate_turn_project_file"
            )
        )
        or "",
        workspace_dir=_str(
            _meta_get(
                meta, "plan_gate_turn_workspace_dir", "plan_gate_turn_workspace_dir"
            )
        )
        or os.getcwd(),
        output_path=_str(
            _meta_get(meta, "plan_gate_turn_output_path", "plan_gate_turn_output_path")
        )
        or "",
        workspace_num=_int(
            _meta_get(
                meta, "plan_gate_turn_workspace_num", "plan_gate_turn_workspace_num"
            )
        )
        or 0,
        timestamp=_str(
            _meta_get(meta, "plan_gate_turn_timestamp", "plan_gate_turn_timestamp")
        )
        or "",
        update_target="",
        project_name=_str(
            _meta_get(
                meta, "plan_gate_turn_project_name", "plan_gate_turn_project_name"
            )
        )
        or "",
        is_home_mode=bool(
            _meta_get(
                meta, "plan_gate_turn_is_home_mode", "plan_gate_turn_is_home_mode"
            )
            or False
        ),
        ...
        or "",
        vcs_tag=_str(
            _meta_get(meta, "plan_gate_turn_vcs_tag", "plan_gate_turn_vcs_tag")
        ),
        agent_name=agent_name,
        agent_model=_str(
            _meta_get(meta, "plan_gate_turn_agent_model", "plan_gate_turn_agent_model")
        ),
    ...
    state = LoopState(
        ...
        sdd_spec_path=_str(
            _meta_get(
                meta, "plan_gate_turn_sdd_spec_path", "plan_gate_turn_sdd_spec_path"
            )
        ),
        original_prompt=_plan_original_prompt(artifacts_dir),
        ...
        feedback_round=_int(
            _meta_get(
                meta, "plan_gate_turn_feedback_round", "plan_gate_turn_feedback_round"
            )
        )
        or 0,
    )
...
def _plan_gate_turn_chain(head_artifacts_dir: str) -> tuple[str, ...]:
    chain: list[str] = []
    seen: set[str] = set()
