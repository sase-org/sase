...
2. Form a candidate title and scope, then gather reproducible evidence. If a file
...
6. Only when neither branch applies, choose a size and create an evidence-rich draft,
   ...
   nearly mechanical; `small` is focused and straightforward; `medium` is substantial
   but bounded with a known design; `large` needs its own planning handoff; `xlarge` is
   rare and likely needs epic decomposition or deliberately deferred planning.
