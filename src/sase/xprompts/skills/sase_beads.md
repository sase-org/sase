...
`compact` is the default and prints `[type_icon] [type] [status_icon] [id] · [title][ ← parent_id]`, where type icons
are `▸` plan, `↳` phase, and `◆` task, and status icons are `○` open, `◎` claimed, `◇` ready, `◐` in_progress, and `✓`
closed. The type word is exactly the value accepted by `--type`. `--color auto|always|never` (`-c`) controls compact
output color and honors `NO_COLOR`; piped or non-TTY output is colorless by default. `full` prints the same detail
blocks as `sase bead show`, separated by 60-dash rules. `json` emits an envelope with `count`, `total`, `statuses`,
`implied_status_closed`, and flat issue objects in `results`.
