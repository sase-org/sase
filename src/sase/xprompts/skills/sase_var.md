...
- Every producer's variables live under `agents` keyed by agent name. `%id:build-@` is
  `{% raw %}{{ agents["build"].result_path }}{% endraw %}`, not `build-0`. Dotted,
  hyphenated, and digit-leading names use bracket access:
  `{% raw %}{{ agents["research.final"].report_path }}{% endraw %}`,
  `{% raw %}{{ agents["0n.cld"].report_path }}{% endraw %}`. Identifier-safe keys also
  support `{% raw %}{{ agents.build.result_path }}{% endraw %}`.
