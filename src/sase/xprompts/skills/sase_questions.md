...
question handoff and creates a **question gate turn** — a named, non-LLM member of your
...
asked so far, continuously numbered — the same `## Your next action` shape a gate turn
always composes. Do not create a question gate turn yourself with `/sase_gate`;
...
turn's settlement observes the terminal response mechanically.
