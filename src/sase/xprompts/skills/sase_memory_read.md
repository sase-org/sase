...
description:
  Guide audited SASE long-term memory reads through `sase memory read`. Use when instructions
  require reading long-term memory context or mention the long-memory read procedure.
skill: true
log_skill_use: false
---
...
- Read canonical long-term memory only through `sase memory read`; it checks project memory first
  and then home memory.
...
- When the note has nested long-term child notes, `sase memory read` appends a `## Children` section
  listing them.
