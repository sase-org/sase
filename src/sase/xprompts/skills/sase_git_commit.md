...
4. **Run the commit** — Execute:
   ...
   - `-B {keep,close}` / `--bead-action {keep,close}`: Required when `$SASE_BEAD_ID`
     is set. `keep` leaves the assigned bead unchanged; `close` closes it after a
   ...
   Exit codes:
   - `0`: Commit succeeded.
   - `1`: Commit failed with a printed reason. Fix the cause and re-run the same command. If `-B close` was
     requested, failed lifecycle validation or close publication must be resolved
     explicitly.
