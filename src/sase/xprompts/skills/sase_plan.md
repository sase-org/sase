...
5. **Submit the validated plan**:
   ...
   runner treats that signal as an intentional handoff. With the `gate_shell_handoff`
   beta flag enabled, it creates the tier-specific `PlanApproval` or `EpicApproval` gate
   shell and ends this agent; the shell owns review settlement and launches the feedback
   replanner or approved tale coder. `%auto` remains synchronous and continues in this
   process without a detached agent. With the flag disabled, the runner uses the legacy
   blocking gate wait and then continues with feedback or the approved follow-up. Do not
