...
Use this skill when you need to plan before implementing. This replaces {{ provider_name }}'s native plan mode, which is disabled.
...
2. **Write a self-contained plan** to `sase_plan_<name>.md` (descriptive underscore name).
   - You should construct the same type of implementation plan that you would have written in {{ provider_name }}'s native plan mode.
