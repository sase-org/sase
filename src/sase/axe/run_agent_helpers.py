...
def finalize_handoff_artifacts_as_completed(artifacts_dir: str) -> None:
    """Unconditionally finalize planner artifacts as completed at handoff.

    Unlike :func:`normalize_handoff_interruption_state` (which only rewrites
    SIGTERM-induced ``failed`` markers), this helper flips any non-terminal
    ``workflow_state.json`` and ``prompt_step_*.json`` markers to
    ``completed``.  Pre-existing terminal statuses (``completed`` or
    ``failed`` from a real error) are preserved so genuine failures remain
    visible.

    Safe to call from ``handle_plan_marker`` because, by then, the planner
    has submitted a plan and is handing off — its own work is genuinely
    done.  In-process SDK providers where SIGTERM bypassed the executor's
    ``except`` arm would otherwise leave these markers stuck at
    ``in_progress`` / ``waiting_hitl``, making the TUI display the plan
    child as ``(RUNNING)`` indefinitely.
    """
    index_dirty = False
    state_path = Path(artifacts_dir) / "workflow_state.json"
    try:
