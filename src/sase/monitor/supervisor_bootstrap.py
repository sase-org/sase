"""Bootstrap the monitor supervisor before importing the monitor stack.

This file is executed by filename, not with ``python -m``. That keeps
``sase.monitor.__init__`` out of the startup path until after the signal
...
_IMPORT_DELAY_ENV = "SASE_MONITOR_BOOTSTRAP_IMPORT_DELAY_SECONDS"
...
def main() -> int:
    ...
    return run_supervisor(args.artifacts_dir, startup_signal=_STARTUP_SIGNAL)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Bootstrap one SASE monitor supervisor."
    )
