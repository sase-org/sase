...
_PROJECT_COMMIT_MESSAGE = "chore: run sase init memory"
...
def _deploy_to_project_repo(
    ...
    try:
        ...
        print(f"{_COMMAND_LABEL}: git not found on PATH", file=sys.stderr)
        return 1
    ...
    if repo_check.returncode != 0 or not repo_check.stdout.strip():
        ...
        print(
            f"{_COMMAND_LABEL}: {project_result.root} is not a git repo{suffix}",
            file=sys.stderr,
        )
        return 1
    ...
    for path in stage_paths:
        ...
        if add.returncode != 0:
            print(
                f"{_COMMAND_LABEL}: git add failed for {path}: {add.stderr.strip()}",
                file=sys.stderr,
            )
            return 1
    ...
    if staged.returncode == 0:
        print(f"{_COMMAND_LABEL}: nothing to commit in {git_root}")
        return 0
    if staged.returncode != 1:
        print(
            f"{_COMMAND_LABEL}: staged diff check failed: {staged.stderr.strip()}",
            file=sys.stderr,
        )
        return 1
    ...
    commit = subprocess.run(
        ["git", "-C", str(git_root), "commit", "-m", _PROJECT_COMMIT_MESSAGE],
        capture_output=True,
        text=True,
        check=False,
    )
