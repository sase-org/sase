...
def _emit_json(value: object) -> NoReturn:
    _emit_json_with_exit(value, exit_code=0)


def _emit_json_with_exit(value: object, *, exit_code: int) -> NoReturn:
    print(
        json.dumps(
...
def _handle_show(args: argparse.Namespace) -> NoReturn:
    result = artifact_facade.artifact_show(_index_path(args), args.artifact_id)
    if args.json:
        _emit_json(result)
    print(_format_detail(result, artifact_id=args.artifact_id))
    sys.exit(0)
...
def _handle_rebuild(args: argparse.Namespace) -> NoReturn:
    ...
    result = artifact_facade.artifact_rebuild(_index_path(args), request)
    _emit_mutation_result(result, json_output=args.json)


def _handle_doctor(args: argparse.Namespace) -> NoReturn:
    result = artifact_facade.artifact_doctor(
        _index_path(args), ArtifactDoctorOptionsWire()
    )
    exit_code = 0 if _doctor_ok(result) else 1
    if args.json:
        _emit_json_with_exit(result, exit_code=exit_code)
    print(_format_doctor(result))
    sys.exit(exit_code)


def _format_node_table(nodes: object) -> str:
...
def _format_detail(detail: ArtifactDetailWire | object, *, artifact_id: str) -> str:
    ...
    if children:
        lines.append(_indent(_format_node_table(children), "  "))
    else:
        lines.append("  none")
...
def _format_doctor(doctor: ArtifactDoctorWire | object) -> str:
    data = _as_dict(doctor)
    issues = _as_list(data.get("issues"))
    lines = [
        f"status: {'OK' if _doctor_ok(data) else 'FAIL'}",
        f"issues: {len(issues)}",
    ]
    if issues:
...
def _doctor_ok(doctor: ArtifactDoctorWire | object) -> bool:
    data = _as_dict(doctor)
