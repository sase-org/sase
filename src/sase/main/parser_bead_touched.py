...
def register_bead_touched_parser(
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "contains, never what it wishes it contained. viewed comes from "
            "a machine-local log, so a remote agent's views are not visible "
            "while its mutations are. linked currently never appears: bead "
            "link events are owner-attributed and task sase-159 tracks that. "
            "The CLI lists touched beads only; the panel also marks assigned "
            "but untouched beads with an own chip."
        ),
        epilog=(
            "examples:\n"
