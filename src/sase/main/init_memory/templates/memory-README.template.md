...
- A memory web is a flat descriptor note (`sase/memory/<web>.md`) plus a sibling strand directory
  ...
  `glossary` web is one: `sase/memory/glossary.md` is its always-loaded descriptor, and each term is a strand file under
  `sase/memory/glossary/`. A strand's body never inlines into `AGENTS.md`. Fetch a strand on demand with
...
- `type`: `core` for always-loaded ordinary notes or `reference` for read-on-demand ordinary notes. The legacy values
  `short` and `long` are still accepted and mean the same thing.
...
- `description`: required for reference notes and used in generated agent instructions and this README. Reference-note
  descriptions may be Markdown blocks authored as YAML literal block scalars. Generated agent instructions collapse
  top-level reference descriptions to one paragraph and reject multi-block descriptions; audited child-reference reads
  still render block descriptions verbatim.
...
- Web descriptor notes: {{ web_descriptor_notes }}
