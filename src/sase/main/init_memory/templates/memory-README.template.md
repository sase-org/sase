...
- `sase/memory/sase.md` and `sase/memory/sase_beads.md` are generated by SASE. The first captures linked repositories
  plus workspace rules; the second provides shared bead workflow guidance.
...
{{ memory_notes }}
{% else %}
No memory notes exist yet. Run `sase memory init` to create the generated `sase/memory/sase.md` and
`sase/memory/sase_beads.md` notes.
{% endif %}
