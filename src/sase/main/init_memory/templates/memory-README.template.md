...
- `sase/memory/sase.md` is always generated by SASE and captures linked repositories plus workspace rules.
  `sase/memory/sase_beads.md` is generated only for SASE-managed project repositories and provides shared bead
  workflow guidance.
...
{{ memory_notes }}
{% else %}
No memory notes exist yet. Run `sase memory init` to create the generated `sase/memory/sase.md` note (and
`sase/memory/sase_beads.md` for SASE-managed project repositories).
{% endif %}
