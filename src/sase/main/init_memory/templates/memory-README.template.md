...
What a memory is and how it renders are independent properties. Today every memory is a note; `type:` declares only
whether that note renders as core memory or reference memory.
...
- `sase memory read <note>.md --reason <reason>` reads a reference note and records an audited access event.
