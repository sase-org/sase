"""Shared scan/cache loop for rules projecting published agent `meta.json`."""
...
_AgentRowsFn = Callable[[V2RunMetadataPayload, str], list[dict[str, str]]]
"""Build cached row dicts from one agent's parsed metadata and `created_at`."""


def project_agent_meta_rows(
    inputs: ProjectionInputs,
    *,
    rule_id: str,
    rows_for_metadata: _AgentRowsFn,
) -> tuple[ProjectedEdge, ...]:
    """Scan published agents, reusing *rule_id*'s per-agent stat cache."""
    ...
    cached_signature, cached_rows = read_rule_cache(inputs.project_key, rule_id)
    ...
    try:
        ...
        return _edges_from_rows(cached_rows, rule_id)
    ...
    for name, meta_path in meta_paths.items():
        ...
        if cached_stats.get(name) == stat_key:
            ...
            continue
        changed = True
        rows.extend(_rows_for_agent(meta_path, stat.st_mtime, rows_for_metadata))

    if changed:
        write_rule_cache(inputs.project_key, rule_id, signature=signature, rows=rows)
    return _edges_from_rows(rows, rule_id)
...
def _rows_for_agent(
    meta_path: Path, mtime: float, rows_for_metadata: _AgentRowsFn
) -> list[dict[str, str]]:
    try:
    ...
    return rows_for_metadata(metadata, created_at)


def _edges_from_rows(
    rows: list[dict[str, str]], rule_id: str
) -> tuple[ProjectedEdge, ...]:
    return tuple(
        ProjectedEdge(
...
__all__ = ["project_agent_meta_rows"]
