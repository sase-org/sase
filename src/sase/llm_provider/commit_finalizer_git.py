    ...
    *,
    ...
    for repo in before.repos:
        if repo.path in remaining_paths:
            continue
        before_head = before_heads.get(repo.path, _UNKNOWN_HEAD_SENTINEL)
        after_head = _git_head_commit_id(repo.path)
        if before_head == after_head or after_head == _UNKNOWN_HEAD_SENTINEL:
            evidence.append(
...
def _new_commit_messages(
    ...
    revision = (
        after_head
        if before_head == _UNKNOWN_HEAD_SENTINEL
