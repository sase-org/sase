...
from sase.gate_turn.store import load_gate_turn_snapshot
...
def _reason_for(
    summary: GateTurnReclaimSummary, handoff: GateHandoffReconcileSummary
) -> str | None:
    if summary.errors or handoff.errors:
        return "reclaim_errors"
    if not summary.scanned and not handoff.scanned:
        return "budget_exhausted" if handoff.deferred else "no_pending_gate_turns"
    return None


@builtin_chop("gate_turn_reclaim")
...
def main() -> None:
    run_builtin_chop("gate_turn_reclaim")
