...
When your prompt matches keywords from memory-tagged xprompts, sase resolves the matched content (via shell
substitution) and inlines it into a temp file. A `DYNAMIC MEMORY: @<temp_file_path>` line is then injected at the bottom
of your prompt. The `@` reference resolves to the already-expanded content automatically. You do not need to take any
action -- the content is included in your context.
