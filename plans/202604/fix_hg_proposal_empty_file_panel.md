...
In `sase ace`, when a Mercurial agent (Gemini + sase-google plugin) successfully creates a proposal via the
...
1. **During agent execution**, `/sase_hg_commit` → `sase commit` runs. Inside
   ...
   - The proposal is then created via the sase-google plugin's `vcs_create_proposal` hook, and the workspace is cleaned.

2. **After the agent finishes**, the `#propose` embedded workflow's post-steps run. The plugin's
   `../sase-google/src/sase_google/xprompts/propose.yml` overrides the in-repo `src/sase/xprompts/propose.yml`. The
...
- Is overridden by the sase-google plugin copy on hg workspaces.
...
1. On Mercurial / sase-google, the file panel populates after `/sase_hg_commit` creates a proposal.
...
4. Keep the two propose.yml files (in-repo vs. sase-google plugin) consistent in spirit so future edits don't drift.
...
#### 1. sase-google plugin: `../sase-google/src/sase_google/xprompts/propose.yml`
...
The current sase-google `propose.yml` calls `sase_propose_workflow --create` unconditionally when `has_changes` is true.
...
| File                                                  | Type                              |
| ----------------------------------------------------- | --------------------------------- |
| `../sase-google/src/sase_google/xprompts/propose.yml` | rework gates + split commit path  |
...
| `../sase-google/tests/xprompts/test_propose_yml.py`   | new/extended coverage             |

## Post-Implementation

- Run `just check` in `sase_100` (required before reporting).
- Run `just check` in `../sase-google` (plugin repo).
