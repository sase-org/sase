...
`#split` setup fails because `sase_google_update` is called with `cl_name="sase"` instead of the expected Mercurial ref
(for example, `yserve_batch_create_update`).

Observed runtime evidence:

- Workflow inputs show `cl_name="sase"`.
- Failing command is `sase_google_update sase`.
