...
sase_101: notify_*()  →  ~/.sase/notifications/notifications.jsonl
                                     ↓
sase-gchat outbound chop polls store  →  format_notification()  →  gchat_client.send_message()
                                                                                ↓
                                                                      gchat CLI → Google Chat
```

- Single notification creation chokepoint: `sase/notifications/senders.py` (sase_101).
- Single send chokepoint for outbound: `sase_gchat/gchat_client.py::send_message()` (sase-gchat).
- Single formatting dispatcher: `sase_gchat/formatting.py::format_notification()` (sase-gchat).
...
- Inbound gchat agent launches (`sase-gchat/.../sase_gc_inbound.py`)
...
which the existing outbound filter already honours (`sase-gchat/outbound.py:43`). No config flag is needed for the
initial cut.
...
### Part B — Launch formatter (sase-gchat)

Add `_format_agent_launch(n)` in `sase_gchat/formatting.py` and route to it from `format_notification()` when
...
### Part C — Universal sase identifier emoji (sase-gchat)
...
**Where to apply it.** Centralise in `sase_gchat/gchat_client.py`:
...
- `src/sase_gchat/formatting.py` — add `_format_agent_launch`, route from `format_notification`.
- `src/sase_gchat/gchat_client.py` — add `SASE_PREFIX`, `_ensure_sase_prefix`, apply in `send_message` and
  `edit_message`.
