...
The main source of product/technical inspiration is `research/gchat_integration_review.md`. Current spot-checks against
...
- `sase-telegram` already advances outbound notification high-water marks per delivered notification in
  ...
  reveal a regression.
- `sase-gchat` already avoids auto-naming `%r:N` repeat prompts in `src/sase_gchat/scripts/sase_gc_inbound.py`, matching
...
`research/gchat_integration_review.md`, and the relevant repo files before editing. If a phase modifies a plugin repo,
it must run `just install` if needed and then `just check` in that plugin repo before reporting completion.

## Guiding Goals

1. Make `sase-gchat` feel like a first-class sibling of `sase-telegram`, not a minimal transport port.
...
- `just check` passes in `../sase-gchat`.
...
**Repos:** `../sase-gchat` and `../sase-telegram`
