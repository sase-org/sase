...
attached to the user's request and referenced in `research/commit_conflict_resume.md`) resolves the conflict and ends up
...
This plan implements **Solution A** from `research/commit_conflict_resume.md`: a resumable `CommitWorkflow` with a JSON
...
- Hg-side `vcs_finalize_commit` (re-mailing, re-uploading) is out of scope for this plan — it lives in the `sase-google`
...
- The user's request and the `sase ace` snapshot that motivated the work are in `research/commit_conflict_resume.md`.
...
- **Hg `vcs_finalize_commit` not implemented here.** The git mixin is the only plugin that implements
  `vcs_finalize_commit` in Phase 3. The hg plugin lives in `sase-google` (see `memory/long/external_repos.md`) and needs
...
- Hg-side `vcs_finalize_commit` (lives in `sase-google`, separate change).
