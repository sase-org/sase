...
6. `workflow_executor_steps_prompt.py:200` — `_expand_embedded_workflows_in_prompt` runs the embedded workflow's
   pre-steps. For `#hg:yserve_read_grow` (defined in `sase-google_100/src/sase_google/xprompts/hg.yml`):
   - `setup` (Python) emits `_chdir=<workspace_dir>` → executor calls `os.chdir` (see
     `workflow_executor_steps_script.py:142-149`).
   - `prepare` (bash) runs `sase_google_clean`, which executes `hg update --clean .` and `hg clean` — **removing
...
- Teaching `sase_google_clean` (in the `sase-google` plugin) to preserve `.sase/`. That would be defense in depth, but
