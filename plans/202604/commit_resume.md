...
`research/commit_conflict_resume.md`) resolves the conflict and ends up with a clean local commit, but no ChangeSpec
...
- **Hg `mail`/`upload` resume isn't covered.** This first iteration adds `vcs_finalize_commit` only on the git mixin.
  The hg dispatch (in the `sase-google` plugin repo, not in `sase_100`) needs an equivalent override that re-runs `mail`
