...
When a workflow step agent completes after a plan/questions handoff (SIGTERM), the TUI "AGENT CHAT" section displays
raw JSON metadata (e.g., `{"meta_workspace": "100", "meta_project": "bug"}`) instead of the agent's actual reply text.
...
In `_agent_display.py`, tighten the fallback condition so `format_output(agent.step_output)` is only used when the
step output contains displayable content (has `_raw` or `_data` key), not when it only contains `meta_*` fields or
other metadata.
