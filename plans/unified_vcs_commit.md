...
Replace all VCS-specific commit/propose/cl xprompt workflows (`#commit`, `#propose`, `#cl` in sase-google; `#pr` in
...
- **sase-google**: Has `#commit` (amend CL with COMMITS entry), `#propose` (create proposal), `#cl` (create CL)
...
**Scope**: sase core + sase-google + sase-github.
...
- `commit.yml` — Sets `$SASE_COMMIT_METHOD=create_commit`. Has a `prompt_part` step with VCS-agnostic instructions for
  making changes and preparing a commit. Content should be similar to the instruction portion of the current sase-google
...
**Files to create/modify in sase-google:**
...
- `just check` passes in sase core, sase-google, sase-github
...
**Scope**: sase core + sase-google + sase-github + chezmoi.

### Part A: Remove old workflows from sase-google

- Delete `src/sase_google/xprompts/commit.yml`
- Delete `src/sase_google/xprompts/propose.yml`
- Delete `src/sase_google/xprompts/cl.yml`
- Remove any scripts only used by these workflows (e.g., `sase_google_commit_workflow`, `sase_propose_workflow`,
...
- `just check` passes in all repos (sase core, sase-google, sase-github)
