...
**sase-google repo (../sase-google):**

- `src/sase_google/default_config.yml` — Add `precommit_command: "sase_hg_fix"`
- Create a new `sase_hg_fix` script: a thin wrapper that checks if we're in an hg repo (`hg root` succeeds) and runs
  ...
  (following the same pattern as other scripts in sase-google — check how they register scripts there).
...
**sase-google repo (../sase-google/src/sase_google/plugin.py):**
