...
Every `sase bead create` requires `-w/--reason`: one or two sentences explaining why the
bead was filed, distinct from its title. `@<path>` reads the reason from a file, blank
or over-2000-character reasons are rejected, and historical beads with no reason still
load.
