...
8. **Expensive Commands Are Recorded Before They Are Admitted** (`record-before-admit`)
   - _[partly superseded by `guarded-recipes`]_ The sase tool control plane lands its
     ...
     already exists.
9. **Guarded Recipes Refuse Raw Agent Runs** (`guarded-recipes`)
...
13. **Memory Webs Render In Their Own Section** (`webs-render-in-their-own-section`)
...
14. **No Retrieval Mechanism Before Its Corpus** (`corpus-before-mechanism`)
    - SASE does not build memory retrieval or linking machinery ahead of a corpus that
      demonstrably needs it.
15. **Size Aliases Descend The Effort Ladder** (`size-alias-effort-ladder`)
