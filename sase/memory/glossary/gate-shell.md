...
A gate shell is a session-attached shell — like a proc shell or an agent shell — that
...
to the next session member. Creating a gate shell from inside an agent hands off and
kills that agent's turn; if the creator had no agent session yet, attaching the gate
shell promotes it into one. Members are named `<session>--gate`, then `--gate-0`,
