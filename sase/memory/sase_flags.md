...
The registry owns key, kind, default, scope, description, and bead id. The flag bead
owns `remove_by_date` and `remove_by_release`. A flag is due only after both thresholds
have passed; the boolean value never changes because a deadline passed.
