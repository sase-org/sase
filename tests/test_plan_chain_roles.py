...
def test_agent_family_role_for_suffix_accepts_new_and_legacy_suffixes() -> None:
    ...
    assert agent_session_role_for_suffix("-q") is None
    assert agent_session_role_for_suffix("--q") is None
    assert agent_session_role_for_suffix("--2") == "feedback"
    assert agent_session_role_for_suffix("--2", agent_session_role="review") == "review"
    assert agent_session_role_for_suffix("--2", agent_session_role="phase") == "phase"
    assert agent_session_role_for_suffix("-2") == "feedback"
    assert agent_session_role_for_suffix(".code") == "code"
...
def test_agent_family_suffix_token_strips_known_separators() -> None:
    ...
    assert agent_session_suffix_token(".xyz") == "xyz"
    assert agent_session_suffix_token("-2") == "2"
    assert agent_session_suffix_token(None) is None
    assert agent_session_suffix_token("") is None
    assert agent_session_suffix_token("--") is None


def test_new_plan_feedback_suffixes_classify_as_feedback() -> None:
    assert canonical_plan_chain_suffix("--plan-0") == "--plan-0"
    assert agent_session_role_for_suffix("--plan-0") == "feedback"
...
def test_numeric_suffixes_are_feedback_or_custom_members() -> None:
    assert agent_session_role_for_suffix("--0") is None
    assert agent_session_role_for_suffix("--1") is None
    assert agent_session_role_for_suffix("--2") == "feedback"
    assert agent_session_role_for_suffix("--2", agent_session_role="review") == "review"
...
def test_historical_canonical_q_suffix_remains_readable_as_custom_token() -> None:
    ...
    assert agent_session_suffix_token("--q") == "q"
    assert plan_chain_agent_name("agent", "--q") == "agent--q"
    assert agent_session_base("agent--q") == "agent"
    assert is_agent_session_member("agent--q")
...
def test_monitor_suffix_classifies_as_monitor_phase() -> None:
    ...
    assert agent_session_role_for_suffix("--mon") == "monitor"
...
def test_monitor_sequence_suffixes_classify_as_monitor_phase() -> None:
    ...
    assert agent_session_role_for_suffix("--mon-0") == "monitor"
...
def test_gate_suffixes_classify_as_gate_phase_members() -> None:
    ...
    assert agent_session_role_for_suffix("--gate") == "gate"
    assert agent_session_role_for_suffix("--gate-0") == "gate"
...
def test_agent_family_helpers_parse_only_known_suffixes() -> None:
    ...
    assert agent_session_base("agent-code") is None
    assert not is_agent_session_member("agent-2")
    assert agent_session_base("agent-code", include_legacy_dash=True) == "agent"
    assert is_agent_session_member("agent-2", include_legacy_dash=True)
    assert agent_session_base("sase-3r") is None
    assert not is_agent_session_member("historical-name")
