...
from ._helpers import _entry, _seed, prompt_id
...
def test_list_json_has_stable_shape_and_excludes_cancelled(
    ...
    assert payload[0]["id"] == prompt_id("launched prompt one")
...
def test_show_raw_is_byte_exact(
    ...
    handle_prompt_show(argparse.Namespace(id=prompt_id(text), format="raw"))
...
def test_show_markdown_has_metadata_header_and_body(
    ...
    handle_prompt_show(argparse.Namespace(id=prompt_id(text), format="markdown"))

    out = capsys.readouterr().out
    assert f"# Prompt {prompt_id(text)}" in out
...
def test_show_json_includes_full_text(
    ...
    handle_prompt_show(argparse.Namespace(id=prompt_id(text), format="json"))

    payload = json.loads(capsys.readouterr().out)
    assert payload["text"] == text
    assert payload["id"] == prompt_id(text)
