...
from tests.main.repo_handler_helpers import init_git_repo, project_context, repo_record


ROOT = Path(__file__).resolve().parents[2]


def _set_git_origin(path: Path, remote_url: str) -> None:
...
def test_repo_name_resolution_matches_github_alias_from_linked_origin(
    ...
    _set_git_origin(Path(linked.path), "git@github.com:sase-org/sase-core.git")
...
def test_repo_name_resolution_does_not_match_basename_only(
    ...
    host_ctx = project_context(tmp_path)
    linked = repo_record(tmp_path, name="sase-core", kind="linked")
    _set_git_origin(Path(linked.path), "git@github.com:other/sase-core.git")
