...
def _spec(request_id: str) -> GateSpec:
    ...
    # gate-shell row itself with ``make_gate_turn``: mark the spec the way
...
def _make_pending_gate_turn(gate_id: str, member_name: str, lane: str) -> str:
...
def test_cancel_settles_a_pending_gate_turn_as_stopped(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    artifacts_dir = _make_pending_gate_turn("custom-1", "acme--gate", "acme")
...
def test_cancel_an_already_terminal_gate_turn_is_a_no_op(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    artifacts_dir = make_gate_turn(
        "proj",
...
def test_cancel_json_envelope_carries_the_gate_turn(
    ...
    artifacts_dir = _make_pending_gate_turn("custom-2", "beta--gate", "beta")
    patch_gate_turn_project_records(monkeypatch, [artifacts_dir])
    ...
    assert payload["gate_turn"]["gate_state"] == "stopped"
