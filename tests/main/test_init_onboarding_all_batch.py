"""Generic batch-flow tests for ``sase init --all``.

Covers project iteration semantics shared by every command: failure
isolation, non-interactive apply, interactive decline, keyboard
interrupts, and ``--project`` selection. Machine- and service-scoped
offer semantics live in ``test_init_onboarding_all_machine.py`` and
``test_init_onboarding_all_service.py``.
"""
...
def test_batch_check_isolates_failures_and_restores_cwd(
    ...
    beta = _batch_target(
        tmp_path, "beta", display_name="Beta", warnings=("old record",)
    )
    ...
    gamma = _batch_target(tmp_path, "gamma", display_name="Gamma")
    monkeypatch.setattr(
        init_onboarding,
...
def test_batch_task_type_registry_reflects_each_project_cwd(
    ...
    def _target_with_task_type(name: str) -> InitProjectTarget:
        target = _batch_target(tmp_path, name)
...
def test_batch_interactive_decline_reports_remaining_attention(
    ...
    target = _batch_target(tmp_path, "alpha")
    monkeypatch.setattr(
        init_onboarding,
        "resolve_init_project_inventory",
...
def test_named_project_check_visits_only_selected_targets(
    ...
    monkeypatch.chdir(original)
    alpha = _batch_target(tmp_path, "alpha", display_name="Alpha")
    beta = _batch_target(tmp_path, "beta", display_name="Beta")
    gamma = _batch_target(tmp_path, "gamma", display_name="Gamma")
    monkeypatch.setattr(
        init_onboarding,
        "resolve_init_project_inventory",
