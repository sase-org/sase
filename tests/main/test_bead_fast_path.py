...
def test_fast_path_defers_list_to_argparse(monkeypatch) -> None:
    ...
    Rust's ``handle_list`` mirrors the glyph-only compact row grammar, but
    Python still owns the complete list contract, including non-compact
    formats, limits, and implicit closed-bead fallback. Keep all list
    invocations on argparse until that wider behavior reaches parity.
    """
