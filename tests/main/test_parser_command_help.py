...
def test_run_help_shows_prompt_positional_and_beginner_examples() -> None:
    ...
    assert (
        'sase run "+home summarize what this repository does; do not change files"'
        in run_help
    )
...
def test_memory_help_marks_primary_command_and_init_alias() -> None:
    ...
    assert "Resolve one or more memory selectors" in memory_read_help
    assert "falling back to ~/sase/memory/" in memory_read_help
    assert "flat note name (generated_skills.md)" in memory_read_help
