...
def test_build_lsp_argv_prefers_sibling_source_cargo_over_stale_binaries(
    tmp_path: Path,
) -> None:
    args = create_parser().parse_args(["lsp"])
    repo_root = tmp_path / "sase"
    sibling_core = tmp_path / "sase-core"
    (sibling_core / "target" / "debug").mkdir(parents=True)
    (sibling_core / "Cargo.toml").write_text("", encoding="utf-8")
    (sibling_core / "target" / "debug" / "sase-xprompt-lsp").write_text(
        "", encoding="utf-8"
    )
    ...
    def fake_which(name: str) -> str | None:
        return {
            "cargo": "/usr/bin/cargo",
            "sase-xprompt-lsp": "/usr/bin/sase-xprompt-lsp",
        }.get(name)
...
def test_build_lsp_argv_finds_sase_core_from_sibling_env(tmp_path: Path) -> None:
    args = create_parser().parse_args(["lsp"])
    sibling_core = tmp_path / "sase-core_11"
    sibling_core.mkdir()
    (sibling_core / "Cargo.toml").write_text("", encoding="utf-8")

    argv = _build_xprompt_lsp_argv(
        args,
        environ={"SASE_SIBLING_REPO_SASE_CORE_DIR": str(sibling_core)},
...
def test_build_lsp_argv_uses_sibling_binary_when_cargo_is_unavailable(
    tmp_path: Path,
) -> None:
    args = create_parser().parse_args(["lsp"])
    repo_root = tmp_path / "sase"
    sibling_core = tmp_path / "sase-core"
    binary = sibling_core / "target" / "debug" / "sase-xprompt-lsp"
    ...
    argv = _build_xprompt_lsp_argv(
        ...
        environ={},
        which=lambda name: (
            "/usr/bin/sase-xprompt-lsp" if name == "sase-xprompt-lsp" else None
        ),
        repo_root=repo_root,
    )
...
def test_build_lsp_argv_uses_path_binary_without_sibling_source(
    tmp_path: Path,
) -> None:
    args = create_parser().parse_args(["lsp"])
    repo_root = tmp_path / "sase"
    ...
    assert argv == ["/usr/bin/sase-xprompt-lsp"]
