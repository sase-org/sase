"""Tests for web, JSON/rich, and supersession rendering of selector batches."""
...
def test_mixed_note_and_web_order_suppresses_rendered_web_reference(
    tmp_path: Path,
) -> None:
    _seed_decisions_web(tmp_path)
    _write(
    ...
    batch = _resolve(tmp_path, ["decisions:single-turn-agents", "foo.md"])
    output = memory_selector_batch_markdown(batch)
    payload = _json_payload(batch)
    rich = _rich_text(batch)

    assert output.index("---------- MEMORY WEB: decisions") < output.index(
        "---------- MEMORY FILE: foo.md"
    )
    assert "### 1. `decisions:single-turn-agents`" not in output
    assert payload["notes"][0]["linked_references"] == []
