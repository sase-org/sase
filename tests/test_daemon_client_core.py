"""Tests for core low-level local daemon Python client behavior."""
...
def test_rpc_error_exposes_typed_fallback(tmp_path: Path) -> None:
    ...
    thread = serve_one(socket_path, response, {})
...
def test_read_events_sends_subscription_request_and_reads_bounded_batches(
    ...
    thread = serve_stream(socket_path, responses, request_holder)
