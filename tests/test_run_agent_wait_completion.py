"""Tests for run-agent wait completion, kill, and index updates."""
...
from tests._run_agent_wait_helpers import patch_index_updates
...
def test_dependency_wait_updates_index_for_waiting_marker_only(
    ...
    with (
        ...
        patch_index_updates(lambda path: calls.append(path)),
    ):
        wait_for_dependencies(
            ["dep"],
            str(tmp_path),
