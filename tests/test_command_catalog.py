...
def test_jump_commands_use_back_and_forward_defaults_on_every_tab() -> None:
    ...
    assert by_id["app.next_chop_run"].tabs == ("services",)
    assert by_id["app.prev_chop_run"].tabs == ("services",)
...
def test_h_commands_describe_navigation_and_contextual_collapsing() -> None:
    ...
    assert upper.label == (
        "Collapse selected workflow/session, then group sase agents, selected "
    ...
    assert "collapse session" in upper.aliases
