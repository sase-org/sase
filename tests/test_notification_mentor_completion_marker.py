...
def test_marker_isolated_per_project_file() -> None:
    mark_notified("/proj-a.sase", "cl-1", "1")
    assert is_notified("/proj-b.sase", "cl-1", "1") is False
...
def test_mark_multiple_entries() -> None:
    mark_notified("/proj.sase", "cl-1", "1")
    mark_notified("/proj.sase", "cl-1", "2")
    mark_notified("/proj.sase", "cl-2", "1")
    assert is_notified("/proj.sase", "cl-1", "1") is True
    assert is_notified("/proj.sase", "cl-1", "2") is True
    assert is_notified("/proj.sase", "cl-2", "1") is True
    assert is_notified("/proj.sase", "cl-2", "2") is False
