...
def test_running_record_carries_custom_role_display_labels(
    fixture_root: Path,
    tmp_path: Path,
) -> None:
    meta_path = (
    ...
    assert rec.agent_meta is not None
    assert rec.agent_meta.agent_family_custom_role == {
        "id": "tester",
        "label": "TESTING",
        "done_label": "TESTED",
    }
    ...
    assert agent.custom_role_label == "TESTING"
    assert agent.custom_role_done_label == "TESTED"
