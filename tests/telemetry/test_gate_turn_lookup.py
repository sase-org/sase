...
def test_indexed_lookup_records_duration(
    ...
    assert (
        sample(reg, "sase_gate_turn_lookup_duration_seconds_count", {"path": "indexed"})
        == 1.0
    )
    assert sample(reg, "sase_gate_turn_lookup_fallbacks_total", {}) is None
...
def test_fallback_scan_records_duration_and_fallback_counter(
    ...
    assert (
        sample(
            reg, "sase_gate_turn_lookup_duration_seconds_count", {"path": "fallback"}
        )
        == 1.0
    )
    assert sample(reg, "sase_gate_turn_lookup_fallbacks_total", {}) == 1.0
