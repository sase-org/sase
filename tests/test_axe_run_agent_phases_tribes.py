...
def _extract_medium_model_meta(tmp_path: Path) -> dict[str, object]:
    workspace_dir = tmp_path / "workspace"
    artifacts_dir = tmp_path / "artifacts"
    workspace_dir.mkdir()
    artifacts_dir.mkdir()
...
def test_medium_directive_metadata_pins_concrete_lane(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    meta = _extract_medium_model_meta(tmp_path)
