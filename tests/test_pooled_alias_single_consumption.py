...
``@large`` pool that ``llm_provider.default_model`` delegates to. Covers the
core round-robin cursor contract: a pooled alias must advance its
machine-global cursor exactly once per launched agent, during bootstrap
reservation and never again during redemption, including batched weighted
schedules, concurrent redemption, and non-agent/non-pooled workflows that
must never touch the cursor.
"""
