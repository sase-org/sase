"""Session-scoped guard against system temporary-directory leakage.

The suite used to strand thousands of ``.lock``, ``-archive``, and bare
``tmpXXXXXXXX`` entries in the shared system temp directory. Once those leaks
were fixed, nothing stopped them from coming back silently, so this guard
snapshots the watched temp roots at session start, compares them at session
finish, and fails the run while naming the entries that appeared.
"""
...
def watched_temp_directories() -> tuple[Path, ...]:
    ...
    This is the temp directory the suite itself resolves, which is the
...
def format_leak_report(leaks: Mapping[Path, tuple[str, ...]]) -> str:
    ...
    lines = ["The test suite left new entries in the system temp directory:"]
