...
def test_wait_for_completion_marker_returns_none_after_all_retries() -> None:
    ...
    with (
        patch(
            "sase.ace.scheduler.hook_checks.check_hook_completion", return_value=None
        ) as mock_check,
        patch("sase.ace.scheduler.hook_checks.time.sleep") as mock_sleep,
    ):
...
def test_check_hooks_logs_warning_on_merge_failure() -> None:
    ...
    with (
        ...
        patch("sase.ace.scheduler.hook_checks.is_process_running", return_value=True),
        patch("sase.ace.scheduler.hook_checks.merge_hook_updates", return_value=False),
    ):
        updates, hooks_started = check_hooks(cs, log)
