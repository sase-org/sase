...
from sase.ace.changespec.locking import write_changespec_atomic
from sase.ace.changespec.models import ChangeSpec
from sase.ace.changespec.project_spec import ProjectSpec, serialize_project_spec
...
def test_find_all_changespecs_prefers_yaml(tmp_path: Path) -> None:
    ...
    proj_dir = tmp_path / ".sase" / "projects" / "myproj"
    ...
    with patch("sase.ace.changespec.Path.home", return_value=tmp_path):
...
def test_find_all_changespecs_skips_dir_with_no_project_file(tmp_path: Path) -> None:
    ...
    proj_dir = tmp_path / ".sase" / "projects" / "empty_proj"
    proj_dir.mkdir(parents=True)
