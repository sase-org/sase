...
from sase.ace.changespec.section_parsers import build_commit_entry
...
def test_build_commit_entry_all_fields() -> None:
    ...
    entry_dict: dict[str, str | int | None] = {
        "number": 1,
        "note": "Initial Commit",
        "chat": "~/.sase/chats/test.md",
        "diff": "~/.sase/diffs/test.diff",
    }
    entry = build_commit_entry(entry_dict)
    assert entry.number == 1
    assert entry.note == "Initial Commit"
    assert entry.chat == "~/.sase/chats/test.md"
    assert entry.diff == "~/.sase/diffs/test.diff"
...
def test_parse_changespec_with_history() -> None:
    ...
    lines = [
        ...
        "COMMITS:\n",
        "  (1) Initial Commit\n",
        "      | CHAT: ~/.sase/chats/test-251221130813.md\n",
        "      | DIFF: ~/.sase/diffs/test_251221130813.diff\n",
        "  (2) Added feature\n",
        "      | DIFF: ~/.sase/diffs/test_251221140000.diff\n",
        "\n",
    ]
    ...
    assert changespec.commits[0].chat == "~/.sase/chats/test-251221130813.md"
    assert changespec.commits[0].diff == "~/.sase/diffs/test_251221130813.diff"
    # Check second entry
    ...
    assert changespec.commits[1].diff == "~/.sase/diffs/test_251221140000.diff"
...
def test_parse_changespec_with_proposed_entries() -> None:
    ...
    lines = [
        ...
        "COMMITS:\n",
        "  (1) Initial Commit\n",
        "      | DIFF: ~/.sase/diffs/first.diff\n",
        "  (2) Second commit\n",
        "      | DIFF: ~/.sase/diffs/second.diff\n",
        "  (2a) First proposal\n",
        "      | DIFF: ~/.sase/diffs/proposal_a.diff\n",
        "  (2b) Second proposal\n",
        "      | CHAT: ~/.sase/chats/proposal_b.md\n",
        "      | DIFF: ~/.sase/diffs/proposal_b.diff\n",
        "\n",
    ]
    changespec, _ = _parse_changespec_from_lines(lines, 0, "/test/file.gp")
    assert changespec is not None
    ...
    assert changespec.commits[3].chat == "~/.sase/chats/proposal_b.md"
