...
- `synthesized-unknown-item.jsonl` is synthetic and exists to pin current parser behavior for an unknown item shape
  until a real MCP or provider-specific Codex event is available.

The current observed tool-relevant events are `item.started` and `item.completed` items with `type` values of
`command_execution` and `file_change`. Codex did not emit separate `function_call` items or separate result-output items
in the captured successful tool run; command output and exit status were present on completed `command_execution` items.
