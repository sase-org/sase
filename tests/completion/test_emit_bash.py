...
def test_emit_bash_registration_and_candidates_helper() -> None:
...
def test_candidates_helper_caches_by_kind_and_never_forwards_prefix() -> None:
    script = emit_bash(_spec())
    assert "__sase_run()" in script
    assert "declare -gA __sase_candidates_cache" in script
    assert "declare -gA __sase_candidates_stamp" in script
...
def test_kinded_call_site_passes_current_word_and_prefix() -> None:
    script = emit_bash(_spec())
    assert '__sase_candidates "${spec#kind:}" "${value_cur}" "${prefix}"' in script
