...
def test_default_config_matches_public_schema() -> None:
    ...
    default_config = with_machine_name(
        yaml.safe_load((REPO_ROOT / "src/sase/default_config.yml").read_text())
    )
...
def test_prompt_completion_word_min_length_schema_contract() -> None:
    ...
    Draft7Validator(public_schema).validate(
        with_machine_name({"ace": {"prompt_completion": {"word_min_length": 3}}})
    )
...
def test_config_schema_requires_valid_machine_name() -> None:
    validator = Draft7Validator(schema())

    with pytest.raises(ValidationError, match="machine_name.*required"):
        validator.validate({})
...
def test_config_schema_accepts_deprecated_activate_control_alias() -> None:
    Draft7Validator(schema()).validate(
        with_machine_name({"ace": {"keymaps": {"gate": {"activate_control": "f12"}}}})
    )
...
def test_config_schema_rejects_invalid_scoped_gate_keymaps(
    ...
    with pytest.raises(ValidationError):
        Draft7Validator(schema()).validate(
            with_machine_name({"ace": {"keymaps": {"gate": gate}}})
        )
...
def test_config_schema_rejects_invalid_scoped_statistics_keymaps(
    ...
    with pytest.raises(ValidationError):
        Draft7Validator(schema()).validate(
            with_machine_name({"ace": {"keymaps": {"statistics": statistics}}})
        )


def test_config_schema_accepts_max_running_agents() -> None:
    Draft7Validator(schema()).validate(with_machine_name({"max_running_agents": 1}))
    Draft7Validator(schema()).validate(with_machine_name({"max_running_agents": 25}))


def test_config_schema_rejects_max_running_agents_below_minimum() -> None:
    with pytest.raises(ValidationError):
        Draft7Validator(schema()).validate(with_machine_name({"max_running_agents": 0}))
...
def test_config_schema_accepts_positive_big_epic_phase_threshold() -> None:
    Draft7Validator(schema()).validate(
        with_machine_name({"bead": {"big_epic_phase_threshold": 1}})
    )
    Draft7Validator(schema()).validate(
        with_machine_name({"bead": {"big_epic_phase_threshold": 8}})
    )
...
def test_config_schema_rejects_invalid_big_epic_phase_threshold(
    ...
    with pytest.raises(ValidationError):
        Draft7Validator(schema()).validate(
            with_machine_name({"bead": {"big_epic_phase_threshold": value}})
        )
