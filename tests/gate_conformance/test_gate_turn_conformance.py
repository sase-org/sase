...
:func:`~sase.gate_turn.settlement.settle_gate_turn` pair; this test is what
...
def _build_gate_turn(project: str, request_id: str) -> GateTurnRecord:
    """Create a real gate bundle plus its gate-shell member, no live creator.

    Mirrors ``tests/test_gate_cli_answer_detach.py``'s ``_make_gate_turn_member``:
    ...
    # the setup.
    gate = create_gate(
        replace(GateSpec.from_mapping(_spec(request_id)), turn_row_managed=True)
    )
    turn = GateTurnSpec.from_mapping(_SHELL_BLOCK, branches=(("cleanup",),))
    ...
    record = read_gate_turn_marker(project, artifacts_dir)
...
def test_shell_gate_settles_identically_across_every_surface(
    ...
    monkeypatch.setattr("sase.gate_turn.followup.spawn_agent_subprocess", _fake_launch)

    outcomes: dict[str, dict[str, Any]] = {}
    for surface in SURFACES:
        project = f"proj-{surface.name}"
        request_id = f"conf-shell-{surface.name}"
        record = _build_gate_turn(project, request_id)
