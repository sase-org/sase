"""Tests for loop_runner_utils module."""
...
def test_check_for_local_changes_with_changes() -> None:
    ...
    with patch("sase.loop_runner_utils.subprocess.run") as mock_run:
...
def test_create_proposal_from_changes_no_changes() -> None:
    ...
    with patch("sase.loop_runner_utils._check_for_local_changes", return_value=False):
...
def test_create_proposal_from_changes_success() -> None:
    ...
    with (
        patch("sase.loop_runner_utils._check_for_local_changes", return_value=True),
        patch("sase.loop_runner_utils.save_chat_history", return_value="/path/chat.md"),
        patch("sase.loop_runner_utils.save_diff", return_value="/path/diff.patch"),
        patch(
            "sase.loop_runner_utils.add_proposed_commit_entry",
            return_value=(True, "abc123"),
        ),
        patch("sase.loop_runner_utils.clean_workspace"),
    ):
        proposal_id, exit_code = create_proposal_from_changes(
            project_file="/path/to/project.gp",
            cl_name="test_cl",
...
def test_create_proposal_from_changes_diff_fails() -> None:
    ...
    with (
        patch("sase.loop_runner_utils._check_for_local_changes", return_value=True),
        patch("sase.loop_runner_utils.save_chat_history", return_value="/path/chat.md"),
        patch("sase.loop_runner_utils.save_diff", return_value=None),
    ):
        proposal_id, exit_code = create_proposal_from_changes(
            project_file="/path/to/project.gp",
            cl_name="test_cl",
...
def test_finalize_loop_runner_success() -> None:
    """Test finalize_loop_runner calls all required functions."""
    mock_cs = MagicMock()
    ...
    with (
        patch(
            "sase.loop_runner_utils.parse_project_file",
            return_value=[mock_cs],
        ),
        patch("sase.loop_runner_utils.release_workspace") as mock_release,
    ):
        finalize_loop_runner(
...
def test_finalize_loop_runner_no_matching_changespec() -> None:
    """Test finalize_loop_runner skips suffix update when changespec not found."""
    mock_cs = MagicMock()
    ...
    with (
        ...
        patch("sase.loop_runner_utils.release_workspace"),
    ):
        finalize_loop_runner(
            project_file="/path/project.gp",
            changespec_name="test_cl",
...
def test_finalize_loop_runner_handles_errors() -> None:
    """Test finalize_loop_runner handles errors gracefully."""
    with (
        patch(
            "sase.loop_runner_utils.parse_project_file",
        ...
        patch(
            "sase.loop_runner_utils.release_workspace",
