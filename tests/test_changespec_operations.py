    ...
    try:
        with patch(
            ...
            result = add_changespec_to_project_file(
                ...
                initial_hooks=["!$sase_hg_presubmit", "sase_hg_lint"],
            )
        ...
        assert "  !$sase_hg_presubmit" in content
        assert "  sase_hg_lint" in content
    finally:
...
def test_add_changespec_hooks_in_correct_order() -> None:
    ...
    try:
        ...
        presubmit_idx = content.index("!$sase_hg_presubmit")
        lint_idx = content.index("sase_hg_lint")
...
"""
    ...
    try:
        ...
        assert child_hooks == [
            "!$sase_hg_presubmit",  # From initial_hooks
            "bb_rabbit_test //foo:child_test",  # From initial_hooks
            "sase_hg_lint",  # Inherited from parent
...
def test_add_changespec_no_parent_hooks_inherited_when_no_parent() -> None:
    ...
    try:
        ...
        assert child_hooks == ["!$sase_hg_presubmit"]
    finally:
        os.unlink(project_file)
