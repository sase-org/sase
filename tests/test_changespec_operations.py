    ...
    try:
        with patch(
            ...
            result = add_changespec_to_project_file(
                ...
                initial_hooks=["!$sase_presubmit", "sase_lint"],
            )
        ...
        assert "  !$sase_presubmit" in content
        assert "  sase_lint" in content
    finally:
...
def test_add_changespec_hooks_in_correct_order() -> None:
    ...
    try:
        ...
        presubmit_idx = content.index("!$sase_presubmit")
        lint_idx = content.index("sase_lint")
    ...
    try:
        with patch(
            ...
            result = add_changespec_to_project_file(
                ...
                # initial_hooks contains !$sase_presubmit (should NOT be duplicated)
                initial_hooks=["!$sase_presubmit"],
            )
        ...
        assert "!$sase_presubmit" in child_hooks  # From initial_hooks
        assert "sase_lint" in child_hooks  # Inherited from parent
        assert "bb_rabbit_test //foo:parent_test" in child_hooks  # Inherited

        # !$sase_presubmit should NOT be duplicated
        assert child_hooks.count("!$sase_presubmit") == 1
    finally:
        os.unlink(project_file)
    ...
    try:
        with patch(
            ...
            result = add_changespec_to_project_file(
                ...
                parent="parent_feature",
                cl_url="http://cl/22222",
                initial_hooks=["!$sase_presubmit", "bb_rabbit_test //foo:child_test"],
            )
        ...
        assert child_hooks == [
            "!$sase_presubmit",  # From initial_hooks
            "bb_rabbit_test //foo:child_test",  # From initial_hooks
            "sase_lint",  # Inherited from parent
...
def test_add_changespec_no_parent_hooks_inherited_when_no_parent() -> None:
    ...
    try:
        ...
        assert child_hooks == ["!$sase_presubmit"]
    finally:
        os.unlink(project_file)
