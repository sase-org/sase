...
from sase.ace.operations import get_workspace_directory
...
def test_running_field_get_workspace_directory_basic() -> None:
    ...
    with patch("sase.running_field.subprocess.run") as mock_run:
        ...
        assert args == ["sase_get_workspace", "myproject", "1"]
...
def test_running_field_get_workspace_directory_with_workspace_num() -> None:
    ...
    with patch("sase.running_field.subprocess.run") as mock_run:
        ...
        mock_run.assert_called_once()
        args = mock_run.call_args[0][0]
        assert args == ["sase_get_workspace", "myproject", "3"]
...
def test_running_field_get_workspace_directory_command_failure() -> None:
    ...
    with patch("sase.running_field.subprocess.run") as mock_run:
        mock_run.side_effect = subprocess.CalledProcessError(
            returncode=1, cmd=["sase_get_workspace", "myproject", "1"], stderr="Error"
        )
        with pytest.raises(RuntimeError, match="sase_get_workspace failed"):
...
def test_get_workspace_directory_for_num_main() -> None:
    ...
    with patch("sase.running_field.subprocess.run") as mock_run:
        ...
        # Verify sase_get_workspace was called correctly
        mock_run.assert_called_once()
        args = mock_run.call_args[0][0]
        assert args == ["sase_get_workspace", "myproject", "1"]
...
def test_get_workspace_directory_uses_running_field() -> None:
    ...
    try:
        ...
        with patch("sase.running_field.subprocess.run") as mock_run:
            ...
            # Verify sase_get_workspace was called with workspace 2
            mock_run.assert_called_once()
            args = mock_run.call_args[0][0]
            assert args[0] == "sase_get_workspace"
