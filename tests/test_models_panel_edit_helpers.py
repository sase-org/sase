...
def test_plan_alias_edit_unset(tmp_path: Path) -> None:
    ...
    assert "coder: opus" not in plan.new_text
...
def test_commit_offer_set_in_repo_with_changes(tmp_path: Path) -> None:
    ...
    assert offer.message.startswith("chore: Update model alias @coder")
...
def test_commit_offer_reset_uses_reset_verb(tmp_path: Path) -> None:
    ...
    offer = build_alias_commit_offer(str(target), op="unset", alias="coder")

    assert offer is not None
    assert offer.message.startswith("chore: Reset model alias @coder")
...
def test_commit_offer_not_in_repo_returns_none(tmp_path: Path) -> None:
    ...
    assert build_alias_commit_offer(str(target), op="set", alias="coder") is None
