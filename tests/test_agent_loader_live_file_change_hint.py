"""Tests for live workspace pencil-hint classification in the loader pass.

The loader's status-override pass (`_apply_status_overrides`) classifies the
Agents-tab pencil badge. For active agents that have edited their workspace but
have not persisted a ``diff_path`` yet, the pass precomputes a live hint off the
event loop so the row badge matches the detail pane's live Deltas.
"""
...
def test_active_agent_with_real_workspace_edits_gets_live_hint(
    tmp_path: Path,
) -> None:
...
def test_active_agent_with_bookkeeping_only_edits_omits_live_hint(
    tmp_path: Path,
) -> None:
    diff_mod._diff_cache.clear()
    workspace = _setup_workspace(tmp_path)
