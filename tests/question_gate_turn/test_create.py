"""The question gate turn's request spec and chain-metadata bookkeeping."""
...
from sase.question_gate_turn.create import (
    _question_gate_turn_spec,
...
def test_turn_block_pins_statuses_and_accents() -> None:
    spec = _question_gate_turn_spec(
        _questions(),
        session_id="round-1",
    ...
    turn = spec["turn"]
    assert turn["pending_status"] == "QUESTION"
    assert turn["settled_status"] == "ANSWERED"
    assert turn["accent"] == "#FFAF00"
    assert turn["workspace"] == "inherit"
    assert turn["next"] == {"fork": "session", "output": ["results"], "prompt": None}
    assert turn["branches"]["submit"]["status"] == "ANSWERED"
    assert turn["branches"]["submit"]["accent"] == "#5FD7FF"
    assert turn["branches"]["submit"]["output"] == ["results"]
    assert turn["branches"]["submit"]["fork"] == "session"
    assert "Implement the feature." in turn["branches"]["submit"]["prompt"]
    assert turn["branches"]["timeout"] == {
        "status": "QUESTION TIMED OUT",
        "accent": "#FFAF00",
        "prompt": None,
    }
    assert turn["branches"]["stopped"] == {
        "status": "QUESTION CANCELLED",
        "accent": "#FFAF00",
        "prompt": None,
    }
    assert turn["branches"]["failed"] == {
...
def test_turn_block_is_accepted_by_gate_turn_spec() -> None:
    spec = _question_gate_turn_spec(
        _questions(),
        session_id="round-1",
        base_prompt="Implement the feature.",
    ...
    parsed = GateTurnSpec.from_mapping(spec["turn"], branches=(("submit",),))
...
def test_default_timeout_is_24_hours() -> None:
    ...
    assert built.gate_timeout_seconds == GATE_TURN_DEFAULT_TIMEOUT_SECONDS
...
def _create_via_fake_transaction(
    ...
    *,
    ...
    """Exercise ``create_question_gate_turn`` without the creator-resolution path.

    ``create_gate_turn`` itself resolves the calling agent from
    ``SASE_AGENT_NAME``/``SASE_ARTIFACTS_DIR`` -- machinery already covered by
    ``tests/gate_turn/``. Faking it here isolates what this module owns: the
    request spec and the chain-metadata bookkeeping written after creation.
    """
    request = _question_gate_turn_spec(
        _questions(),
    ...
    shell = GateTurnSpec.from_mapping(request["turn"], branches=(("submit",),))
    ...
    fake_creation = GateTurnCreation(
        gate=gate, record=record, project_file=None, claim_move=None, cl_name=None
    )

    def _fake_create_gate_turn(_request: Any) -> GateTurnCreation:
        return fake_creation

    monkeypatch.setattr(
        "sase.question_gate_turn.create.create_gate_turn", _fake_create_gate_turn
    )
    return create_question_gate_turn(
        _questions(),
        session_id=session_id,
        base_prompt=base_prompt,
        prior_rounds=prior_rounds,
