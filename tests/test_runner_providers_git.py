...
class TestPrepareGitWorkspace:
    """Tests for prepare_git_workspace()."""
    ...
    def test_clean_workspace(self, mock_git: MagicMock) -> None:
        ...
        head = prepare_git_workspace("/tmp/ws")
        assert head == "abc123"
...
# ---------------------------------------------------------------------------
# capture_git_diff tests
# ---------------------------------------------------------------------------


class TestCaptureGitDiff:
    """Tests for capture_git_diff()."""
    ...
    def test_committed_changes(self, mock_git: MagicMock) -> None:
        ...
        result = capture_git_diff("/tmp/ws", "old-head")
    ...
    def test_uncommitted_changes(self, mock_git: MagicMock) -> None:
        ...
        result = capture_git_diff("/tmp/ws", "same-head")
        assert result is not None
        with open(result) as f:
    ...
    def test_empty_head_before(self) -> None:
        assert capture_git_diff("/tmp/ws", "") is None
...
class TestGitRunnerProvider:
    ...
    @patch(f"{_GIT_MOD}.prepare_git_workspace", return_value="abc123")
    ...
    @patch(f"{_GIT_MOD}.capture_git_diff", return_value="/tmp/diff.patch")
    ...
    @patch(f"{_GIT_MOD}.capture_git_diff", return_value=None)
    @patch(f"{_GIT_MOD}.release_workspace")
