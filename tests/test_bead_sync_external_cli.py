...
def test_output_shows_closed_and_reopened_counts_when_nonzero(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
) -> None:
    from sase.external_mirror.issues import MirrorReport
    ...
    assert "reopened=2" in out


def test_output_omits_closed_and_reopened_counts_when_zero(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
) -> None:
    from sase.external_mirror.issues import MirrorReport
...
def test_dry_run_prints_planned_status_transitions(
    monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture
) -> None:
    from sase.external_mirror.issues import MirrorReport
