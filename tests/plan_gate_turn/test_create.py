...
from sase.plan_gate_turn.create import create_plan_gate_turn, plan_gate_turn_block
...
def _sandbox_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "sase.plan_gate_turn.create.list_gate_turns", lambda **_kwargs: []
    )
    monkeypatch.setattr(
...
def test_tale_shell_block_covers_all_supported_approval_subsets() -> None:
    ...
    parsed = GateTurnSpec.from_mapping(
        shell,
...
def test_builtin_gate_turn_accents_match_agent_list_ladder_statuses() -> None:
    question_spec = _question_gate_turn_spec(
    ...
    parsed_shells = (
        GateTurnSpec.from_mapping(
            plan_gate_turn_block("tale"),
            branches=(("approve", "commit"), ("reject",), ("feedback",)),
            allow_branch_subsets=True,
        ),
        GateTurnSpec.from_mapping(
            plan_gate_turn_block("epic"),
...
def test_create_plan_gate_turn_records_context_before_auto_settlement(
    tmp_path: Path,
