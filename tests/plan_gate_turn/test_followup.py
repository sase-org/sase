...
from sase.plan_gate_turn.followup import (
...
def _write_feedback_shell(
    ...
    *,
    ...
    if original_prompt is not None:
        path = artifacts / "plan_gate_turn_original_prompt.md"
        path.write_text(original_prompt, encoding="utf-8")
        meta["plan_gate_turn_original_prompt_path"] = str(path)
    if qa_rounds is not None:
        qa_path = artifacts / "plan_gate_turn_qa_rounds.json"
        qa_path.write_text(
            json.dumps(
                [
                    {
        ...
        meta["plan_gate_turn_qa_rounds_path"] = str(qa_path)
    (artifacts / "agent_meta.json").write_text(json.dumps(meta), encoding="utf-8")
    return artifacts, response


def test_feedback_bullets_walk_settled_plan_gate_turn_chain(tmp_path: Path) -> None:
...
def test_accepted_tale_next_action_uses_shared_successor_preparation(
    ...
    bundle.mkdir()
    plan.write_text(VALID_TALE_PLAN, encoding="utf-8")
    current_prompt = artifacts / "plan_gate_turn_current_prompt.md"
    original_prompt = artifacts / "plan_gate_turn_original_prompt.md"
    ...
    meta: dict[str, Any] = {
        "gate_bundle_path": str(bundle),
        "gate_kind": "plan",
        "plan_gate_turn_project_name": "proj",
        "plan_gate_turn_project_file": str(tmp_path / "proj.sase"),
        "plan_gate_turn_workspace_dir": str(tmp_path),
        "plan_gate_turn_workspace_num": 1,
        "plan_gate_turn_output_path": str(tmp_path / "out.txt"),
        "plan_gate_turn_timestamp": "20260827T120000",
        "plan_gate_turn_artifacts_timestamp": "20260827_120000",
        "plan_gate_turn_vcs_tag": "#gh:sase ",
        "plan_gate_turn_agent_name": "agent",
        "plan_gate_turn_source_role_suffix": PLAN_CHAIN_PLAN_SUFFIX,
        "plan_gate_turn_agent_model": "gpt-5",
        "plan_gate_turn_agent_llm_provider": "openai",
        "plan_gate_turn_agent_vcs_provider": "github",
        "plan_gate_turn_agent_meta": {"model": "gpt-5"},
        "plan_gate_turn_current_prompt_path": str(current_prompt),
        "plan_gate_turn_original_prompt_path": str(original_prompt),
        "patch_name": "demo",
    }
    (artifacts / "agent_meta.json").write_text(json.dumps(meta), encoding="utf-8")
