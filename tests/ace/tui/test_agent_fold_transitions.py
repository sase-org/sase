"""Level transition table tests for the agents-tab fold mixin.

Covers the boundary between the global group fold (L0..L2) and the
existing per-workflow ``FoldStateManager``: ``l`` should always feel
like "show me one more level" — bumping the group fold up to L2 first,
then delegating to per-workflow expansion. ``h`` reverses, collapsing
the focused workflow before retreating up the group ladder. ``L``/``H``
...
def test_l_steps_group_fold_when_below_max() -> None:
    app = _StubApp([_agent()])
    ...
    # Past L2 the workflow fold logic kicks in but this agent isn't a
    # workflow — no further state change.
    app.action_expand_or_layout()
    assert app._group_fold_state.level == 2


def test_l_at_max_advances_per_workflow_fold() -> None:
    parent = _agent(raw_suffix="ts1", agent_type=AgentType.WORKFLOW)
    app = _StubApp([parent])
    app._group_fold_state.level = 2
...
def test_h_collapses_workflow_first_then_steps_group_down() -> None:
    ...
    # First `h` collapses the workflow (group level stays at 2).
...
def test_group_fold_changes_snap_focus_to_ancestor_banner() -> None:
    """Below max the focused agent is hidden — snap to nearest visible banner."""
    app = _StubApp([_agent()])
    app._group_fold_state.level = 2
    app.current_idx = 0

    app.action_hooks_or_collapse()  # 2 -> 1
    assert app._group_fold_state.level == 1
    # At L1 the agent is hidden — snap-to-ancestor populated current_group_key.
    assert app._current_group_key is not None
    # Returning to L2 clears it again.
    app.action_expand_or_layout()
    assert app._group_fold_state.level == 2
