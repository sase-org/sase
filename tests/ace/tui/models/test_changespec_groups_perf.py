"""Performance-correctness tests for Patch grouping.
...
(``keys_for_patches`` for the L0 label, ``walk_order`` for the date
anchor, ``enumerate_patch_group_keys`` for the registry GC pass) —
...
:func:`latest_patch_timestamp` from a monkey-patched module shim.
"""
...
from sase.ace.tui.models.patch_groups import _buckets, _keys
...
def _patch_parse_counter(monkeypatch: Any) -> list[int]:
    ...
    """
    counter = [0]
    real = _buckets.parse_timestamp_value

    def _counted(raw: str) -> Any:
        counter[0] += 1
        return real(raw)

    monkeypatch.setattr(_buckets, "parse_timestamp_value", _counted)
    return counter


def test_by_date_build_parses_each_cs_once(monkeypatch: Any) -> None:
    """One ``build_patch_tree`` call ⇒ one parse per CS in BY_DATE."""
    counter = _patch_parse_counter(monkeypatch)
    css = [_cs(f"cs_{i}", timestamps=[_ts("260426_120000")]) for i in range(20)]

    build_patch_tree(css, PatchGroupingMode.BY_DATE, now=_NOW)
...
def test_by_date_enumerate_parses_each_cs_once(monkeypatch: Any) -> None:
    """``enumerate_patch_group_keys`` shares the same one-pass cache."""
    counter = _patch_parse_counter(monkeypatch)
    css = [_cs(f"cs_{i}", timestamps=[_ts("260426_120000")]) for i in range(15)]

    enumerate_patch_group_keys(css, PatchGroupingMode.BY_DATE, now=_NOW)
...
def test_by_status_build_does_not_parse_timestamps(monkeypatch: Any) -> None:
    ...
    build_patch_tree(css, PatchGroupingMode.BY_STATUS, now=_NOW)
...
def test_walk_order_uses_supplied_latest_map(monkeypatch: Any) -> None:
    ...
    counter[0] = 0
    keys = _keys.keys_for_patches(
        css, PatchGroupingMode.BY_DATE, _NOW, latest_map=cache
    )
    counter[0] = 0
    _keys.walk_order(css, keys, PatchGroupingMode.BY_DATE, latest_map=cache)
    assert counter[0] == 0
