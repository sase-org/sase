"""Tree layout shape: project / Patch / name-root banners."""
...
# --- Three-level layout (panel has at least one Patch) ---


def test_single_agent_emits_project_and_patch_banner() -> None:
...
def test_two_agents_sharing_name_root_emit_three_banners() -> None:
    ...
    entries = build_agent_tree([a, b])
    # Project + Patch + name-root banners, then both agents.
    assert _kinds(entries) == [
        ("group", 0),
        ("group", 1),
        ("group", 2),
...
# --- Two-level fallback (no agent in the panel has a Patch) ---


def test_no_patch_panel_drops_to_two_level_layout() -> None:
    """When no agent has a Patch the renderer matches the pre-split shape."""
    a = _agent(cl_name="", project_file="/r/projA/proj.sase")
    entries = build_agent_tree([a])
    # Just project banner + agent — no Patch banner inserted.
...
def test_project_scoped_agent_does_not_emit_duplicate_patch_banner() -> None:
    """Project agents have a display cl_name but no real Patch bucket."""
...
def test_project_scoped_workflow_child_does_not_force_patch_level() -> None:
    parent = _agent(
        cl_name="home",
...
# --- Mixed-case bucket (some agents have Patch, some don't) ---


def test_mixed_panel_synthesizes_no_patch_bucket() -> None:
    """Agents without a Patch collect under a synthetic bucket."""
    a = _agent(cl_name="fix-a", project_file="/r/proj/proj.sase")
    b = _agent(cl_name="", project_file="/r/proj/proj.sase")
    entries = build_agent_tree([a, b])
    # Two Patch banners under the same project.
    cs_banners = [
        e
        for e in entries
    ...
    # Named Patch sorts before the synthetic bucket.
    assert suffixes == ["fix-a", ""]
    # And the synthetic bucket renders with the placeholder label.
    assert banner_label(cs_banners[1].group) == NO_PATCH_LABEL  # type: ignore[arg-type]
...
def test_mixed_panel_synthetic_bucket_is_independently_collapsible() -> None:
    ...
    # Patch banner is still expanded.
    cs_banners = [
        e
        for e in entries
        if e.kind == "group" and e.group is not None and e.group.level == 1
