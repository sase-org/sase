"""Tree shape and key enumeration for Patch grouping modes."""
...
from ._patch_groups_helpers import _NOW, _cs, _group_keys, _kinds
...
def test_empty_input_handled() -> None:
    assert build_patch_tree([], PatchGroupingMode.BY_PROJECT, now=_NOW) == []


def test_enumerate_keys_empty_for_empty_input() -> None:
    assert enumerate_patch_group_keys([], PatchGroupingMode.BY_PROJECT) == []
...
def test_by_project_enumerate_keys_includes_only_grouped_roots_at_l1() -> None:
    ...
    keys = enumerate_patch_group_keys(cl, PatchGroupingMode.BY_PROJECT)
    assert ("proj",) in keys
...
def test_by_date_emits_l0_banners_in_fixed_bucket_order() -> None:
    ...
    entries = build_patch_tree(cl, PatchGroupingMode.BY_DATE, now=_NOW)
...
def test_by_date_today_emits_hourly_groups_and_newest_first() -> None:
    ...
    cs_indices = [e.patch_idx for e in entries if e.kind == "patch"]
...
def test_by_date_collapsed_subgroup_hides_only_that_subgroup() -> None:
    ...
    entries = build_patch_tree(
        cl, PatchGroupingMode.BY_DATE, fold_registry=registry, now=_NOW
    )
    assert _kinds(entries) == [
        ("group", 0),
        ("group", 1),
...
def test_by_date_enumerate_keys_includes_l1_subgroup_keys() -> None:
    ...
    keys = enumerate_patch_group_keys(cl, PatchGroupingMode.BY_DATE, now=_NOW)
    assert keys == [
        ("Today",),
        ("Today", "10:00"),
        ("Yesterday",),
...
def test_by_status_l0_banners_in_display_order() -> None:
    ...
    entries = build_patch_tree(cl, PatchGroupingMode.BY_STATUS, now=_NOW)
    assert _group_keys(entries, 0) == [
        ("Ready",),
        ("WIP",),
        ("Draft",),
...
def test_by_status_enumerate_keys_includes_only_grouped_roots_at_l1() -> None:
    ...
    keys = enumerate_patch_group_keys(cl, PatchGroupingMode.BY_STATUS)
    assert ("WIP",) in keys
...
def test_by_status_collapsed_sibling_root_hides_only_its_members() -> None:
    ...
    entries = build_patch_tree(
        cl, PatchGroupingMode.BY_STATUS, fold_registry=registry, now=_NOW
    )
    assert _kinds(entries) == [
        ("group", 0),
        ("patch", 2),
...
def test_by_status_uses_literal_status_for_heading_label() -> None:
    ...
    keys = _group_keys(build_patch_tree(cl, PatchGroupingMode.BY_STATUS, now=_NOW), 0)
...
def test_collapsed_group_suppresses_descendants_and_leaves_siblings_alone() -> None:
    ...
    entries = build_patch_tree(
        cl, PatchGroupingMode.BY_PROJECT, fold_registry=registry, now=_NOW
    )
...
def test_default_fold_registry_renders_everything_expanded() -> None:
    ...
    assert _kinds(entries) == [("group", 0), ("patch", 0)]
