"""Current-project seeding precedence for the Snippets panel's initial load."""

from __future__ import annotations

import pytest

from sase.ace.tui.modals import snippets_panel_load as spl
from sase.current_project import CurrentProject

from .snippets_panel_test_helpers import project_ref, project_snapshot
...
def _install(
    ...
    *,
    ...
    monkeypatch.setattr(spl, "build_snippet_project_ring", lambda _workspace: ring)
    monkeypatch.setattr(
        spl,
    ...
    monkeypatch.setattr(spl, "resolve_current_project", fake_resolve)
...
def test_seeds_from_current_project_when_no_initial_key(
    ...
    result = spl.load_snippets_panel_initial_state(
...
def test_current_project_resolve_failure_falls_back_to_default(
    ...
    monkeypatch.setattr(spl, "resolve_current_project", raising_resolve)
...
def test_empty_ring_returns_no_snapshot(monkeypatch: pytest.MonkeyPatch) -> None:
    _install(monkeypatch, ring=(), current_project=None)
