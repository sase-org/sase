"""Rendering helper tests for the artifact-file selection modal."""
...
from sase.ace.tui.modals.artifact_files_modal import _artifact_file_option_text
from sase.core.artifact_file_facade import list_artifact_files
from tests.ace.tui.modals.artifact_files_modal_test_helpers import _artifact


def test_artifact_file_modal_unmarked_option_text_omits_empty_checkbox() -> None:
    plain = _artifact_file_option_text("1", _artifact(1), marked=False).plain

    assert "[ ]" not in plain


def test_artifact_file_modal_marked_option_text_displays_selected_marker() -> None:
    plain = _artifact_file_option_text("1", _artifact(1), marked=True).plain

    assert "[x]" in plain


def test_artifact_file_modal_option_text_truncates_long_label_and_path() -> None:
    artifact = _artifact(
        1,
        label="label-" + ("x" * 90),
    ...
    plain = _artifact_file_option_text("1", artifact).plain
...
def test_artifact_file_modal_option_text_displays_home_relative_path(
    tmp_path: Path,
...
def test_artifact_file_modal_option_text_prefers_workspace_relative_path(
    tmp_path: Path,
) -> None:
...
def test_artifact_file_modal_displays_committed_plan_workspace_relative_path(
    tmp_path: Path,
) -> None:
    ...
    plain = _artifact_file_option_text("1", artifacts[0]).plain
...
def test_artifact_file_modal_option_text_renders_agent_prefix() -> None:
    plain = _artifact_file_option_text(
        "1",
...
def test_artifact_file_modal_option_text_displays_pdf_markdown_source_path(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "workspace"
