...
def test_handler_passes_launch_workspace_from_prompt_context() -> None:
    harness = _GlossaryOpenHarness(prompt_context=_prompt_context(workspace="/ws/sase"))
...
def test_handler_skips_home_workspace() -> None:
    harness = _GlossaryOpenHarness(prompt_context=_prompt_context(home=True))
...
async def test_gg_opens_panel_and_escape_restores_normal_focus(
    ...
    app = _GlossaryOpenApp("solo draft")
...
async def test_ctrl_g_g_from_insert_restores_insert_mode(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _stub_panel_load(monkeypatch)
    app = _GlossaryOpenApp("solo draft")
