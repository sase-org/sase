"""Tests for off-UI-thread agent artifact discovery."""
...
class _DiscoveryApp(AgentPanelArtifactMixin):
...
async def test_cached_agent_artifacts_returns_none_on_miss(tmp_path: Path) -> None:
    app = _DiscoveryApp(None)
    agent = _DiscoveryAgent(tmp_path)

    assert app._cached_agent_artifacts(agent) is None  # type: ignore[arg-type]


async def test_cached_agent_artifacts_returns_cached_copy(tmp_path: Path) -> None:
    app = _DiscoveryApp(None)
    agent = _DiscoveryAgent(tmp_path)
    ...
    app._agent_artifact_page_cache[row_key] = [artifact]

    cached = app._cached_agent_artifacts(agent)  # type: ignore[arg-type]
...
async def test_schedule_discovery_runs_off_event_loop_and_refreshes_footer(
    ...
    app._schedule_agent_artifacts_discovery(agent)  # type: ignore[arg-type]
    inflight = app._agent_artifact_discovery_inflight
...
async def test_schedule_discovery_dedupes_in_flight_calls(
    ...
    assert len(app._agent_artifact_discovery_inflight) == 1
    task = next(iter(app._agent_artifact_discovery_inflight.values()))
    await task

    assert call_count == 1
    assert app._agent_artifact_discovery_inflight == {}
...
async def test_discovery_continuation_skips_stale_selection(
    ...
    app._schedule_agent_artifacts_discovery(agent_a)  # type: ignore[arg-type]
...
async def test_cancel_pending_artifact_discovery_cancels_inflight_tasks(
    ...
    app._run_agent_artifact_discovery = stub  # type: ignore[method-assign]

    app._schedule_agent_artifacts_discovery(agent)  # type: ignore[arg-type]
    await started.wait()
    task = next(iter(app._agent_artifact_discovery_inflight.values()))

    app._cancel_pending_artifact_discovery()
