...
no platform font stack is consulted. Comparison is byte-exact by default; explicit area and alpha-aware color-intensity
overrides are available for bounded non-canonical-platform drift. See the Visual Snapshot Workflow in
`docs/development.md` for the comparison and regeneration contract.
...
- `FiraCode-Regular.ttf`, `FiraCode-Bold.ttf` — Fira Code 6.2 (https://github.com/tonsky/FiraCode/releases/tag/6.2),
  ...
  `just update-visual-snapshots` on Linux and refreshing their hashes in `renderer_env.json`.
