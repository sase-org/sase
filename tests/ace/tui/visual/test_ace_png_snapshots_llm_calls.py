"""sase's TUI PNG visual snapshots for the populated LLM Calls panel.
...
def _llm_calls_agent(artifacts_dir: Path) -> Agent:
    return Agent(
        agent_type=AgentType.RUNNING,
        cl_name="visual-llm-calls",
        project_file="/workspace/sase/visual_project.sase",
        status="DONE",
        start_time=datetime(2026, 5, 9, 10, 0, 0),
        stop_time=datetime(2026, 5, 9, 10, 5, 0),
        raw_suffix="20260509-100000-llm-calls",
        agent_name="llm-calls",
        llm_provider="codex",
        model="gpt-5.5",
...
def _populate_tool_calls(artifacts_dir: Path) -> None:
    ...
    records.extend(
        _codex_pair(
            ...
            status="success",
            tool_input_summary={"file_path": "src/sase/ace/tui/llm_calls/reader.py"},
...
def _populate_expanded_tool_calls(artifacts_dir: Path) -> None:
    ...
    records.append(
        _codex_pending(
            ...
            recorded_at="2026-05-09T14:01:30+00:00",
            tool_input_summary={
                "file_path": "src/sase/ace/tui/widgets/llm_calls_panel.py",
...
def _pin_llm_calls_panel_now(monkeypatch: pytest.MonkeyPatch) -> None:
    """Pin configured-local clocks in the LLM Calls panel to a fixed wall-clock.
...
def _clear_llm_calls_cache() -> None:
    llm_calls_panel_module._llm_calls_cache.clear()


async def _wait_for_llm_calls_loaded(page: AcePage) -> None:
...
async def _open_llm_calls_panel(page: AcePage) -> AgentLLMCallsPanel:
...
async def test_agents_llm_calls_panel_populated_png_snapshot(
    ...
    async with AcePage(query='"visual"', patches=patches()) as page:
        ...
        ace_png_visual.assert_page_png(
            page,
            "agents_llm_calls_panel_populated_120x40",
            title="ACE agents LLM Calls panel populated with Codex rows",
        )
