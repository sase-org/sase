"""Direct tests for ``PatchList.patch_patch_row``.
...
def _seed_flat_patch_state(widget: PatchList, cs: Any) -> None:
    ...
    widget._option_idx_by_patch_name = {cs.name: 0}
...
def test_patch_returns_false_before_initial_render(monkeypatch: Any) -> None:
    ...
    cs = make_patch(name="alpha")
    assert widget.patch_patch_row(0, cs, selected=True, marked=False) is False
...
def test_patch_refuses_when_banner_rows_offset_indices(monkeypatch: Any) -> None:
    ...
    path's ``option_count == len(self._patches)`` gate is never
    satisfied on the Patches tab.  Single-row patches always fall back to a
    full :meth:`update_list` rebuild."""
    widget, _ = _wire_widget(monkeypatch)
    cs0 = make_patch(name="alpha")
    cs1 = make_patch(name="beta")
    widget.update_list([cs0, cs1], current_idx=0)

    ok = widget.patch_patch_row(0, cs0, selected=True, marked=True)
    assert ok is False
...
def test_patch_unmarks_row_falls_back(monkeypatch: Any) -> None:
    ...
    ok = widget.patch_patch_row(0, cs0, selected=True, marked=False)
    assert ok is False
...
def test_patch_falls_back_when_name_drifts(monkeypatch: Any) -> None:
    ...
    other = make_patch(name="zulu")  # different name at same idx
    ok = widget.patch_patch_row(0, other, selected=True, marked=False)
    assert ok is False
...
def test_patch_falls_back_when_idx_out_of_range(monkeypatch: Any) -> None:
    ...
    ok = widget.patch_patch_row(5, cs0, selected=True, marked=False)
    assert ok is False
...
def test_patch_records_optimal_target_width(monkeypatch: Any) -> None:
    ...
    assert widget._option_idx_by_patch_name == {"alpha": 0}
