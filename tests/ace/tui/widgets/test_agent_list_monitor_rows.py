...
def test_running_custom_pair_renders_bold_accent() -> None:
    ...
    assert "✓" not in left.plain
    assert _style_at(left, left.plain.index("TESTING")) == f"bold {_TESTING_ACCENT}"
...
def test_completed_custom_pair_renders_accent_and_check_glyph() -> None:
    ...
    assert _style_at(left, left.plain.index("TESTED")) == _TESTING_ACCENT
    assert _style_at(left, left.plain.index("✓")) == _TESTING_ACCENT
...
def test_failure_states_render_red_and_keep_existing_markers(
    ...
    assert _style_at(left, left.plain.index("TESTED")) == MONITOR_STATUS_FAILURE_STYLE
...
def test_different_pairs_render_different_styles() -> None:
    ...
    testing_style = _style_at(testing, testing.plain.index("TESTING"))
    sleeping_style = _style_at(sleeping, sleeping.plain.index("SLEEPING"))
...
def test_same_pair_renders_the_same_style_on_two_rows() -> None:
    ...
    assert _style_at(first, first.plain.index("TESTING")) == _style_at(
...
def test_starting_monitor_member_keeps_starting_style() -> None:
    ...
    assert _style_at(left, left.plain.index("STARTING")) == "bold #87D7FF"
...
def test_mirrored_family_container_uses_the_same_style_as_the_monitor() -> None:
    ...
    assert _style_at(monitor_left, monitor_left.plain.index("TESTING")) == _style_at(
