"""Pilot block-spread and deck-spread navigation tests behind card_blocks."""
...
async def test_flag_off_spread_unchanged() -> None:
    app = _DetailApp()
    async with app.run_test(size=(100, 30)) as pilot:
        await pilot.pause()
        _pin(app, deck=0, blocks=1000)
        ...
        with override_flags(card_blocks=False):
            panel.show_main_document(
                _document("s1", _reply_card(3), digest="spread-8"), "reply"
            )
            await pilot.pause()
