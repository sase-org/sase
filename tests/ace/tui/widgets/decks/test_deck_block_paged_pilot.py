"""Pilot block-paged projection tests behind the card_blocks beta flag."""
...
async def test_streaming_new_shell_following_advances() -> None:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        with override_flags(card_blocks=True):
            ...
            assert view.active_block_id("reply") == "b2"
            panel.show_main_document(
                _document("s1", _reply_card(4), digest="d2"), "reply"
            )
            await pilot.pause()
...
async def test_subject_reset_on_new_subject() -> None:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        with override_flags(card_blocks=True):
            ...
            assert panel.cycle_block(-1) is True
            panel.show_main_document(
                _document("s2", _reply_card(3), digest="d3"), "reply"
            )
            await pilot.pause()
            view = panel.main_view
            assert view.active_block_id("reply") == "b2"
...
async def test_split_panels_keep_independent_cursors() -> None:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        with override_flags(card_blocks=True):
            left.show_main_document(
                _document("s1", _reply_card(3), digest="d1"), "reply"
            )
            right.show_main_document(
                _document("s1", _reply_card(3), digest="d1"), "reply"
            )
            await pilot.pause()
...
async def test_cycle_focused_card_block_sets_preferred() -> None:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        with override_flags(card_blocks=True):
            ...
            assert panel.main_view.active_block_id("reply") == "b1"
            assert (
                detail.deck_area.state.panels[panel.panel_index].preferred_card
                == "reply"
            )
...
async def test_flag_off_renders_legacy_card() -> None:
    app = _DetailApp()
    async with app.run_test(size=(100, 30)) as pilot:
        await pilot.pause()
        _pin(app)
        ...
        with override_flags(card_blocks=False):
            ...
            assert active == "reply"
            view = panel.main_view
            assert view._last_render_key == ("d1", "reply", False, "paged")
            assert view._block_cursors == {}
            assert panel.card_blocks_navigable is False
            assert panel.cycle_block(-1) is False
            assert panel.select_block("b0") is False
            assert view.active_block_id("reply") is None
            assert panel.block_mode_for_active_card() is None
            # The whole card still renders: hidden blocks are visible text.
            corpus = deck_search_corpus(panel)
            assert "block-0-line-0" in corpus
