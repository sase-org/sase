...
def test_parent_does_not_aggregate_non_followup_child_artifacts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    home = tmp_path / "home"
    home.mkdir()
    ...
    followup_source = child_dir / "followup_prompt.md"
    followup_source.write_text("feedback body", encoding="utf-8")
    followup_artifact = store_explicit_agent_artifact(
        followup_source,
        child_dir,
        label="Full feedback prompt",
        kind="markdown",
    ...
    other_artifact = store_explicit_agent_artifact(
        other_source,
        child_dir,
        label="Child report",
        kind="markdown",
