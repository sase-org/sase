...
from sase.ace.tui.graphics._viewer_launch import _agent_artifact_view_spec
...
def test_agent_artifact_view_spec_rerenders_markdown_pdf_source(
    tmp_path: Path,
) -> None:
    ...
    spec = _agent_artifact_view_spec(
        _Artifact(path=str(pdf), kind="pdf", source_path=str(source))
    )

    assert spec == ArtifactViewSpec(source, "markdown")


def test_agent_artifact_view_spec_keeps_pdf_when_source_is_unusable(
    tmp_path: Path,
) -> None:
    pdf = tmp_path / "artifacts" / "markdown_pdfs" / "report.pdf"
    ...
    assert _agent_artifact_view_spec(_Artifact(path=str(pdf), kind="pdf")) == (
        ArtifactViewSpec(str(pdf), "pdf")
    )
    assert _agent_artifact_view_spec(
...
def test_view_agent_artifacts_passes_normalized_specs_to_viewer_loop(
    tmp_path: Path,
    monkeypatch,
) -> None:
    pdf = tmp_path / "artifacts" / "markdown_pdfs" / "report.pdf"
    ...
    assert calls == [
        (
            ArtifactViewSpec(source, "markdown"),
            ArtifactViewSpec(str(image), "image"),
        )
    ]


def test_view_agent_artifacts_tmux_pane_uses_normalized_module_command(
    tmp_path: Path,
    monkeypatch,
) -> None:
    pdf = tmp_path / "artifacts" / "markdown_pdfs" / "report.pdf"
    ...
    result = view_agent_artifacts_in_tmux_pane(
        (_Artifact(path=str(pdf), kind="pdf", source_path=str(source)),)
    )
...
def test_normalized_markdown_pdf_reaches_renderer_with_image_area(
    ...
    monkeypatch.setattr(
        "sase.ace.tui.graphics._viewer_loop.render_artifact_pages",
