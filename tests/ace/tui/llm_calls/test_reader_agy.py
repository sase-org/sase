"""Parity gate: the LLM Calls panel stays provider-neutral for ``agy``.
...
LLM Calls panel stays on the provider-neutral artifact contract and must not learn
...
def test_agy_run_without_tool_calls_artifact_shows_nothing(tmp_path: Path) -> None:
    ...
    present it returns ``None`` (the LLM Calls panel shows nothing), proving there is
...
def test_llm_calls_panel_contract_is_unchanged_for_agy_runtime(tmp_path: Path) -> None:
