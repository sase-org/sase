...
def test_evaluate_name_no_substring_match(
    ...
    assert evaluate_query(query, make_patch.create(name="foo_bar")) is False
    assert evaluate_query(query, make_patch.create(name="bar_foo")) is False
    assert evaluate_query(query, make_patch.create(name="foobar")) is False
    assert evaluate_query(query, make_patch.create(name="foo")) is True
...
def test_evaluate_name_case_insensitive(
    ...
    assert evaluate_query(query, make_patch.create(name="myfeature")) is True
    assert evaluate_query(query, make_patch.create(name="MYFEATURE")) is True
    assert evaluate_query(query, make_patch.create(name="myfeature_2")) is False
...
def test_evaluate_sibling_combined_with_status(
    ...
    cs1 = make_patch.create(name="feature", status="Draft")
    assert evaluate_query(query, cs1) is True

    cs2 = make_patch.create(name="feature__2", status="Draft")
    assert evaluate_query(query, cs2) is True

    cs3 = make_patch.create(name="feature__3", status="Mailed")
    assert evaluate_query(query, cs3) is False

    cs4 = make_patch.create(name="other_feature", status="Draft")
