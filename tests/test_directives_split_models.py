...
def test_split_prompt_for_models_three_models() -> None:
    ...
    assert result[0] == "%id:foo.cld_opus\n%model:opus\nDo work"
    assert result[1] == "%id:foo.cld_sonnet\n%model:sonnet\nDo work"
    assert result[2] == "%id:foo.cld_haiku\n%model:haiku\nDo work"
...
def test_split_prompt_for_models_scalar_alias_kwargs_are_not_a_fanout_axis() -> None:
    assert (
        split_prompt_for_models("%m(opus, medium_phase_worker=sonnet)\nDo work") is None
    )
...
def test_split_prompt_for_models_preserves_alias_kwargs_per_alt_branch() -> None:
    variants = split_prompt_for_models(
        ...
        "%m(haiku, medium_phase_worker=opus)}\nDo work"
    )
...
def test_split_prompt_for_models_xprompt_model_axis_composes_with_alts() -> None:
    ...
    prompt = (
        "%i:foo Describe this repo %(briefly, in detail). "
