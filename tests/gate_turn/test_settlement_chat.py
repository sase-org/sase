"""Settle-time decision record and chat file written by ``settle_gate_turn``."""
...
def _make_gate_turn_member(gate_home: Path, request_id: str, bundle_path: Path) -> str:
...
def test_settle_gate_turn_writes_decision_record_and_chat(
    gate_home: Path,
) -> None:
    request_id = "reclaim-1"
    gate = create_gate(_spec(request_id))
    artifacts_dir = _make_gate_turn_member(gate_home, request_id, gate.bundle_path)
    ...
    settled = settle_gate_turn(record, gate_state="answered", reason="gate answered")
...
def test_settle_gate_turn_writes_chat_when_branch_helper_unavailable(
    gate_home: Path,
) -> None:
...
def test_settle_gate_turn_is_idempotent_once_terminal(gate_home: Path) -> None:
    ...
    first = settle_gate_turn(record, gate_state="answered", reason="gate answered")
    again = settle_gate_turn(first, gate_state="answered", reason="gate answered")
