...
import sase.gate_turn as gate_turn_package
...
@pytest.mark.parametrize(
    ("name", "target"),
    sorted(gate_turn_package._LAZY_EXPORTS.items()),
)
def test_every_lazy_export_resolves(name: str, target: tuple[str, str]) -> None:
    ...
    assert getattr(gate_turn_package, name) is not None


def _record(gate_state: GateTurnState = "pending") -> GateTurnRecord:
...
def test_maybe_handoff_gate_from_agent_clears_intent_on_handoff_error(
    ...
    with pytest.raises(GateTurnError, match="pending handoff already exists"):
