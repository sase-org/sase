"""Shared builders for gate-turn settlement follow-up tests."""
...
def gate_spec(
    request_id: str, *, turn: dict[str, Any] | None
    ...
    if turn is not None:
        spec["turn"] = turn
        # These tests establish the gate-turn row themselves with
        # ``make_gate_turn_member``: mark the spec the way the production
        # transaction does so the turn-row guard accepts the setup.
        return replace(GateSpec.from_mapping(spec), turn_row_managed=True)
    return spec


def make_gate_turn_member(
    request_id: str,
    bundle_path: Path,
    *,
    turn: dict[str, Any],
    ...
    """Build the gate-turn member from the *same* turn block as the bundle.
    ...
    test that wants a resolvable policy must give both the same turn block.
    """
