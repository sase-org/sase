...
from sase.gate_turn.models import GateTurnLaneError, GateTurnRecord
from sase.gate_turn.start_claim import GateClaimMove
...
def _record(tmp_path: Path, state: str = "pending") -> GateTurnRecord:
...
def _install_transaction_fakes(
    ...
    *,
    ...
    monkeypatch.setattr(transaction_module, "has_any_gate_turn", lambda *_: False)
    monkeypatch.setattr(
        transaction_module,
    ...
    monkeypatch.setattr(
        transaction_module, "restore_gate_turn_claim", lambda *_, **__: None
    )
    monkeypatch.setattr(
        transaction_module,
        "create_gate",
...
def test_clean_lane_error_clears_intent_marker(
    ...
    monkeypatch.setattr(
        transaction_module,
        "_resolve_creator",
        lambda _project: (_ for _ in ()).throw(GateTurnLaneError("clean failure")),
    )

    with pytest.raises(GateTurnLaneError, match="clean failure"):
        create_gate_turn(_shell_gate_spec())
...
def test_finalizer_owned_turn_writes_no_intent(
    ...
    with pytest.raises(transaction_module.GateTurnError, match="host finalizer turn"):
        create_gate_turn(_shell_gate_spec())
