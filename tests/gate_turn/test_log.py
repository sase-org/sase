...
from sase.gate_turn.log import (
    _append_gate_turn_log_text as append_gate_turn_log_text,
    _gate_turn_log_path as gate_turn_log_path,
    _gate_turn_queue_weight as gate_turn_queue_weight,
    bind_gate_turn_execution_callbacks,
    gate_turn_output_tail,
)
...
def test_gate_turn_queue_weight_accepts_explicit_zero() -> None:
    assert (
        gate_turn_queue_weight({"queue_weight": 0.0, "queue_weight_explicit": True})
        == 0.0
    )


def test_gate_turn_queue_weight_rejects_implicit_zero() -> None:
    with pytest.raises(RuntimeError, match="Invalid queue_weight"):
        gate_turn_queue_weight({"queue_weight": 0.0, "queue_weight_explicit": False})


def test_append_gate_turn_log_text_appends_bounded(artifacts_dir: str) -> None:
    append_gate_turn_log_text(artifacts_dir, "$ commands/cleanup\n")
    append_gate_turn_log_text(artifacts_dir, "line one\n")
    append_gate_turn_log_text(artifacts_dir, "")

    text = gate_turn_log_path(artifacts_dir).read_text(encoding="utf-8")
...
def test_bind_execution_callbacks_streams_command_header_and_lines(
    ...
    callbacks = bind_gate_turn_execution_callbacks(artifacts_dir)
...
def test_bind_execution_callbacks_claims_gate_capacity_before_command(
    ...
    callbacks = bind_gate_turn_execution_callbacks(str(directory))
    callbacks.on_command_start("option", "cleanup", "Cleanup", ("commands/cleanup",))
    ...
    assert gate_turn_log_path(str(directory)).read_text(encoding="utf-8") == (
        "$ commands/cleanup\n"
    )
...
def test_gate_turn_output_tail_reads_back_the_newest_lines(artifacts_dir: str) -> None:
    for index in range(5):
        append_gate_turn_log_text(artifacts_dir, f"line {index}\n")

    assert gate_turn_output_tail(artifacts_dir, lines=2) == "line 3\nline 4\n"


def test_gate_turn_output_tail_is_empty_before_any_output(artifacts_dir: str) -> None:
    assert gate_turn_output_tail(artifacts_dir) == ""
