...
# ---------------------------------------------------------------------------
# PlanApprovalModal E-key behavior
# ---------------------------------------------------------------------------


def test_modal_epic_hint_shown_when_enabled() -> None:
    """Epic hint appears in compose output when show_epic=True."""
    from sase.ace.tui.modals.plan_approval_modal import PlanApprovalModal

    modal = PlanApprovalModal.__new__(PlanApprovalModal)
    modal._plan_file = "/tmp/plan.md"
    modal._feedback_mode = False
    modal._show_epic = True
    # The compose method builds hints string; verify the flag is stored
    assert modal._show_epic is True


def test_modal_epic_hint_hidden_when_disabled() -> None:
    """Epic hint should not appear when show_epic=False."""
    from sase.ace.tui.modals.plan_approval_modal import PlanApprovalModal

    modal = PlanApprovalModal.__new__(PlanApprovalModal)
    modal._plan_file = "/tmp/plan.md"
    modal._feedback_mode = False
    modal._show_epic = False
    assert modal._show_epic is False


def test_modal_action_epic_noop_when_disabled() -> None:
    """action_epic should be a no-op when show_epic is False."""
    from sase.ace.tui.modals.plan_approval_modal import PlanApprovalModal
    ...
    modal._feedback_mode = False
    modal._show_epic = False
    # action_epic should return without calling dismiss
    # (dismiss would fail since we're not in a running app)
    modal.action_epic()  # Should not raise


def test_modal_action_epic_noop_in_feedback_mode() -> None:
    """action_epic should be a no-op when in feedback mode."""
    from sase.ace.tui.modals.plan_approval_modal import PlanApprovalModal

    modal = PlanApprovalModal.__new__(PlanApprovalModal)
    modal._plan_file = "/tmp/plan.md"
    modal._feedback_mode = True
    modal._show_epic = True
    modal.action_epic()  # Should not raise
