...
def test_get_last_accepted_history_entry_id_skips_proposals(
    ...
    cs = make_patch.create(commits=commits)
    # Should return "2" (the last all-numeric), not "2b"
    assert get_last_accepted_history_entry_id(cs) == "2"


def test_get_last_accepted_history_entry_id_all_proposals(make_patch: Any) -> None:
...
def test_get_history_entry_by_id_none_commits(make_patch: Any) -> None:
    """Test get_history_entry_by_id returns None when commits is None."""
    cs = make_patch.create(commits=None)
    assert get_history_entry_by_id(cs, "1") is None


def test_get_history_entry_by_id_not_found(make_patch: Any) -> None:
...
def test_get_history_entry_by_id_proposal(make_patch: Any) -> None:
