...
def test_is_monitor_member_record_requires_role_and_monitor_id() -> None:
    ...
    valid = _record(
        agent_meta=AgentMetaWire(
            name="acme--mon",
            agent_session_role="monitor",
            agent_session_turn=AgentSessionTurnWire(kind="monitor", id="abc123"),
        )
    )
    other_role = _record(
        agent_meta=AgentMetaWire(
...
def test_from_record_prefers_done_marker_over_running_meta() -> None:
    meta = AgentMetaWire(
        ...
        agent_session_turn=AgentSessionTurnWire(
            ...
            id="abc123",
            state="running",
            monitor=AgentSessionTurnMonitorWire(command="sh -c 'exit 3'"),
        ),
    )
    done = DoneMarkerWire(
        agent_session_turn=AgentSessionTurnWire(
            kind="monitor",
            state="failed",
            monitor=AgentSessionTurnMonitorWire(exit_code=3),
        )
    )
...
def test_from_record_treats_unsettled_terminal_meta_as_active() -> None:
    meta = AgentMetaWire(
        ...
        agent_session_turn=AgentSessionTurnWire(
            ...
            id="abc123",
            state="completed",
            monitor=AgentSessionTurnMonitorWire(command="true", settled=False),
        ),
    )
    record = MonitorRecord.from_record(_record(agent_meta=meta))
...
def test_from_record_uses_settled_meta_without_done_marker() -> None:
    meta = AgentMetaWire(
        ...
        agent_session_turn=AgentSessionTurnWire(
            ...
            monitor=AgentSessionTurnMonitorWire(command="true", settled=True),
        ),
    )
    record = MonitorRecord.from_record(_record(agent_meta=meta))
...
def test_from_record_preserves_a_zero_exit_code() -> None:
    meta = AgentMetaWire(
        ...
        agent_session_turn=AgentSessionTurnWire(
            ...
            id="abc123",
            state="running",
            monitor=AgentSessionTurnMonitorWire(command="true", exit_code=0),
        ),
    )
    record = MonitorRecord.from_record(_record(agent_meta=meta))
