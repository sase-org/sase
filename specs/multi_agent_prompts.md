...
Can you help me add support for any sase agent prompt (ex: those started from the `sase ace` TUI or
  from `sase run` or `sase run --daemon`) for triggering multiple agents from a single prompt?
